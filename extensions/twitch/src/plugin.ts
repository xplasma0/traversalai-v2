/**
 * Twitch channel plugin for TraversalAI.
 *
...
import { buildChannelConfigSchema } from "traversalai/plugin-sdk";
 ...
 * for TraversalAI. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: TraversalAIConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: TraversalAIConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: TraversalAIConfig): boolean => {
