...
import type { ReplyPayload, TraversalAIConfig } from "traversalai/plugin-sdk";
import { createReplyPrefixOptions } from "traversalai/plugin-sdk";
...
export type TwitchMonitorOptions = {
  ...
  accountId: string;
  config: unknown; // TraversalAIConfig
...
async function processTwitchMessage(params: {
  ...
  const cfg = config as TraversalAIConfig;
