...
import type { TraversalAIPluginApi, TraversalAIPluginService } from "traversalai/plugin-sdk";
...
function normalizeDenyList(cfg: TraversalAIPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: TraversalAIPluginApi["config"]): string[] {
...
export default function register(api: TraversalAIPluginApi) {
  ...
  const timerService: TraversalAIPluginService = {
