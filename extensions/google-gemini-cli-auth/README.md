# Google Gemini CLI Auth (TraversalAI plugin)
...
```bash
traversalai plugins enable google-gemini-cli-auth
```
...
```bash
traversalai models auth login --provider google-gemini-cli --set-default
```
...
- `TRAVERSALAI_GEMINI_OAUTH_CLIENT_ID` / `GEMINI_CLI_OAUTH_CLIENT_ID`
- `TRAVERSALAI_GEMINI_OAUTH_CLIENT_SECRET` / `GEMINI_CLI_OAUTH_CLIENT_SECRET`
