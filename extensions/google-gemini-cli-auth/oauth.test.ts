...
vi.mock("traversalai/plugin-sdk", () => ({
  isWSL2Sync: () => false,
...
describe("loginGeminiCliOAuth", () => {
  ...
  const ENV_KEYS = [
    "TRAVERSALAI_GEMINI_OAUTH_CLIENT_ID",
    "TRAVERSALAI_GEMINI_OAUTH_CLIENT_SECRET",
  ...
  beforeEach(() => {
    ...
    process.env.TRAVERSALAI_GEMINI_OAUTH_CLIENT_ID = "test-client-id.apps.googleusercontent.com";
    process.env.TRAVERSALAI_GEMINI_OAUTH_CLIENT_SECRET = "GOCSPX-test-client-secret";
  ...
  it("falls back across loadCodeAssist endpoints with aligned headers and metadata", async () => {
    ...
    const fetchMock = vi.fn(async (input: string | URL | Request, init?: RequestInit) => {
      ...
      if (url === USERINFO_URL) {
        return responseJson({ email: "lobster@traversalai.ai" });
      }
      if (url === LOAD_PROD) {
