...
TraversalAI extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
traversalai channels login --channel zalouser
...
```bash
traversalai message send --channel zalouser --target <threadId> --message "Hello from TraversalAI!"
```
...
```bash
traversalai channels login --channel zalouser              # Login via QR
traversalai channels login --channel zalouser --account work
traversalai channels status --probe
traversalai channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
traversalai directory self --channel zalouser
traversalai directory peers list --channel zalouser --query "name"
traversalai directory groups list --channel zalouser --query "work"
traversalai directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
traversalai message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
traversalai message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `traversalai channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
traversalai channels login --channel zalouser --account work
traversalai message send --channel zalouser --account work --target <id> --message "Hello"
