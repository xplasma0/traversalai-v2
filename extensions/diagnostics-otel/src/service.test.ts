...
vi.mock("traversalai/plugin-sdk", async () => {
  const actual = await vi.importActual<typeof import("traversalai/plugin-sdk")>("traversalai/plugin-sdk");
  return {
    ...actual,
...
import type { TraversalAIPluginServiceContext } from "traversalai/plugin-sdk";
import { emitDiagnosticEvent } from "traversalai/plugin-sdk";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/traversalai-diagnostics-otel-test";
...
): TraversalAIPluginServiceContext {
  return {
    config: {
      diagnostics: {
        enabled: true,
...
function createTraceOnlyContext(endpoint: string): TraversalAIPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("traversalai.webhook.received")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("traversalai.webhook.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("traversalai.message.queued")?.add).toHaveBeenCalled();
    expect(telemetryState.counters.get("traversalai.message.processed")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("traversalai.message.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.histograms.get("traversalai.queue.wait_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("traversalai.session.stuck")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("traversalai.session.stuck_age_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("traversalai.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("traversalai.webhook.processed");
    expect(spanNames).toContain("traversalai.message.processed");
    expect(spanNames).toContain("traversalai.session.stuck");
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["traversalai.token"];
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionCounter = telemetryState.counters.get("traversalai.session.state");
    expect(sessionCounter?.add).toHaveBeenCalledWith(
      1,
      expect.objectContaining({
        "traversalai.reason": expect.stringContaining("…"),
      }),
    );
    const attrs = sessionCounter?.add.mock.calls[0]?.[1] as Record<string, unknown> | undefined;
    expect(typeof attrs?.["traversalai.reason"]).toBe("string");
    expect(String(attrs?.["traversalai.reason"])).not.toContain(
