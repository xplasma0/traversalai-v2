import type { AnyAgentTool, TraversalAIPluginApi } from "traversalai/plugin-sdk";
...
export function createToolFactoryHarness(cfg: TraversalAIPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<TraversalAIPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
  ...
  return {
    api: api as TraversalAIPluginApi,
    resolveTool,
  };
}
