...
function resolveTraversalAIRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.TRAVERSALAI_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set TRAVERSALAI_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveTraversalAIRoot(), "dist", "extensionAPI.js");
