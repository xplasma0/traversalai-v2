...
`traversalai.invoke --each`) without adding new TraversalAI code per workflow.
...
This extension depends on TraversalAI internal modules (the embedded agent runner).
It is intended to ship as a **bundled** TraversalAI extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.traversalai/extensions` as a standalone plugin directory.
