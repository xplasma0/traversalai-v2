...
import type { TraversalAIPluginApi, TraversalAIPluginToolContext } from "../../../src/plugins/types.js";
import {
...
function fakeApi(overrides: Partial<TraversalAIPluginApi> = {}): TraversalAIPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<TraversalAIPluginToolContext> = {}): TraversalAIPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  beforeAll(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "traversalai-lobster-plugin-"));
  });
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: TraversalAIPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
