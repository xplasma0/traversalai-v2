...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["traversalaiPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["traversalai", "pollId"]) ??
    readNestedString(value, ["traversalai", "poll", "id"]) ??
    readNestedString(value, ["data", "traversalaiPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "traversalai", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          traversalaiPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "traversalai poll vote",
          displayText: "Vote recorded",
          value: { traversalaiPollId: pollId, pollId },
        },
      },
    ],
  };
