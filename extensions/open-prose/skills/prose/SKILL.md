...
metadata: { "traversalai": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == TraversalAI `sessions_spawn`
- **File I/O** == TraversalAI `read`/`write`
- **Remote fetch** == TraversalAI `web_fetch` (or `exec` with curl when POST is required)
