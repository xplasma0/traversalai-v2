...
function resolveGatewayPort(cfg: TraversalAIPluginApi["config"]): number {
  const envPort =
    parsePositiveInteger(process.env.TRAVERSALAI_GATEWAY_PORT?.trim()) ??
...
function resolveScheme(
  cfg: TraversalAIPluginApi["config"],
...
function resolveAuth(cfg: TraversalAIPluginApi["config"]): ResolveAuthResult {
...
async function resolveGatewayUrl(api: TraversalAIPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the TraversalAI iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the TraversalAI iOS app:",
            "",
            "```",
            qrAscii,
            "```",
