...
import type { PluginRuntime, RuntimeEnv, RuntimeLogger } from "traversalai/plugin-sdk";
  ...
  it("stores sender-labeled BodyForAgent for group thread messages", async () => {
    ...
    const core = {
      channel: {
        ...
        session: {
          resolveStorePath: vi.fn().mockReturnValue("/tmp/traversalai-test-session.json"),
