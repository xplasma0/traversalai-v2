...
function usage() {
  ...
  process.stderr.write(
    "Usage: node scripts/ui.js <install|dev|build|dev:agents|build:agents|test> [...args]\n",
  );
}
...
function resolveRunner() {
  ...
  if (pnpm) {
    return { cmd: pnpm, prefixArgs: [], kind: "pnpm" };
  }
  const corepack = which("corepack");
  if (corepack) {
    return { cmd: corepack, prefixArgs: ["pnpm"], kind: "corepack-pnpm" };
  }
  return null;
}
...
function resolveScriptAction(action) {
  ...
  if (action === "dev:agents") {
    return "dev:agents";
  }
  if (action === "build") {
    return "build";
  }
  if (action === "build:agents") {
    return "build:agents";
  }
  if (action === "test") {
    return "test";
...
export function main(argv = process.argv.slice(2)) {
  ...
  if (action === "install") {
    run(runner.cmd, [...runner.prefixArgs, "install", ...rest]);
    return;
  }
  ...
  if (!depsInstalled(action === "test" ? "test" : "build")) {
    ...
    runSync(runner.cmd, [...runner.prefixArgs, ...installArgs], installEnv);
  }

  run(runner.cmd, [...runner.prefixArgs, "run", script, ...rest]);
}
