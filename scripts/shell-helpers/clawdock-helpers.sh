#!/usr/bin/env bash
# ClawDock - Docker helpers for TraversalAI
# Inspired by Simon Willison's "Running TraversalAI in Docker"
# https://til.simonwillison.net/llms/traversalai-docker
#
# Installation:
...
# Common paths to check for TraversalAI
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/traversalai"
  "${HOME}/workspace/traversalai"
  "${HOME}/projects/traversalai"
  "${HOME}/dev/traversalai"
  "${HOME}/code/traversalai"
  "${HOME}/src/traversalai"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found TraversalAI at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/traversalai"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ TraversalAI not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/traversalai/traversalai.git ~/traversalai"
    echo "  cd ~/traversalai && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^TRAVERSALAI_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d traversalai-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart traversalai-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f traversalai-gateway
}
...
clawdock-workspace() {
  cd ~/.traversalai/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec traversalai-gateway \
    bash -c 'echo "alias traversalai=\"./traversalai.mjs\"" > /tmp/.bashrc_traversalai && bash --rcfile /tmp/.bashrc_traversalai'
}
...
clawdock-cli() {
  _clawdock_compose run --rm traversalai-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build traversalai-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "TRAVERSALAI_GATEWAY_TOKEN=$token" traversalai-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" traversalai-gateway \
    bash -c './traversalai.mjs config set gateway.remote.token "$TOKEN" && ./traversalai.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec traversalai-gateway \
    bash -c "./traversalai.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart traversalai-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm traversalai-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec traversalai-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'traversalai config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for TraversalAI${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (traversalai alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to traversalai project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.traversalai)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'traversalai channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'traversalai status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.traversalai.ai${_CLR_RESET}"
  echo ""
}
