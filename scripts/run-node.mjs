...
const runnerLabel = process.env.TRAVERSALAI_APP === "1" ? "traversalai" : "traversalai";
...
const canRunPnpmDirectly = (deps) => {
  try {
    const probe = deps.spawnSync("pnpm", ["--version"], {
      cwd: deps.cwd,
      env: deps.env,
      stdio: "ignore",
    });
...
const shouldBuild = (deps) => {
  if (deps.env.TRAVERSALAI_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.TRAVERSALAI_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[${runnerLabel}] ${message}\n`);
};

const runTraversalAI = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["traversalai.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
...
export async function runNodeMain(params = {}) {
  ...
  if (!shouldBuild(deps)) {
    return await runTraversalAI(deps);
  }

  logRunner("Building TypeScript (dist is stale).", deps);
  const hasPnpm = canRunPnpmDirectly(deps);
  const buildCmd = deps.platform === "win32" ? "cmd.exe" : hasPnpm ? "pnpm" : "corepack";
  const buildArgs =
    deps.platform === "win32"
      ? ["/d", "/s", "/c", hasPnpm ? "pnpm" : "corepack pnpm", ...compilerArgs]
      : hasPnpm
        ? compilerArgs
        : ["pnpm", ...compilerArgs];
