 ...
 * `traversalai update`. Use it to verify the completion installation flow
...
\`traversalai update\`, \`traversalai doctor\`, and \`traversalai onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# TraversalAI Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`traversalai completion --install\` later to enable.`));
    return;
  }
