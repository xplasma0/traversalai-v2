...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/traversalai/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "TraversalAI Re-Auth" -c "After getting key, run: ssh $SERVER '~/traversalai/scripts/mobile-reauth.sh'" --id traversalai-auth
        ;;
    *)
