...
BASE_IMAGE="${BASE_IMAGE:-traversalai-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-traversalai-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=traversalai.sandbox=1)
NOTE
