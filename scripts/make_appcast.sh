...
ZIP=${1:?"Usage: $0 TraversalAI-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/traversalai/traversalai/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  ...
  if [[ "$ZIP_NAME" =~ ^TraversalAI-([0-9]+(\.[0-9]+){1,2}([-.][0-9A-Za-z]+([.-][0-9A-Za-z]+)*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/traversalai/traversalai/releases/download/v${VERSION}/"}
