...
INSTALL_URL="${TRAVERSALAI_INSTALL_URL:-https://traversalai.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${TRAVERSALAI_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${TRAVERSALAI_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="traversalai"
PACKAGE_NAME="${TRAVERSALAI_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (TRAVERSALAI_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${TRAVERSALAI_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${TRAVERSALAI_INSTALL_LATEST_OUT:-}"
fi
if [[ -n "$CMD_PATH" ]]; then
