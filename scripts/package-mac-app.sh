...
# Build and bundle TraversalAI into a minimal .app we can open.
# Outputs to dist/TraversalAI.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/TraversalAI.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="TraversalAI"
BUNDLE_ID="${BUNDLE_ID:-ai.traversalai.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/traversalai/traversalai/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/TraversalAI/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :TraversalAIBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :TraversalAIGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/TraversalAI"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/TraversalAI"
fi
chmod +x "$APP_ROOT/Contents/MacOS/TraversalAI"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/TraversalAI" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/TraversalAI/Resources/TraversalAI.icns" "$APP_ROOT/Contents/Resources/TraversalAI.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/TraversalAI/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying TraversalAIKit resources"
TRAVERSALAIKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/TraversalAIKit_TraversalAIKit.bundle"
if [ -d "$TRAVERSALAIKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/TraversalAIKit_TraversalAIKit.bundle"
  cp -R "$TRAVERSALAIKIT_BUNDLE" "$APP_ROOT/Contents/Resources/TraversalAIKit_TraversalAIKit.bundle"
else
  echo "WARN: TraversalAIKit resource bundle not found at $TRAVERSALAIKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running TraversalAI"
killall -q TraversalAI 2>/dev/null || true
