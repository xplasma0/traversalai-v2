...
IMAGE_NAME="traversalai-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="traversalai-net-e2e-$$"
GW_NAME="traversalai-gateway-e2e-$$"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "TRAVERSALAI_GATEWAY_TOKEN=$TOKEN" \
  -e "TRAVERSALAI_SKIP_CHANNELS=1" \
  -e "TRAVERSALAI_SKIP_GMAIL_WATCHER=1" \
  -e "TRAVERSALAI_SKIP_CRON=1" \
  -e "TRAVERSALAI_SKIP_CANVAS_HOST=1" \
  "$IMAGE_NAME" \
