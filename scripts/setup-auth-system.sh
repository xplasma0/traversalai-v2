#!/bin/bash
# Setup TraversalAI Auth Management System
...
echo "=== TraversalAI Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'traversalai-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  TraversalAI message: Send warning via TraversalAI itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/traversalai-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/traversalai-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/traversalai-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now traversalai-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status traversalai-auth-monitor.timer"
echo ""
echo "Quick commands:"
