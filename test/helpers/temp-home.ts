...
type EnvSnapshot = {
  ...
  traversalaiHome: string | undefined;
...
function snapshotEnv(): EnvSnapshot {
  return {
    ...
    traversalaiHome: process.env.TRAVERSALAI_HOME,
...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("TRAVERSALAI_HOME", snapshot.traversalaiHome);
  restoreKey("TRAVERSALAI_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked TRAVERSALAI_HOME.
  delete process.env.TRAVERSALAI_HOME;
  process.env.TRAVERSALAI_STATE_DIR = path.join(base, ".traversalai");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "traversalai-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".traversalai", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
