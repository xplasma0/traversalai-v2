...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `traversalai-e2e-${name}-`));
  const configDir = path.join(homeDir, ".traversalai");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          TRAVERSALAI_SKIP_PROVIDERS: "1",
          TRAVERSALAI_SKIP_GMAIL_WATCHER: "1",
          TRAVERSALAI_SKIP_CRON: "1",
          TRAVERSALAI_SKIP_BROWSER_CONTROL_SERVER: "1",
          TRAVERSALAI_SKIP_CANVAS_HOST: "1",
          TRAVERSALAI_TEST_MINIMAL_GATEWAY: "1",
          VITEST: "1",
        },
