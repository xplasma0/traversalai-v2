...
private let chatUILogger = Logger(subsystem: "ai.traversalai", category: "TraversalAIChatUI")

@MainActor
@Observable
public final class TraversalAIChatViewModel {
    public private(set) var messages: [TraversalAIChatMessage] = []
    ...
    public var attachments: [TraversalAIPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [TraversalAIChatPendingToolCall] = []
    public private(set) var sessions: [TraversalAIChatSessionEntry] = []
    private let transport: any TraversalAIChatTransport
    ...
    private var pendingToolCallsById: [String: TraversalAIChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any TraversalAIChatTransport) {
    ...
    public var sessionChoices: [TraversalAIChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }

        var result: [TraversalAIChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: TraversalAIPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [TraversalAIChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: TraversalAIChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: TraversalAIChatMessage) -> TraversalAIChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> TraversalAIChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return TraversalAIChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageIdentityKey(for message: TraversalAIChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [TraversalAIChatMessage],
        incoming: [TraversalAIChatMessage]) -> [TraversalAIChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [TraversalAIChatMessage]) -> [TraversalAIChatMessage] {
        var result: [TraversalAIChatMessage] = []
    ...
    private static func dedupeKey(for message: TraversalAIChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [TraversalAIChatMessageContent] = [
            TraversalAIChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> TraversalAIChatAttachmentPayload in
            TraversalAIChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> TraversalAIChatSessionEntry {
        TraversalAIChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: TraversalAIChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: TraversalAIChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: TraversalAIAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = TraversalAIChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> TraversalAIPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
