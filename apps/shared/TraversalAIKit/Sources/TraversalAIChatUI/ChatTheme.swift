...
enum TraversalAIChatTheme {
    ...
    static let assistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("TraversalAIChatTheme.assistantBubble"),
        dynamicProvider: resolvedAssistantBubbleColor(for:))

    static let onboardingAssistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("TraversalAIChatTheme.onboardingAssistantBubble"),
...
enum TraversalAIPlatformImageFactory {
    static func image(_ image: TraversalAIPlatformImage) -> Image {
        #if os(macOS)
        Image(nsImage: image)
        #else
        Image(uiImage: image)
