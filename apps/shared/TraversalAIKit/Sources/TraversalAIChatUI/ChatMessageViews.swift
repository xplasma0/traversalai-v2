...
struct ChatMessageBubble: View {
    let message: TraversalAIChatMessage
    let style: TraversalAIChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? TraversalAIChatTheme.userText : TraversalAIChatTheme.assistantText
    ...
    private var inlineAttachments: [TraversalAIChatMessageContent] {
    ...
    private var toolCalls: [TraversalAIChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [TraversalAIChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? TraversalAIChatTheme.userBubble
        }
        if self.style == .onboarding {
            return TraversalAIChatTheme.onboardingAssistantBubble
        }
        return TraversalAIChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return TraversalAIChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: TraversalAIChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? TraversalAIChatTheme.userText : TraversalAIChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: TraversalAIChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(TraversalAIChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(TraversalAIChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [TraversalAIChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: TraversalAIChatTheme.assistantText)
            }
        }
    }
}
