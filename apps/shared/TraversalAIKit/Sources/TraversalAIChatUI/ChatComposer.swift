...
@MainActor
struct TraversalAIChatComposer: View {
    @Bindable var viewModel: TraversalAIChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(TraversalAIChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(TraversalAIChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \TraversalAIPendingAttachment.id)
                { (att: TraversalAIPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            TraversalAIPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if !self.isComposerCompacted {
                Rectangle()
                    .fill(TraversalAIChatTheme.divider)
                    .frame(height: 1)
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(TraversalAIChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(TraversalAIChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(TraversalAIChatTheme.subtleCard)
        .clipShape(Capsule())
    }
