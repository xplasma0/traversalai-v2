...
import Foundation
import Testing
@testable import TraversalAIChatUI
...
private final class TestChatTransport: @unchecked Sendable, TraversalAIChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [TraversalAIChatHistoryPayload]
    private let sessionsResponses: [TraversalAIChatSessionsListResponse]

    private let stream: AsyncStream<TraversalAIChatTransportEvent>
    private let continuation: AsyncStream<TraversalAIChatTransportEvent>.Continuation

    init(
        historyResponses: [TraversalAIChatHistoryPayload],
        sessionsResponses: [TraversalAIChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<TraversalAIChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> TraversalAIChatHistoryPayload {
        ...
        return self.historyResponses.last ?? TraversalAIChatHistoryPayload(
        ...
        attachments _: [TraversalAIChatAttachmentPayload]) async throws -> TraversalAIChatSendResponse
    {
        ...
        return TraversalAIChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> TraversalAIChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? TraversalAIChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: TraversalAIChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = TraversalAIChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = TraversalAIChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { TraversalAIChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = TraversalAIChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = TraversalAIChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = TraversalAIChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { TraversalAIChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
