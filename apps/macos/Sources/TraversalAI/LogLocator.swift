...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["TRAVERSALAI_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/traversalai")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("traversalai-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("traversalai-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/traversalai/ (rolling or stdout), or nil if none exist.
