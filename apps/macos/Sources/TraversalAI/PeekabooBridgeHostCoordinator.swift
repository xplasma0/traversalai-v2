...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.traversalai", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: TraversalAIPeekabooBridgeServices?
    private static var traversalaiSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("TraversalAI", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = TraversalAIPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.traversalaiSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.traversalaiSocketPath, privacy: .public)")
    }
...
@MainActor
private final class TraversalAIPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.traversalai.peekaboo")
