...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: TraversalAIProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.traversalai", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: TraversalAIKit.AnyCodable]()) {
                $0[$1.key] = TraversalAIKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: TraversalAIProtocol.AnyCodable?) -> [String: TraversalAIProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: TraversalAIProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: TraversalAIKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: TraversalAIProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("traversalai.control.heartbeat")
    static let controlAgentEvent = Notification.Name("traversalai.control.agent")
}
