...
enum TraversalAILogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = TraversalAIOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = TraversalAIFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        TraversalAILogging.bootstrapIfNeeded()
        let label = TraversalAILogging.makeLabel(subsystem: subsystem, category: category)
...
struct TraversalAIOSLogHandler: LogHandler {
...
struct TraversalAIFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = TraversalAILogging.parseLabel(self.label)
