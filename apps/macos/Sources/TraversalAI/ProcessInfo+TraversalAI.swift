...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.traversalai.mac`) even if the shipped
    ...
    {
        if environment["TRAVERSALAI_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "traversalai.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "traversalai.nixMode") { return true }
