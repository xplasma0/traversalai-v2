...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.traversalai/traversalai.json. Prefer TRAVERSALAI_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.traversalai/traversalai.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.traversalai/traversalai.json. Restart the gateway to apply."
        }
