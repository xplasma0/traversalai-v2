...
enum DebugActions {
    private static let verboseDefaultsKey = "traversalai.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "traversalai.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = TraversalAIPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "TraversalAI", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = TraversalAIPaths.configURL
        guard
