...
private let webChatSwiftLogger = Logger(subsystem: "ai.traversalai", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: TraversalAIChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> TraversalAIChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(TraversalAIChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> TraversalAIChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                TraversalAIGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "chat":
                ...
                guard let chat = try? JSONDecoder().decode(
                    TraversalAIChatEventPayload.self,
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    TraversalAIAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<TraversalAIChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any TraversalAIChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = TraversalAIChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: TraversalAIChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<TraversalAIChatView>) -> NSViewController
    {
