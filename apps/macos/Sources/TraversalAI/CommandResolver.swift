...
enum CommandResolver {
    private static let projectRootDefaultsKey = "traversalai.gatewayProjectRootPath"
    private static let helperName = "traversalai"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let traversalaiEntry = root.appendingPathComponent("traversalai.mjs").path
        if FileManager().isReadableFile(atPath: traversalaiEntry) { return traversalaiEntry }
        let binEntry = root.appendingPathComponent("bin/traversalai.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/traversalai")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let traversalaiPaths = self.traversalaiManagedPaths(home: home)
        if !traversalaiPaths.isEmpty {
            extras.insert(contentsOf: traversalaiPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + traversalaiPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func traversalaiManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".traversalai"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func traversalaiExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectTraversalAIExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("traversalai.mjs").path,
            root.appendingPathComponent("bin/traversalai.js").path,
        ]
    ...
    static func hasAnyTraversalAIInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.traversalaiExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func traversalaiNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        let root = self.projectRoot()
        if let traversalaiPath = self.projectTraversalAIExecutable(projectRoot: root) {
            return [traversalaiPath, subcommand] + extraArgs
        }
        if let traversalaiPath = self.traversalaiExecutable(searchPaths: searchPaths) {
            return [traversalaiPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "traversalai", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            traversalai entrypoint missing (looked for dist/index.js or traversalai.mjs); run pnpm build.
            """
    ...
    static func traversalaiCommand(
        subcommand: String,
        extraArgs: [String] = [],
        defaults: UserDefaults = .standard,
        configRoot: [String: Any]? = nil,
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real traversalai CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/traversalai"
        ...
        \(cliSection)
        \(projectSection)
        if command -v traversalai >/dev/null 2>&1; then
          CLI="$(command -v traversalai)"
          traversalai \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/traversalai.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/traversalai.mjs"
            node "$PRJ/traversalai.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/traversalai.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/traversalai.js"
            node "$PRJ/bin/traversalai.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent traversalai"
          pnpm --silent traversalai \(quotedArgs);
        else
          echo "traversalai CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? TraversalAIConfigFile.loadDict()
