...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.traversalai", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> TraversalAISessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: TraversalAISessionPreviewEntry,
    ...
    private static func cache(payload: TraversalAISessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: TraversalAIChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [TraversalAIKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [TraversalAIKit.AnyCodable]) -> [TraversalAIChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(TraversalAIChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: TraversalAIChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: TraversalAIChatMessage) -> Bool {
    ...
    private static func toolNames(for message: TraversalAIChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: TraversalAIChatMessage) -> String? {
