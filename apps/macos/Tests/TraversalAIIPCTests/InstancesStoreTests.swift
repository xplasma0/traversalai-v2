...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: TraversalAIProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: TraversalAIProtocol.AnyCodable] = [
            "presence": .init([TraversalAIProtocol.AnyCodable(entry)]),
        ]
        let payload = TraversalAIProtocol.AnyCodable(payloadMap)
