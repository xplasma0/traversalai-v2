...
final class MotionService: MotionServicing {
    func activities(params: TraversalAIMotionActivityParams) async throws -> TraversalAIMotionActivityPayload {
        ...
        let mapped = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[TraversalAIMotionActivityEntry], Error>) in
            manager.queryActivityStarting(from: start, to: end, to: OperationQueue()) { activity, error in
                if let error {
                    ...
                    let entries = sliced.map { entry in
                        TraversalAIMotionActivityEntry(
        ...
        return TraversalAIMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: TraversalAIPedometerParams) async throws -> TraversalAIPedometerPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<TraversalAIPedometerPayload, Error>) in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = TraversalAIPedometerPayload(
