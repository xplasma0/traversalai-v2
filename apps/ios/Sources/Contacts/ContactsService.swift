...
final class ContactsService: ContactsServicing {
    ...
    func search(params: TraversalAIContactsSearchParams) async throws -> TraversalAIContactsSearchPayload {
        ...
        return TraversalAIContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: TraversalAIContactsAddParams) async throws -> TraversalAIContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return TraversalAIContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return TraversalAIContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> TraversalAIContactPayload {
        TraversalAIContactPayload(
