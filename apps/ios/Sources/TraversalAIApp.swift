...
@MainActor
final class TraversalAIAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.traversalai.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.traversalai.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.traversalai.ios.bgrefresh"
...
enum WatchPromptNotificationBridge {
    static let typeKey = "traversalai.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "traversalai.watch.promptId"
    static let sessionKeyKey = "traversalai.watch.sessionKey"
    static let actionPrimaryIDKey = "traversalai.watch.action.primary.id"
    static let actionPrimaryLabelKey = "traversalai.watch.action.primary.label"
    static let actionSecondaryIDKey = "traversalai.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "traversalai.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "traversalai.watch.action.primary"
    static let actionSecondaryIdentifier = "traversalai.watch.action.secondary"
    static let actionIdentifierPrefix = "traversalai.watch.action."
    static let actionIDKeyPrefix = "traversalai.watch.action.id."
    static let actionLabelKeyPrefix = "traversalai.watch.action.label."
    static let categoryPrefix = "traversalai.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: TraversalAIWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> TraversalAIWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return TraversalAIWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "TraversalAI" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [TraversalAIWatchAction]) -> [UNNotificationAction] {
...
@main
struct TraversalAIApp: App {
    @State private var appModel: NodeAppModel
    @State private var gatewayController: GatewayConnectionController
    @UIApplicationDelegateAdaptor(TraversalAIAppDelegate.self) private var appDelegate
...
extension TraversalAIApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("TraversalAI: installing uncaught exception handler")
