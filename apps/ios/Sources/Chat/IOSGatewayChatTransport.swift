...
struct IOSGatewayChatTransport: TraversalAIChatTransport, Sendable {
    private static let logger = Logger(subsystem: "ai.traversalai", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> TraversalAIChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(TraversalAIChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> TraversalAIChatHistoryPayload {
        ...
        return try JSONDecoder().decode(TraversalAIChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [TraversalAIChatAttachmentPayload]) async throws -> TraversalAIChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [TraversalAIChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(TraversalAIChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(TraversalAIGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: TraversalAIGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: TraversalAIChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: TraversalAIAgentEventPayload.self)
                        {
