...
final class RemindersService: RemindersServicing {
    func list(params: TraversalAIRemindersListParams) async throws -> TraversalAIRemindersListPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[TraversalAIReminderPayload], Error>) in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return TraversalAIReminderPayload(
        ...
        return TraversalAIRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: TraversalAIRemindersAddParams) async throws -> TraversalAIRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = TraversalAIReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return TraversalAIRemindersAddPayload(reminder: payload)
    }
