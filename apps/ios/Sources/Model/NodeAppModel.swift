...
final class NodeAppModel {
    ...
    private let deepLinkLogger = Logger(subsystem: "ai.traversalai.ios", category: "DeepLink")
    private let pushWakeLogger = Logger(subsystem: "ai.traversalai.ios", category: "PushWake")
    private let locationWakeLogger = Logger(subsystem: "ai.traversalai.ios", category: "LocationWake")
    private let watchReplyLogger = Logger(subsystem: "ai.traversalai.ios", category: "WatchReply")
    enum CameraHUDKind {
        case photo
        case recording
        case success
    ...
    private func handleCanvasA2UIAction(body: [String: Any]) async {
        ...
        guard let name = TraversalAICanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId: String = {
        ...
        let contextJSON = TraversalAICanvasA2UIAction.compactJSON(userAction["context"])
        ...
        let message = TraversalAICanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        let js = TraversalAICanvasA2UIAction.jsDispatchA2UIActionStatus(actionId: actionId, ok: ok, error: errorText)
        do {
    ...
    func requestLocationPermissions(mode: TraversalAILocationMode) async -> Bool {
        ...
        self.gatewayHealthMonitor.start(
            check: { [weak self] in
                ...
                do {
                    ...
                    guard let decoded = try? JSONDecoder().decode(TraversalAIGatewayHealthOK.self, from: data) else {
                        return false
                    }
        ...
        do {
            ...
            case .unknownCommand:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: TraversalAINodeError(code: .invalidRequest, message: "INVALID_REQUEST: unknown command"))
            case .handlerUnavailable:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: TraversalAINodeError(code: .unavailable, message: "node handler unavailable"))
            }
        } catch {
            ...
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: TraversalAINodeError(code: .unavailable, message: error.localizedDescription))
        }
    }
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params = (try? Self.decodeParams(TraversalAILocationGetParams.self, from: req.paramsJSON)) ??
            TraversalAILocationGetParams()
        ...
        let payload = TraversalAILocationPayload(
    ...
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case TraversalAICanvasCommand.present.rawValue:
            // iOS ignores placement hints; canvas always fills the screen.
            let params = (try? Self.decodeParams(TraversalAICanvasPresentParams.self, from: req.paramsJSON)) ??
                TraversalAICanvasPresentParams()
        ...
        case TraversalAICanvasCommand.hide.rawValue:
            self.screen.showDefaultCanvas()
            return BridgeInvokeResponse(id: req.id, ok: true)
        case TraversalAICanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(TraversalAICanvasNavigateParams.self, from: req.paramsJSON)
            self.screen.navigate(to: params.url)
            return BridgeInvokeResponse(id: req.id, ok: true)
        case TraversalAICanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(TraversalAICanvasEvalParams.self, from: req.paramsJSON)
            let result = try await self.screen.eval(javaScript: params.javaScript)
            let payload = try Self.encodePayload(["result": result])
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case TraversalAICanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(TraversalAICanvasSnapshotParams.self, from: req.paramsJSON)
    ...
    private func handleCanvasA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let command = req.command
        switch command {
        case TraversalAICanvasA2UICommand.reset.rawValue:
            ...
            (() => {
              const host = globalThis.traversalaiA2UI;
              if (!host) return JSON.stringify({ ok: false, error: "missing traversalaiA2UI" });
              return JSON.stringify(host.reset());
            })()
            """)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case TraversalAICanvasA2UICommand.push.rawValue, TraversalAICanvasA2UICommand.pushJSONL.rawValue:
            let messages: [TraversalAIKit.AnyCodable]
            if command == TraversalAICanvasA2UICommand.pushJSONL.rawValue {
                let params = try Self.decodeParams(TraversalAICanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
                messages = try TraversalAICanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
            } else {
                do {
                    let params = try Self.decodeParams(TraversalAICanvasA2UIPushParams.self, from: req.paramsJSON)
            ...
            let messagesJSON = try TraversalAICanvasA2UIJSONL.encodeMessagesJSONArray(messages)
            let js = """
            (() => {
              try {
                const host = globalThis.traversalaiA2UI;
    ...
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case TraversalAICameraCommand.list.rawValue:
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case TraversalAICameraCommand.snap.rawValue:
            self.showCameraHUD(text: "Taking photo…", kind: .photo)
            self.triggerCameraFlash()
            let params = (try? Self.decodeParams(TraversalAICameraSnapParams.self, from: req.paramsJSON)) ??
                TraversalAICameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case TraversalAICameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(TraversalAICameraClipParams.self, from: req.paramsJSON)) ??
                TraversalAICameraClipParams()
    ...
    private func handleScreenRecordInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(TraversalAIScreenRecordParams.self, from: req.paramsJSON)) ??
            TraversalAIScreenRecordParams()
    ...
    private func handleSystemNotify(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(TraversalAISystemNotifyParams.self, from: req.paramsJSON)
        ...
        if title.isEmpty, body.isEmpty {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: TraversalAINodeError(code: .invalidRequest, message: "INVALID_REQUEST: empty notification"))
        }
        ...
        guard finalStatus == .authorized || finalStatus == .provisional || finalStatus == .ephemeral else {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: TraversalAINodeError(code: .unavailable, message: "NOT_AUTHORIZED: notifications"))
        }
        ...
        if case let .failure(error) = addResult {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: TraversalAINodeError(code: .unavailable, message: "NOTIFICATION_FAILED: \(error.message)"))
        }
        return BridgeInvokeResponse(id: req.id, ok: true)
    }

    private func handleChatPushInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(TraversalAIChatPushParams.self, from: req.paramsJSON)
        let text = params.text.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !text.isEmpty else {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: TraversalAINodeError(code: .invalidRequest, message: "INVALID_REQUEST: empty chat.push text"))
        }
        ...
        let payload = TraversalAIChatPushPayload(messageId: messageId)
        let json = try Self.encodePayload(payload)
        return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
    }
    ...
    private func handleDeviceInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case TraversalAIDeviceCommand.status.rawValue:
            let payload = try await self.deviceStatusService.status()
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case TraversalAIDeviceCommand.info.rawValue:
    ...
    private func handlePhotosInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(TraversalAIPhotosLatestParams.self, from: req.paramsJSON)) ??
            TraversalAIPhotosLatestParams()
    ...
    private func handleContactsInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case TraversalAIContactsCommand.search.rawValue:
            let params = (try? Self.decodeParams(TraversalAIContactsSearchParams.self, from: req.paramsJSON)) ??
                TraversalAIContactsSearchParams()
            let payload = try await self.contactsService.search(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case TraversalAIContactsCommand.add.rawValue:
            let params = try Self.decodeParams(TraversalAIContactsAddParams.self, from: req.paramsJSON)
    ...
    private func handleCalendarInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case TraversalAICalendarCommand.events.rawValue:
            let params = (try? Self.decodeParams(TraversalAICalendarEventsParams.self, from: req.paramsJSON)) ??
                TraversalAICalendarEventsParams()
            let payload = try await self.calendarService.events(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case TraversalAICalendarCommand.add.rawValue:
            let params = try Self.decodeParams(TraversalAICalendarAddParams.self, from: req.paramsJSON)
    ...
    private func handleRemindersInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case TraversalAIRemindersCommand.list.rawValue:
            let params = (try? Self.decodeParams(TraversalAIRemindersListParams.self, from: req.paramsJSON)) ??
                TraversalAIRemindersListParams()
            let payload = try await self.remindersService.list(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case TraversalAIRemindersCommand.add.rawValue:
            let params = try Self.decodeParams(TraversalAIRemindersAddParams.self, from: req.paramsJSON)
    ...
    private func handleMotionInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case TraversalAIMotionCommand.activity.rawValue:
            let params = (try? Self.decodeParams(TraversalAIMotionActivityParams.self, from: req.paramsJSON)) ??
                TraversalAIMotionActivityParams()
            let payload = try await self.motionService.activities(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case TraversalAIMotionCommand.pedometer.rawValue:
            let params = (try? Self.decodeParams(TraversalAIPedometerParams.self, from: req.paramsJSON)) ??
                TraversalAIPedometerParams()
    ...
    private func handleTalkInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case TraversalAITalkCommand.pttStart.rawValue:
            ...
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case TraversalAITalkCommand.pttStop.rawValue:
            ...
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case TraversalAITalkCommand.pttCancel.rawValue:
            let payload = await self.talkMode.cancelPushToTalk()
            self.voiceWake.resumeAfterExternalAudioCapture(wasSuspended: self.pttVoiceWakeSuspended)
            self.pttVoiceWakeSuspended = false
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case TraversalAITalkCommand.pttOnce.rawValue:
            self.pttVoiceWakeSuspended = self.voiceWake.suspendForExternalAudioCapture()
            defer {
                self.voiceWake.resumeAfterExternalAudioCapture(wasSuspended: self.pttVoiceWakeSuspended)
                self.pttVoiceWakeSuspended = false
...
private extension NodeAppModel {
    ...
    func buildCapabilityRouter() -> NodeCapabilityRouter {
        ...
        register([TraversalAILocationCommand.get.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleLocationInvoke(req)
        }

        register([
            TraversalAICanvasCommand.present.rawValue,
            TraversalAICanvasCommand.hide.rawValue,
            TraversalAICanvasCommand.navigate.rawValue,
            TraversalAICanvasCommand.evalJS.rawValue,
            TraversalAICanvasCommand.snapshot.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            TraversalAICanvasA2UICommand.reset.rawValue,
            TraversalAICanvasA2UICommand.push.rawValue,
            TraversalAICanvasA2UICommand.pushJSONL.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            TraversalAICameraCommand.list.rawValue,
            TraversalAICameraCommand.snap.rawValue,
            TraversalAICameraCommand.clip.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleCameraInvoke(req)
        }

        register([TraversalAIScreenCommand.record.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleScreenRecordInvoke(req)
        }

        register([TraversalAISystemCommand.notify.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleSystemNotify(req)
        }

        register([TraversalAIChatCommand.push.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleChatPushInvoke(req)
        }

        register([
            TraversalAIDeviceCommand.status.rawValue,
            TraversalAIDeviceCommand.info.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            TraversalAIWatchCommand.status.rawValue,
            TraversalAIWatchCommand.notify.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleWatchInvoke(req)
        }

        register([TraversalAIPhotosCommand.latest.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handlePhotosInvoke(req)
        }

        register([
            TraversalAIContactsCommand.search.rawValue,
            TraversalAIContactsCommand.add.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            TraversalAICalendarCommand.events.rawValue,
            TraversalAICalendarCommand.add.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            TraversalAIRemindersCommand.list.rawValue,
            TraversalAIRemindersCommand.add.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            TraversalAIMotionCommand.activity.rawValue,
            TraversalAIMotionCommand.pedometer.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            TraversalAITalkCommand.pttStart.rawValue,
            TraversalAITalkCommand.pttStop.rawValue,
            TraversalAITalkCommand.pttCancel.rawValue,
            TraversalAITalkCommand.pttOnce.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
    ...
    func handleWatchInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case TraversalAIWatchCommand.status.rawValue:
            let status = await self.watchMessagingService.status()
            let payload = TraversalAIWatchStatusPayload(
            ...
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case TraversalAIWatchCommand.notify.rawValue:
            let params = try Self.decodeParams(TraversalAIWatchNotifyParams.self, from: req.paramsJSON)
            ...
            do {
                ...
                let payload = TraversalAIWatchNotifyPayload(
    ...
    func locationMode() -> TraversalAILocationMode {
        let raw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        return TraversalAILocationMode(rawValue: raw) ?? .off
    }
...
private extension NodeAppModel {
                        ...
                        await MainActor.run {
                            ...
                            if let requestId, !requestId.isEmpty {
                                self.gatewayStatusText =
                                    "Pairing required (requestId: \(requestId)). Approve on gateway and return to TraversalAI."
                            } else {
                                self.gatewayStatusText = "Pairing required. Approve on gateway and return to TraversalAI."
                            }
                        }
    ...
    func legacyClientIdFallback(currentClientId: String, error: Error) -> String? {
        ...
        guard normalizedClientId == "traversalai-ios" else { return nil }
...
extension NodeAppModel {
    ...
    func runSharePipelineSelfTest() async {
        ...
        let payload = SharedContentPayload(
            title: "TraversalAI Share Self-Test",
            url: URL(string: "https://traversalai.ai/share-self-test"),
    ...
    func handleSilentPushWake(_ userInfo: [AnyHashable: Any]) async -> Bool {
        ...
        let pushKind = Self.traversalaiPushKind(userInfo)
        self.pushWakeLogger.info(
    ...
    private static func traversalaiPushKind(_ userInfo: [AnyHashable: Any]) -> String {
        if let payload = userInfo["traversalai"] as? [String: Any],
           let kind = payload["kind"] as? String
        {
            let trimmed = kind.trimmingCharacters(in: .whitespacesAndNewlines)
            if !trimmed.isEmpty { return trimmed }
        }
        if let payload = userInfo["traversalai"] as? [AnyHashable: Any],
           let kind = payload["kind"] as? String
        {
            let trimmed = kind.trimmingCharacters(in: .whitespacesAndNewlines)
            if !trimmed.isEmpty { return trimmed }
