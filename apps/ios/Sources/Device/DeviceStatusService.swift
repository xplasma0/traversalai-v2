...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> TraversalAIDeviceStatusPayload {
        ...
        return TraversalAIDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    func info() -> TraversalAIDeviceInfoPayload {
        ...
        return TraversalAIDeviceInfoPayload(
    ...
    private func batteryStatus() -> TraversalAIBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: TraversalAIBatteryState = switch device.batteryState {
        ...
        return TraversalAIBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> TraversalAIThermalStatusPayload {
        let state: TraversalAIThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return TraversalAIThermalStatusPayload(state: state)
    }

    private func storageStatus() -> TraversalAIStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return TraversalAIStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
