...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__TRAVERSALAI_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<TraversalAITalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.traversalai", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = TraversalAITalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> TraversalAITalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return TraversalAITalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> TraversalAITalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> TraversalAITalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return TraversalAITalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> TraversalAITalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = TraversalAITalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: TraversalAITalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(payload, as: TraversalAIAgentEventPayload.self) else {
                continue
            }
