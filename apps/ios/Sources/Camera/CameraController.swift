...
actor CameraController {
    ...
    func snap(params: TraversalAICameraSnapParams) async throws -> (
        format: String,
        base64: String,
        width: Int,
        height: Int)
    ...
    func clip(params: TraversalAICameraClipParams) async throws -> (
        format: String,
        base64: String,
        durationMs: Int,
        hasAudio: Bool)
    {
        ...
        let movURL = FileManager().temporaryDirectory
            .appendingPathComponent("traversalai-camera-\(UUID().uuidString).mov")
        let mp4URL = FileManager().temporaryDirectory
            .appendingPathComponent("traversalai-camera-\(UUID().uuidString).mp4")
    ...
    private nonisolated static func pickCamera(
        facing: TraversalAICameraFacing,
