...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [TraversalAICapability.canvas.rawValue, TraversalAICapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(TraversalAICapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(TraversalAICapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = TraversalAILocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(TraversalAICapability.location.rawValue) }

        caps.append(TraversalAICapability.device.rawValue)
        if WatchMessagingService.isSupportedOnDevice() {
            caps.append(TraversalAICapability.watch.rawValue)
        }
        caps.append(TraversalAICapability.photos.rawValue)
        caps.append(TraversalAICapability.contacts.rawValue)
        caps.append(TraversalAICapability.calendar.rawValue)
        caps.append(TraversalAICapability.reminders.rawValue)
        if Self.motionAvailable() {
            caps.append(TraversalAICapability.motion.rawValue)
        }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            TraversalAIScreenCommand.record.rawValue,
            TraversalAISystemCommand.notify.rawValue,
            TraversalAIChatCommand.push.rawValue,
        ...
        if caps.contains(TraversalAICapability.camera.rawValue) {
            commands.append(TraversalAICameraCommand.list.rawValue)
            commands.append(TraversalAICameraCommand.snap.rawValue)
            commands.append(TraversalAICameraCommand.clip.rawValue)
        }
        if caps.contains(TraversalAICapability.location.rawValue) {
            commands.append(TraversalAILocationCommand.get.rawValue)
        }
        if caps.contains(TraversalAICapability.device.rawValue) {
            commands.append(TraversalAIDeviceCommand.status.rawValue)
            commands.append(TraversalAIDeviceCommand.info.rawValue)
        }
        if caps.contains(TraversalAICapability.watch.rawValue) {
            commands.append(TraversalAIWatchCommand.status.rawValue)
            commands.append(TraversalAIWatchCommand.notify.rawValue)
        }
        if caps.contains(TraversalAICapability.photos.rawValue) {
            commands.append(TraversalAIPhotosCommand.latest.rawValue)
        }
        if caps.contains(TraversalAICapability.contacts.rawValue) {
            commands.append(TraversalAIContactsCommand.search.rawValue)
            commands.append(TraversalAIContactsCommand.add.rawValue)
        }
        if caps.contains(TraversalAICapability.calendar.rawValue) {
            commands.append(TraversalAICalendarCommand.events.rawValue)
            commands.append(TraversalAICalendarCommand.add.rawValue)
        }
        if caps.contains(TraversalAICapability.reminders.rawValue) {
            commands.append(TraversalAIRemindersCommand.list.rawValue)
            commands.append(TraversalAIRemindersCommand.add.rawValue)
        }
        if caps.contains(TraversalAICapability.motion.rawValue) {
            commands.append(TraversalAIMotionCommand.activity.rawValue)
            commands.append(TraversalAIMotionCommand.pedometer.rawValue)
        }
