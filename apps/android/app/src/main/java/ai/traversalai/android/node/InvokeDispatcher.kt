...
class InvokeDispatcher(
  ...
  suspend fun handleInvoke(command: String, paramsJson: String?): GatewaySession.InvokeResult {
    ...
    return when (command) {
      ...
      TraversalAICanvasCommand.Hide.rawValue -> GatewaySession.InvokeResult.ok(null)
      TraversalAICanvasCommand.Navigate.rawValue -> {
        val url = CanvasController.parseNavigateUrl(paramsJson)
        canvas.navigate(url)
        GatewaySession.InvokeResult.ok(null)
      }
      TraversalAICanvasCommand.Eval.rawValue -> {
        val js =
      ...
      TraversalAICanvasA2UICommand.Push.rawValue, TraversalAICanvasA2UICommand.PushJSONL.rawValue -> {
        val messages =
          try {
      ...
      // Camera commands
      TraversalAICameraCommand.Snap.rawValue -> cameraHandler.handleSnap(paramsJson)
      TraversalAICameraCommand.Clip.rawValue -> cameraHandler.handleClip(paramsJson)

      // Location command
      TraversalAILocationCommand.Get.rawValue -> locationHandler.handleLocationGet(paramsJson)

      // Device commands
      TraversalAIDeviceCommand.Status.rawValue -> deviceHandler.handleDeviceStatus(paramsJson)
      TraversalAIDeviceCommand.Info.rawValue -> deviceHandler.handleDeviceInfo(paramsJson)

      // Notifications command
      TraversalAINotificationsCommand.List.rawValue -> notificationsHandler.handleNotificationsList(paramsJson)

      // Screen command
      TraversalAIScreenCommand.Record.rawValue -> screenHandler.handleScreenRecord(paramsJson)

      // SMS command
      TraversalAISmsCommand.Send.rawValue -> smsHandler.handleSmsSend(paramsJson)
