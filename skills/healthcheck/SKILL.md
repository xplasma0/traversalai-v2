---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for TraversalAI deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, TraversalAI cron scheduling for periodic checks, or version status checks on a machine running TraversalAI (laptop, workstation, Pi, VPS).
---

# TraversalAI Host Hardening

## Overview

Assess and harden the host running TraversalAI, then align it to a user-defined risk tolerance without breaking access. Use TraversalAI security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim TraversalAI changes the host firewall, SSH, or OS updates; it does not.
...
5. TraversalAI gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if TraversalAI can access sensitive data.
...
### 2) Run TraversalAI security audits (read-only)

As part of the default read-only checks, run `traversalai security audit --deep`. Only offer alternatives if the user requests them:

1. `traversalai security audit` (faster, non-probing)
2. `traversalai security audit --json` (structured output)

Offer to apply TraversalAI safe defaults (numbered):

1. `traversalai security audit --fix`

Be explicit that `--fix` only tightens TraversalAI defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check TraversalAI version/update status (read-only)

As part of the default read-only checks, run `traversalai update status`.
...
- Credential hygiene notes (location of TraversalAI creds, prefer disk encryption)
...
- TraversalAI security audit (re-run)
...
After TraversalAI install or first hardening pass, run at least one baseline audit and version check:

- `traversalai security audit`
- `traversalai security audit --deep`
- `traversalai update status`

Ongoing monitoring is recommended. Use the TraversalAI cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `traversalai cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `traversalai update status`
...
Before creating, `traversalai cron list` and match on exact `name`. If found, `traversalai cron edit <id> ...`.
If not found, `traversalai cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `traversalai update status` (preferred for source checkouts and channels)
2. `npm view traversalai version` (published npm version)

## TraversalAI command accuracy

Use only supported commands and flags:

- `traversalai security audit [--deep] [--fix] [--json]`
- `traversalai status` / `traversalai status --deep`
- `traversalai health --json`
- `traversalai update status`
- `traversalai cron add|list|runs|run`

Do not invent CLI flags or imply TraversalAI enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by TraversalAI compaction:
