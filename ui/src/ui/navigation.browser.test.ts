...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/traversalai/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/traversalai");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/traversalai/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__TRAVERSALAI_CONTROL_UI_BASE_PATH__ = "/traversalai";
    const app = mountApp("/traversalai/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/traversalai");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/traversalai/sessions");
  });
