...
export async function handleWhatsAppStart(host: TraversalAIApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: TraversalAIApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: TraversalAIApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: TraversalAIApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: TraversalAIApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: TraversalAIApp): string {
...
function resolveGatewayHttpAuthHeader(host: TraversalAIApp): string | null {
...
function buildGatewayHttpHeaders(host: TraversalAIApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: TraversalAIApp) {
...
export function handleNostrProfileToggleAdvanced(host: TraversalAIApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: TraversalAIApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: TraversalAIApp) {
  const state = host.nostrProfileFormState;
