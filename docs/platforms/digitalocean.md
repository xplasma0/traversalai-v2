---
summary: "TraversalAI on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up TraversalAI on DigitalOcean
  - Looking for cheap VPS hosting for TraversalAI
title: "DigitalOcean"
---

# TraversalAI on DigitalOcean

## Goal

Run a persistent TraversalAI Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
# Check service
systemctl --user status traversalai-gateway.service

# View logs
journalctl --user -u traversalai-gateway.service -f
```
...
```bash
traversalai channels login whatsapp
# Scan QR code
```
...
- `~/.traversalai/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl -u traversalai --no-pager -n 50
```
