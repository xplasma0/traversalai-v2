...
- Same path: run the app from a fixed location (for TraversalAI, `dist/TraversalAI.app`).
...
```bash
sudo tccutil reset Accessibility ai.traversalai.mac
sudo tccutil reset ScreenCapture ai.traversalai.mac
...
Workaround: move files into the TraversalAI workspace (`~/.traversalai/workspace`) if you want to avoid per-folder grants.
