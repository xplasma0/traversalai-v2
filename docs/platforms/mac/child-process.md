...
service via the external `traversalai` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.traversalai.gateway`
  (or `ai.traversalai.<profile>` when using `--profile`/`TRAVERSALAI_PROFILE`; legacy `com.traversalai.*` is supported).
