---
summary: "TraversalAI on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up TraversalAI on a Raspberry Pi
  - Running TraversalAI on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# TraversalAI on Raspberry Pi

## Goal

Run a persistent, always-on TraversalAI Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
# Update config
traversalai config set gateway.bind tailnet
...
Most TraversalAI features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled traversalai

# Enable if not
sudo systemctl enable traversalai

# Start on boot
sudo systemctl start traversalai
```
...
```bash
# Check logs
journalctl -u traversalai --no-pager -n 100

# Common fix: rebuild
cd ~/traversalai  # if using hackable install
npm run build
sudo systemctl restart traversalai
```
