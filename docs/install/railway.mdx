...
Deploy TraversalAI on Railway with a one-click template and finish setup in your browser.
...
- `https://<your-railway-domain>/traversalai` — Control UI
...
- `TRAVERSALAI_STATE_DIR=/data/.traversalai` (recommended)
- `TRAVERSALAI_WORKSPACE_DIR=/data/workspace` (recommended)
- `TRAVERSALAI_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your TraversalAI state + workspace so you can migrate to another host without losing config or memory.
