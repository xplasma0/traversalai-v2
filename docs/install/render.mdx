...
Deploy TraversalAI on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/traversalai/traversalai)
...
repository configures everything needed to run TraversalAI:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: TRAVERSALAI_STATE_DIR
        value: /data/.traversalai
      - key: TRAVERSALAI_WORKSPACE_DIR
        value: /data/workspace
      - key: TRAVERSALAI_GATEWAY_TOKEN
        generateValue: true # auto-generates a secure token
    disk:
      name: traversalai-data
      mountPath: /data
      sizeGB: 1
```
...
The web dashboard is available at `https://<your-service>.onrender.com/traversalai`.
...
If you use the original TraversalAI repository, Render will not auto-deploy your TraversalAI. To update it, run a manual Blueprint sync from the dashboard.
...
For TraversalAI, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any TraversalAI host.
