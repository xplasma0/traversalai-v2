...
- Jobs persist under `~/.traversalai/cron/` so restarts don’t lose schedules.
...
traversalai cron list
traversalai cron run <job-id>
traversalai cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.traversalai/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `traversalai cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, TraversalAI applies a
...
If `delivery` is omitted for isolated jobs, TraversalAI defaults to `announce`.
...
- Job store: `~/.traversalai/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.traversalai/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned by size and line count).
...
```json5
{
  cron: {
    ...
    store: "~/.traversalai/cron/jobs.json",
...
- For removed run sessions no longer referenced by the session store, TraversalAI archives transcript files and purges old deleted archives on the same retention window.
...
- review growth periodically with `traversalai cron runs` and adjust retention before logs become large
...
traversalai cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
traversalai cron edit <jobId> --agent ops
traversalai cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
traversalai cron run <jobId>
traversalai cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
traversalai cron edit <jobId> \
...
```bash
traversalai cron edit <jobId> --exact
```
...
```bash
traversalai system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`traversalai system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `TRAVERSALAI_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- TraversalAI applies exponential retry backoff for recurring jobs after consecutive errors:
