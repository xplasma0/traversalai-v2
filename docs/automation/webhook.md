...
read_when:
  ...
  - Wiring external systems into TraversalAI
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your TraversalAI config directory (typically `~/.traversalai/hooks/transforms`).
...
- `traversalai webhooks gmail setup` writes `hooks.gmail` config for `traversalai webhooks gmail run`.
