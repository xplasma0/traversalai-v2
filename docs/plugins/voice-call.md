...
read_when:
  - You want to place an outbound voice call from TraversalAI
...
Voice calls for TraversalAI via a plugin. Supports outbound notifications and
...
- Use `traversalai voicecall ...` or the `voice_call` tool
...
```bash
traversalai plugins install ./extensions/voice-call
