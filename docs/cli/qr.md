---
summary: "CLI reference for `traversalai qr` (generate iOS pairing QR + setup code)"
read_when:
...
```bash
traversalai qr
traversalai qr --setup-code-only
traversalai qr --json
traversalai qr --remote
traversalai qr --url wss://gateway.example/ws --token '<token>'
```
