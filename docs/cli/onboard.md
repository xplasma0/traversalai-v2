---
summary: "CLI reference for `traversalai onboard` (interactive onboarding wizard)"
read_when:
...
```bash
traversalai onboard
traversalai onboard --flow quickstart
traversalai onboard --flow manual
traversalai onboard --mode remote --remote-url ws://gateway-host:18789
```
...
- Fastest first chat: `traversalai dashboard` (Control UI, no channel setup).
