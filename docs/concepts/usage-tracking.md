...
- `/usage cost` in chats: local cost summary aggregated from TraversalAI session logs.
- CLI: `traversalai status --usage` prints a full per-provider breakdown.
- CLI: `traversalai channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
