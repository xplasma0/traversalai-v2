...
```bash
traversalai tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `traversalai tui --deliver`
...
- Check the Gateway logs: `traversalai logs --follow`.
- Confirm the agent can run: `traversalai status` and `traversalai models status`.
...
- No agents in picker: check `traversalai agents list` and your routing config.
