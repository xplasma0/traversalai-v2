...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/traversalai`)
...
If the page fails to load, start the Gateway first: `traversalai gateway`.
...
you revoke it with `traversalai devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.traversalai/traversalai.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop traversalai`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. TraversalAI
...
TraversalAI **blocks** Control UI connections without device identity.
