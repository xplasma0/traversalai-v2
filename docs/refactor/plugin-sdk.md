...
- Publish as `traversalai/plugin-sdk` (or export from core under `traversalai/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `TraversalAIPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: TraversalAIConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: TraversalAIConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: TraversalAIConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: TraversalAIConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: TraversalAIConfig): string;
  };
};
```
...
- Introduce `traversalai/plugin-sdk`.
- Add `api.runtime` to `TraversalAIPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `traversalaiRuntime: ">=2026.2.0"`).
