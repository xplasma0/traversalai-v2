...
LINE 通过 LINE Messaging API 连接到 TraversalAI。该插件作为 webhook 接收器在 Gateway 网关上运行，使用你的 channel access token + channel secret 进行身份验证。
...
```bash
traversalai plugins install @traversalai/line
```

本地检出（从 git 仓库运行时）：

```bash
traversalai plugins install ./extensions/line
```
...
```bash
traversalai pairing list line
traversalai pairing approve line <CODE>
```
