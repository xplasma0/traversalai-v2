...
通过一键模板在 Railway 上部署 TraversalAI，并在浏览器中完成设置。
...
- `https://<your-railway-domain>/traversalai` — 控制面板 UI
...
- `TRAVERSALAI_STATE_DIR=/data/.traversalai`（推荐）
- `TRAVERSALAI_WORKSPACE_DIR=/data/workspace`（推荐）
- `TRAVERSALAI_GATEWAY_TOKEN`（推荐；请视为管理员密钥）
...
这会导出你的 TraversalAI 状态和工作区，方便你迁移到其他主机而不丢失配置或记忆。
