...
你可以在 `~/.traversalai/traversalai.json` 中覆盖此设置：
...
所有日志配置都在 `~/.traversalai/traversalai.json` 的 `logging` 下。
...
- TraversalAI 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `traversalai plugins enable diagnostics-otel` 启用插件。
...
- `traversalai.tokens`（计数器，属性：`traversalai.token`、`traversalai.channel`、`traversalai.provider`、`traversalai.model`）
- `traversalai.cost.usd`（计数器，属性：`traversalai.channel`、`traversalai.provider`、`traversalai.model`）
- `traversalai.run.duration_ms`（直方图，属性：`traversalai.channel`、`traversalai.provider`、`traversalai.model`）
- `traversalai.context.tokens`（直方图，属性：`traversalai.context`、`traversalai.channel`、`traversalai.provider`、`traversalai.model`）

消息流：

- `traversalai.webhook.received`（计数器，属性：`traversalai.channel`、`traversalai.webhook`）
- `traversalai.webhook.error`（计数器，属性：`traversalai.channel`、`traversalai.webhook`）
- `traversalai.webhook.duration_ms`（直方图，属性：`traversalai.channel`、`traversalai.webhook`）
- `traversalai.message.queued`（计数器，属性：`traversalai.channel`、`traversalai.source`）
- `traversalai.message.processed`（计数器，属性：`traversalai.channel`、`traversalai.outcome`）
- `traversalai.message.duration_ms`（直方图，属性：`traversalai.channel`、`traversalai.outcome`）

队列 + 会话：

- `traversalai.queue.lane.enqueue`（计数器，属性：`traversalai.lane`）
- `traversalai.queue.lane.dequeue`（计数器，属性：`traversalai.lane`）
- `traversalai.queue.depth`（直方图，属性：`traversalai.lane` 或 `traversalai.channel=heartbeat`）
- `traversalai.queue.wait_ms`（直方图，属性：`traversalai.lane`）
- `traversalai.session.state`（计数器，属性：`traversalai.state`、`traversalai.reason`）
- `traversalai.session.stuck`（计数器，属性：`traversalai.state`）
- `traversalai.session.stuck_age_ms`（直方图，属性：`traversalai.state`）
- `traversalai.run.attempt`（计数器，属性：`traversalai.attempt`）
...
- `traversalai.webhook.error`
  - `traversalai.channel`、`traversalai.webhook`、`traversalai.chatId`、`traversalai.error`
- `traversalai.message.processed`
  - `traversalai.channel`、`traversalai.outcome`、`traversalai.chatId`、`traversalai.messageId`、`traversalai.sessionKey`、`traversalai.sessionId`、`traversalai.reason`
- `traversalai.session.stuck`
  - `traversalai.state`、`traversalai.ageMs`、`traversalai.queueDepth`、`traversalai.sessionKey`、`traversalai.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `traversalai doctor`。
