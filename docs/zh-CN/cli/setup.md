...
read_when:
  ...
  - 你想设置默认工作区路径
summary: "`traversalai setup` 的 CLI 参考（初始化配置 + 工作区）"
title: setup
x-i18n:
...
初始化 `~/.traversalai/traversalai.json` 和智能体工作区。
...
```bash
traversalai setup
traversalai setup --workspace ~/.traversalai/workspace
```

通过 setup 运行向导：

```bash
traversalai setup --wizard
```
