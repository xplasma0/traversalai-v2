...
- `~/Library/Application Support/TraversalAI/canvas/<session>/...`

Canvas 面板通过**自定义 URL 方案**提供这些文件：

- `traversalai-canvas://<session>/<path>`

示例：

- `traversalai-canvas://main/` → `<canvasRoot>/main/index.html`
- `traversalai-canvas://main/assets/app.css` → `<canvasRoot>/main/assets/app.css`
- `traversalai-canvas://main/widgets/todo/` → `<canvasRoot>/main/widgets/todo/index.html`
...
```bash
traversalai nodes canvas present --node <id>
traversalai nodes canvas navigate --node <id> --url "/"
traversalai nodes canvas eval --node <id> --js "document.title"
traversalai nodes canvas snapshot --node <id>
```
...
```
http://<gateway-host>:18793/__traversalai__/a2ui/
```
...
traversalai nodes canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```

快速测试：

```bash
traversalai nodes canvas a2ui push --node <id> --text "Hello from A2UI"
```
...
```js
window.location.href = "traversalai://agent?message=Review%20this%20design";
```
