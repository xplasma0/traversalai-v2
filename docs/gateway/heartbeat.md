...
- During heartbeat runs, TraversalAI treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `traversalai sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, TraversalAI skips the heartbeat run entirely (no model call).
...
```bash
traversalai system event --text "Check for urgent follow-ups" --mode now
```
