...
> TraversalAI is **not** a hostile multi-tenant security boundary for multiple adversarial users sharing one agent/gateway.
...
TraversalAI security guidance assumes a **personal assistant** deployment: one trusted operator boundary, potentially many agents.
...
## Quick check: `traversalai security audit`
...
```bash
traversalai security audit
traversalai security audit --deep
traversalai security audit --fix
traversalai security audit --json
```

It flags common footguns (Gateway auth exposure, browser control exposure, elevated allowlists, filesystem permissions).

TraversalAI is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
TraversalAI assumes the host and config boundary are trusted:

- If someone can modify Gateway host state/config (`~/.traversalai`, including `traversalai.json`), treat them as a trusted operator.
- Running one Gateway for multiple mutually untrusted/adversarial operators is **not a recommended setup**.
- For mixed-trust teams, split trust boundaries with separate gateways (or at minimum separate OS users/hosts).
- TraversalAI can run multiple gateway instances on one machine, but recommended operations favor clean trust-boundary separation.
- Recommended default: one user per machine/host (or VPS), one gateway for that user, and one or more agents in that gateway.
- If multiple users want TraversalAI, use one VPS/host per user.
...
TraversalAI is designed as a personal assistant security model: one trusted operator boundary, potentially many agents.
...
4. Claim is not listed in [Out of Scope](https://github.com/traversalai/traversalai/blob/main/SECURITY.md#out-of-scope).
...
If you run `--deep`, TraversalAI also attempts a best-effort live Gateway probe.
...
| -------------------------------------------------- | ------------- | ---------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- | -------- |
| `fs.state_dir.perms_world_writable`                | critical      | Other users/processes can modify full TraversalAI state                               | filesystem perms on `~/.traversalai`                                                                 | yes      |
| `fs.config.perms_writable`                         | critical      | Others can change auth/tool policy/config                                          | filesystem perms on `~/.traversalai/traversalai.json`                                                   | yes      |
...
`traversalai security audit` warns when this setting is enabled.

## Insecure or dangerous flags summary

`traversalai security audit` includes `config.insecure_or_dangerous_flags` when
...
Complete `dangerous*` / `dangerously*` config keys defined in TraversalAI config
schema:
...
gateway:
  ...
  auth:
    mode: password
    password: ${TRAVERSALAI_GATEWAY_PASSWORD}
```
...
- TraversalAI gateway is local/loopback first. If you terminate TLS at a reverse proxy, set HSTS on the proxy-facing HTTPS domain there.
- If the gateway itself terminates HTTPS, you can set `gateway.http.securityHeaders.strictTransportSecurity` to emit the HSTS header from TraversalAI responses.
...
TraversalAI stores session transcripts on disk under `~/.traversalai/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.traversalai` (see the audit section below). If you need
...
TraversalAI can refresh the skills list mid-session:
...
- If you install plugins from npm (`traversalai plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.traversalai/extensions/<pluginId>/` (or `$TRAVERSALAI_STATE_DIR/extensions/<pluginId>/`).
  - TraversalAI uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
```bash
traversalai pairing list <channel>
traversalai pairing approve <channel> <code>
```
...
By default, TraversalAI routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
TraversalAI has two separate “who can trigger me?” layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.allowFrom` / `channels.slack.allowFrom`; legacy: `channels.discord.dm.allowFrom`, `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to the account-scoped pairing allowlist store under `~/.traversalai/credentials/` (`<channel>-allowFrom.json` for default account, `<channel>-<accountId>-allowFrom.json` for non-default accounts), merged with config allowlists.
...
- “Paste the full contents of ~/.traversalai or your logs.”

## Unsafe external content bypass flags

TraversalAI includes explicit bypass flags that disable external-content safety wrapping:
...
- `~/.traversalai/traversalai.json`: `600` (user read/write only)
- `~/.traversalai`: `700` (user only)

`traversalai doctor` can warn and offer to tighten these permissions.
...
- Default: `18789`
- Config/flags/env: `gateway.port`, `--port`, `TRAVERSALAI_GATEWAY_PORT`

This HTTP surface includes the Control UI and the canvas host:

- Control UI (SPA assets) (default base path `/`)
- Canvas host: `/__traversalai__/canvas/` and `/__traversalai__/a2ui/` (arbitrary HTML/JS; treat as untrusted content)
...
The Gateway broadcasts its presence via mDNS (`_traversalai-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
4. **Environment variable** (alternative): set `TRAVERSALAI_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
Doctor can generate one for you: `traversalai doctor --generate-gateway-token`.
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `TRAVERSALAI_GATEWAY_PASSWORD`).
...
1. Generate/set a new secret (`gateway.auth.token` or `TRAVERSALAI_GATEWAY_PASSWORD`).
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), TraversalAI
accepts Tailscale Serve identity headers (`tailscale-user-login`) for Control
UI/WebSocket authentication. TraversalAI verifies the identity by resolving the
...
- TraversalAI will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
Assume anything under `~/.traversalai/` (or `$TRAVERSALAI_STATE_DIR/`) may contain secrets or private data:

- `traversalai.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- When sharing diagnostics, prefer `traversalai status --all` (pasteable, secrets redacted) over raw logs.
...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": ["@traversalai", "@mybot"] }
      }
    ]
  }
}
...
- Keep filesystem roots narrow: avoid broad roots like your home directory for agent workspaces/sandbox workspaces. Broad roots can expose sensitive local files (for example state/config under `~/.traversalai`) to filesystem tools.
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.traversalai/sandboxes`
...
- Prefer a dedicated profile for the agent (the default `traversalai` profile).
...
- The Chrome extension relay’s CDP endpoint is auth-gated; only TraversalAI clients can connect.
- Disable browser proxy routing when you don’t need it (`gateway.nodes.browser.mode="off"`).
- Chrome extension relay mode is **not** “safer”; it can take over your existing Chrome tabs. Assume it can act as you in whatever that tab/profile can reach.

### Browser SSRF policy (trusted-network default)

TraversalAI’s browser network policy defaults to the trusted-operator model: private/internal destinations are allowed unless you explicitly disable them.
    ...
    list: [
      {
        ...
        // Session tools can reveal sensitive data from transcripts. By default TraversalAI limits these tools
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `traversalai gateway` process.
2. **Close exposure:** set `gateway.bind: "loopback"` (or disable Tailscale Funnel/Serve) until you understand what happened.
3. **Freeze access:** switch risky DMs/groups to `dmPolicy: "disabled"` / require mentions, and remove `"*"` allow-all entries if you had them.

### Rotate (assume compromise if secrets leaked)

1. Rotate Gateway auth (`gateway.auth.token` / `TRAVERSALAI_GATEWAY_PASSWORD`) and restart.
...
1. Check Gateway logs: `/tmp/traversalai/traversalai-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.traversalai/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).
4. Re-run `traversalai security audit --deep` and confirm critical findings are resolved.

### Collect for a report

- Timestamp, gateway host OS + TraversalAI version
...
Found a vulnerability in TraversalAI? Please report responsibly:

1. Email: [security@traversalai.ai](mailto:security@traversalai.ai)
