...
read_when:
  - Running TraversalAI behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of TraversalAI
...
- You run TraversalAI behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. TraversalAI checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. TraversalAI extracts the user identity from the configured header
...
- TraversalAI can stay on loopback HTTP behind the proxy.
...
If TraversalAI itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://traversalai.example.com
    to: http://traversalai-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
traversalai.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy traversalai:18789 {
...
location / {
    ...
    proxy_pass http://traversalai:18789;
...
`traversalai security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update TraversalAI config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `traversalai security audit` and review findings
