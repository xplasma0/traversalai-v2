...
description: "Complete field-by-field reference for ~/.traversalai/traversalai.json"
summary: "Complete reference for every TraversalAI config key, defaults, and channel settings"
read_when:
...
Every field available in `~/.traversalai/traversalai.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — TraversalAI uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `traversalai doctor` into `whatsapp/default`.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["traversalai-dm"] },
      guilds: {
        "123456789012345678": {
...
- TraversalAI additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
...
Mattermost ships as a plugin: `traversalai plugins install @traversalai/mattermost`.
...
TraversalAI spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
- If you add a non-default account via `traversalai channels add` (or channel onboarding) while still on a single-account top-level channel config, TraversalAI moves account-scoped top-level single-account values into `channels.<channel>.accounts.default` first so the original account keeps working.
- Existing channel-only bindings (no `accountId`) keep matching the default account; account-scoped bindings remain optional.
- `traversalai doctor --fix` also repairs mixed shapes by moving account-scoped top-level single-account values into `accounts.default` when named accounts exist but `default` is missing.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@traversalai"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `traversalai.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, TraversalAI auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, TraversalAI assumes `anthropic` (deprecated).
    ...
    defaults: {
      sandbox: {
        ...
        browser: {
          ...
          network: "traversalai-sandbox-browser",
          cdpPort: 9222,
...
- `none`: per-scope sandbox workspace under `~/.traversalai/sandboxes`
...
noVNC observer access uses VNC auth by default and TraversalAI emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `traversalai-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
    ...
    list: [
      {
        ...
        agentDir: "~/.traversalai/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@traversalai"] },
        sandbox: { mode: "off" },
...
- **`parentForkMaxTokens`**: max parent-session `totalTokens` allowed when creating a forked thread session (default `100000`).
  - If parent `totalTokens` is above this value, TraversalAI starts a fresh thread session instead of inheriting parent transcript history.
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:traversalai`   | All built-in tools (excludes provider plugins)                                           |
...
TraversalAI uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.traversalai/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `TRAVERSALAI_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `traversalai onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `traversalai onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `traversalai onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `traversalai onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `traversalai onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `traversalai onboard --auth-choice minimax-api`.
...
- Loaded from `~/.traversalai/extensions`, `<workspace>/.traversalai/extensions`, plus `plugins.load.paths`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or TRAVERSALAI_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `TRAVERSALAI_GATEWAY_PORT` > `gateway.port` > `18789`.
...
Convenience flags: `--dev` (uses `~/.traversalai-dev` + port `19001`), `--profile <name>` (uses `~/.traversalai-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.traversalai/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-traversalai-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `TRAVERSALAI_SKIP_GMAIL_WATCHER=1` to disable.
...
```json5
{
  canvasHost: {
    ...
    liveReload: true,
    // enabled: false, // or TRAVERSALAI_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__traversalai__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__traversalai__/a2ui/`
...
- Also serves A2UI at `/__traversalai__/a2ui/`.
...
- Hostname defaults to `traversalai`. Override with `TRAVERSALAI_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.traversalai/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `traversalai dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.traversalai/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${TRAVERSALAI_GATEWAY_TOKEN}" },
  },
}
```
...
```json5
{
  secrets: {
    providers: {
      ...
      filemain: {
        source: "file",
        path: "~/.traversalai/secrets.json",
        mode: "json",
        timeoutMs: 5000,
      },
      vault: {
        source: "exec",
        command: "/usr/local/bin/traversalai-vault-resolver",
...
- Legacy OAuth imports from `~/.traversalai/credentials/oauth.json`.
...
- Default log file: `/tmp/traversalai/traversalai-YYYY-MM-DD.log`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `traversalai doctor --fix` can strip unknown keys).
