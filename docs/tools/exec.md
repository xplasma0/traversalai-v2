...
- `gateway`/`node` approvals are controlled by `~/.traversalai/exec-approvals.json`.
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  TraversalAI prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
...
`traversalai security audit` warns when interpreter/runtime `safeBins` entries are missing explicit profiles, and `traversalai doctor --fix` can scaffold missing custom `safeBinProfiles` entries.
