---
summary: "Direct `traversalai agent` CLI runs (with optional delivery)"
read_when:
  - Adding or modifying the agent CLI entrypoint
title: "Agent Send"
---

# `traversalai agent` (direct agent runs)

`traversalai agent` runs a single agent turn without needing an inbound chat message.
...
- Optional delivery back to a channel with `--deliver` + `--channel` (target formats match `traversalai message --target`).
...
```bash
traversalai agent --to +15555550123 --message "status update"
...
traversalai agent --to +15555550123 --message "Summon reply" --deliver
