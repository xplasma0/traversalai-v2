...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `traversalai sandbox explain`.
...
Legacy `agent.*` configs are migrated by `traversalai doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=traversalai-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${TRAVERSALAI_STATE_DIR:-$HOME/.traversalai}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
