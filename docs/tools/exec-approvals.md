...
- **gateway host** → `traversalai` process on the gateway machine
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.traversalai/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.traversalai/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `traversalai approvals allowlist ...`).
- `traversalai security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `traversalai doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.traversalai/exec-approvals.json` directly.

CLI: `traversalai approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
