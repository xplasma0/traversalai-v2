...
ACP sessions let TraversalAI run external coding harnesses (for example Pi, Claude Code, Codex, OpenCode, and Gemini CLI) through an ACP backend plugin.

If you ask TraversalAI in plain language to "run this in Codex" or "start Claude Code in a thread", TraversalAI should route that request to the ACP runtime (not the native sub-agent runtime).
...
Use ACP when you want an external harness runtime. Use sub-agents when you want TraversalAI-native delegated runs.

| Area          | ACP session                           | Sub-agent run                      |
| ------------- | ------------------------------------- | ---------------------------------- |
| Runtime       | ACP backend plugin (for example acpx) | TraversalAI native sub-agent runtime  |
...
- TraversalAI binds a thread to a target ACP session.
- Follow-up messages in that thread route to the bound ACP session.
- ACP output is delivered back to the same thread.
- Unfocus/close/archive/TTL expiry removes the binding.

Thread binding support is adapter-specific. If the active channel adapter does not support thread bindings, TraversalAI returns a clear unsupported/unavailable message.
...
- If `agentId` is omitted, TraversalAI uses `acp.defaultAgent` when configured.
...
- `mode` (optional): `run` (one-shot) or `session` (persistent).
  - default is `run`
  - if `thread: true` and mode omitted, TraversalAI may default to persistent behavior per runtime path
...
If no target resolves, TraversalAI returns a clear error (`Unable to resolve session target: ...`).
...
Some controls depend on backend capabilities. If a backend does not support a control, TraversalAI returns a clear unsupported-control error.
...
When TraversalAI uses the acpx backend, prefer these values for `agentId` unless your acpx config defines custom agent aliases.

Direct acpx CLI usage can also target arbitrary adapters via `--agent <command>`, but that raw escape hatch is an acpx CLI feature (not the normal TraversalAI `agentId` path).
...
```bash
traversalai plugins install @traversalai/acpx
traversalai config set plugins.entries.acpx.enabled true
```

Local workspace install during development:

```bash
traversalai plugins install ./extensions/acpx
```
...
`@traversalai/acpx` now enforces a strict plugin-local pinning model:
...
- TraversalAI startup stays non-blocking while acpx ensure runs.
