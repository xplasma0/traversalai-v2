---
title: Lobster
summary: "Typed workflow runtime for TraversalAI with resumable approval gates."
description: Typed workflow runtime for TraversalAI — composable pipelines with approval gates.
read_when:
...
Lobster is a workflow shell that lets TraversalAI run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: TraversalAI runs one Lobster tool call and gets a structured result.
...
TraversalAI launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
gog.gmail.search --query 'newer_than:1d' \
  | traversalai.invoke --tool message --action send --each --item-key message --args-json '{"provider":"telegram","to":"..."}'
```
...
```lobster
traversalai.invoke --tool llm-task --action json --args-json '{
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In TraversalAI tool calls, set `pipeline` to the file path.
...
Install the Lobster CLI on the **same host** that runs the TraversalAI Gateway (see the [Lobster repo](https://github.com/traversalai/lobster)), and ensure `lobster` is on `PATH`.
...
plugin tools (like `lobster`), TraversalAI keeps core tools enabled. To restrict core
...
→ traversalai calls gmail.list
→ LLM summarizes
→ User: "draft replies to #2 and #5"
→ LLM drafts
→ User: "send #2"
→ traversalai calls gmail.send
...
- **No secrets** — Lobster doesn't manage OAuth; it calls TraversalAI tools that do.
