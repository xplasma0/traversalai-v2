---
summary: "Chrome extension: let TraversalAI drive your existing Chrome tab"
read_when:
...
The TraversalAI Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate traversalai-managed Chrome profile.
...
TraversalAI then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the TraversalAI release (npm package) as static files. There is no separate “build” step.

After upgrading TraversalAI:

- Re-run `traversalai browser extension install` to refresh the installed files under your TraversalAI state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

TraversalAI ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.

Before first attach, open extension Options and set:

- `Port` (default `18792`)
- `Gateway token` (must match `gateway.auth.token` / `TRAVERSALAI_GATEWAY_TOKEN`)
...
- Open the tab you want TraversalAI to control.
...
- `ON`: attached; TraversalAI can drive that tab.
...
Debugging: `traversalai sandbox explain`
...
`traversalai browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `traversalai browser extension install` first to copy the extension to a stable location under your TraversalAI state directory.
...
- **This is not isolated** like the dedicated traversalai-managed profile.
