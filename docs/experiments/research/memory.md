...
read_when:
  - Designing workspace memory (~/.traversalai/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep TraversalAI integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.traversalai/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.traversalai/workspace` as canonical human-readable memory.
...
```
~/.traversalai/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in TraversalAI**, but keep a separable core library.

### Why integrate into TraversalAI?

- TraversalAI already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `traversalai memory recall "…" --k 25 --since 30d`
  - `traversalai memory reflect --since 7d`
...
Pragmatic take for `~/.traversalai/workspace`:
