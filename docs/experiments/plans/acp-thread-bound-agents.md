...
This plan defines how TraversalAI should support ACP coding agents in thread-capable channels (Discord first) with production-level lifecycle and recovery.
...
- TraversalAI core owns ACP control plane concerns
...
TraversalAI should not reimplement ACP transport internals in core.
TraversalAI should not rely on a pure plugin-only interception path for routing.

## North-star architecture (holy grail)

Treat ACP as a first-class control plane in TraversalAI, with pluggable runtime adapters.
...
- move ACP control-plane state to a dedicated SQLite store (WAL mode) under TraversalAI state dir
...
Core (must be in TraversalAI core):
...
- ACP event stream is normalized to TraversalAI reply chunks
...
For the first production backend (`extensions/acpx`), TraversalAI and acpx are
...
- encoded payload fields:
  - `name` (acpx named session; uses TraversalAI `sessionKey`)
...
- TraversalAI consumes ndjson events from `acpx --format json --json-strict`
