...
```bash
traversalai plugins install @traversalai/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
traversalai plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "TraversalAI" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `traversalai pairing list nextcloud-talk`
  - `traversalai pairing approve nextcloud-talk <CODE>`
