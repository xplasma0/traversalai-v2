...
read_when:
  - Setting up Twitch chat integration for TraversalAI
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. TraversalAI connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
traversalai plugins install @traversalai/twitch
```

Local checkout (when running from a git repo):

```bash
traversalai plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `TRAVERSALAI_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "traversalai", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use TRAVERSALAI_TWITCH_ACCESS_TOKEN env var)
...
```bash
TRAVERSALAI_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
