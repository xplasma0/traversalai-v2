---
summary: "Mattermost bot setup and TraversalAI config"
read_when:
...
```bash
traversalai plugins install @traversalai/mattermost
```

Local checkout (when running from a git repo):

```bash
traversalai plugins install ./extensions/mattermost
```
...
4. Configure TraversalAI and start the gateway.
...
- Approve via:
  - `traversalai pairing list mattermost`
  - `traversalai pairing approve mattermost <CODE>`
...
Use these target formats with `traversalai message send` or cron/webhooks:
