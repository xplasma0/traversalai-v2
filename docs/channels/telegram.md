...
```
    ...
    Telegram does **not** use `traversalai channels login telegram`; configure token in config/env, then start gateway.
...
<Tabs>
  <Tab title="DM policy">
    ...
    If you upgraded and your config contains `@username` allowlist entries, run `traversalai doctor --fix` to resolve them (best-effort; requires a Telegram bot token).
...
```
    ...
    - or read `chat.id` from `traversalai logs --follow`
...
- DM messages can carry `message_thread_id`; TraversalAI routes them with thread-aware session keys and preserves thread ID for replies.
...
<AccordionGroup>
  <Accordion title="Live stream preview (message edits)">
    TraversalAI can stream partial replies by sending a temporary Telegram message and editing it as text arrives.
    ...
    For text-only replies, TraversalAI keeps the same preview message and performs a final edit in place (no second message).

    For complex replies (for example media payloads), TraversalAI falls back to normal final delivery and then cleans up the preview message.

    Preview streaming is separate from block streaming. When block streaming is explicitly enabled for Telegram, TraversalAI skips the preview stream to avoid double-streaming.
  ...
  <Accordion title="Formatting and HTML fallback">
    ...
    - If Telegram rejects parsed HTML, TraversalAI retries as plain text.
...
```
    ...
    - `~/.traversalai/telegram/sticker-cache.json`
...
```
  ...
  <Accordion title="Reaction notifications">
    ...
    When enabled, TraversalAI enqueues system events like:
...
```bash
traversalai message send --channel telegram --target 123456789 --message "hi"
traversalai message send --channel telegram --target @name --message "hi"
```
...
<AccordionGroup>
  <Accordion title="Bot does not respond to non mention group messages">
    ...
    - `traversalai channels status` warns when config expects unmentioned group messages.
    - `traversalai channels status --probe` can check explicit numeric group IDs; wildcard `"*"` cannot be membership-probed.
  ...
  <Accordion title="Bot not seeing group messages at all">
    ...
    - review logs: `traversalai logs --follow` for skip reasons
  ...
  <Accordion title="Polling or network instability">
    ...
    - If logs include `TypeError: fetch failed` or `Network request for 'getUpdates' failed!`, TraversalAI now retries these as recoverable network errors.
...
```
    ...
    - Environment overrides (temporary):
      - `TRAVERSALAI_TELEGRAM_DISABLE_AUTO_SELECT_FAMILY=1`
      - `TRAVERSALAI_TELEGRAM_ENABLE_AUTO_SELECT_FAMILY=1`
      - `TRAVERSALAI_TELEGRAM_DNS_RESULT_ORDER=ipv4first`
...
- `channels.telegram.allowFrom`: DM allowlist (numeric Telegram user IDs). `open` requires `"*"`. `traversalai doctor --fix` can resolve legacy `@username` entries to IDs.
- `channels.telegram.groupPolicy`: `open | allowlist | disabled` (default: allowlist).
- `channels.telegram.groupAllowFrom`: group sender allowlist (numeric Telegram user IDs). `traversalai doctor --fix` can resolve legacy `@username` entries to IDs.
