...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects TraversalAI to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
traversalai plugins install @traversalai/feishu
```

Local checkout (when running from a git repo):

```bash
traversalai plugins install ./extensions/feishu
```
...
If you just installed TraversalAI, run the wizard:
...
2. Configuring app credentials in TraversalAI
...
1. You already ran `traversalai channels add` for Feishu
2. The gateway is running (`traversalai gateway status`)
...
Edit `~/.traversalai/traversalai.json`:
...
```bash
traversalai pairing approve feishu <CODE>
```
...
- **Approve pairing**:
  ...
  ```bash
  traversalai pairing list feishu
  traversalai pairing approve feishu <CODE>
  ```
...
2. Run `traversalai logs --follow` and look for `chat_id`
...
2. Run `traversalai logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `traversalai gateway status`  | Show gateway status           |
| `traversalai gateway install` | Install/start gateway service |
| `traversalai gateway stop`    | Stop gateway service          |
| `traversalai gateway restart` | Restart gateway service       |
| `traversalai logs --follow`   | Tail gateway logs             |
...
4. Check logs: `traversalai logs --follow`
...
5. Ensure the gateway is running: `traversalai gateway status`
6. Check logs: `traversalai logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.traversalai/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.traversalai/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
