...
read_when:
  ...
  - Reviewing TraversalAI security posture
title: "Pairing"
---

# Pairing

“Pairing” is TraversalAI’s explicit **owner approval** step.
...
Stored under `~/.traversalai/credentials/`:
...
3. On your phone, open the TraversalAI iOS app → Settings → Gateway.
...
Stored under `~/.traversalai/devices/`:
...
- The legacy `node.pair.*` API (CLI: `traversalai nodes pending/approve`) is a
