...
Matrix is an open, decentralized messaging protocol. TraversalAI connects as a Matrix **user**
...
```bash
traversalai plugins install @traversalai/matrix
```

Local checkout (when running from a git repo):

```bash
traversalai plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `traversalai plugins install @traversalai/matrix`
   - From a local checkout: `traversalai plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: TraversalAI calls the same
     login endpoint, stores the access token in `~/.traversalai/credentials/matrix/credentials.json`,
...
- On first connection, TraversalAI requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  TraversalAI logs a warning.
...
`~/.traversalai/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `traversalai pairing list matrix`
  - `traversalai pairing approve matrix <CODE>`
...
- On startup, TraversalAI resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
