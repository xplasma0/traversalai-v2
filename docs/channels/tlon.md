...
Tlon is a decentralized messenger built on Urbit. TraversalAI connects to your Urbit ship and can
...
```bash
traversalai plugins install @traversalai/tlon
```

Local checkout (when running from a git repo):

```bash
traversalai plugins install ./extensions/tlon
```
...
By default, TraversalAI blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `traversalai message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, TraversalAI replies in-thread.
