...
read_when:
  - You want TraversalAI to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables TraversalAI to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`traversalai onboard`) and `traversalai channels add` list optional channel plugins.
...
```bash
traversalai plugins install @traversalai/nostr
```

Use a local checkout (dev workflows):

```bash
traversalai plugins install --link <path-to-traversalai>/extensions/nostr
```
...
```json
{
  "channels": {
    "nostr": {
      ...
      "profile": {
        "name": "traversalai",
        "displayName": "TraversalAI",
        "about": "Personal assistant DM bot",
        "picture": "https://example.com/avatar.png",
        "banner": "https://example.com/banner.png",
        "website": "https://example.com",
        "nip05": "traversalai@example.com",
        "lud16": "traversalai@example.com"
      }
    }
  }
}
