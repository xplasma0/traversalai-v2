---
summary: "Synology Chat webhook setup and TraversalAI config"
read_when:
  - Setting up Synology Chat with TraversalAI
...
```bash
traversalai plugins install ./extensions/synology-chat
```
...
3. Point the outgoing webhook URL to your TraversalAI gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Configure `channels.synology-chat` in TraversalAI.
...
- Pairing approvals work with:
  - `traversalai pairing list synology-chat`
  - `traversalai pairing approve synology-chat <CODE>`
...
```bash
traversalai message send --channel synology-chat --target 123456 --text "Hello from TraversalAI"
traversalai message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
```
