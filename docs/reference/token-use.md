---
summary: "How TraversalAI builds prompt context and reports token usage + costs"
read_when:
...
TraversalAI tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

TraversalAI assembles its own system prompt on every run. It includes:
...
For images, TraversalAI downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from TraversalAI session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `traversalai status --usage` and `traversalai channels list` show
...
`cacheWrite`. If pricing is missing, TraversalAI shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. TraversalAI can
...
Anthropic's 1M context window is currently beta-gated. TraversalAI can inject the
...
TraversalAI skips the `context-1m-*` beta header because Anthropic currently
