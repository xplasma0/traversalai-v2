...
TraversalAI features that can generate provider usage or paid API calls.
...
- `traversalai status --usage` and `traversalai channels list` show provider **usage windows**
...
TraversalAI can pick up credentials from:
...
- `traversalai models status --json`
...
`traversalai models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
