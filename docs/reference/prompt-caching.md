...
- With Anthropic API-key auth profiles, TraversalAI seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, TraversalAI injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
TraversalAI exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.traversalai/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$TRAVERSALAI_STATE_DIR/logs/cache-trace.jsonl`
...
- `TRAVERSALAI_CACHE_TRACE=1` enables cache tracing.
- `TRAVERSALAI_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `TRAVERSALAI_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `TRAVERSALAI_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `TRAVERSALAI_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
