...
TraversalAI integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- TraversalAI owns lifecycle when `channels.signal.autoStart=true`.
...
- TraversalAI spawns `imsg rpc` as a child process (legacy iMessage integration).
