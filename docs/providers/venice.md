---
summary: "Use Venice AI privacy-focused models in TraversalAI"
read_when:
  - You want privacy-focused inference in TraversalAI
...
```bash
traversalai onboard --auth-choice venice-api-key
```
...
```bash
traversalai agent --model venice/llama-3.3-70b --message "Hello, are you working?"
```

## Model Selection

After setup, TraversalAI shows all available Venice models. Pick based on your needs:
...
```bash
traversalai models set venice/claude-opus-45
traversalai models set venice/llama-3.3-70b
```

List all available models:

```bash
traversalai models list | grep venice
```

## Configure via `traversalai configure`

1. Run `traversalai configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
TraversalAI automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
traversalai agent --model venice/llama-3.3-70b --message "Quick health check"

# Use Claude via Venice (anonymized)
traversalai agent --model venice/claude-opus-45 --message "Summarize this task"

# Use uncensored model
traversalai agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
traversalai agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
traversalai agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `traversalai models list` to see currently available models. Some models may be temporarily offline.
