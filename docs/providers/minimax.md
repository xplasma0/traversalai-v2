---
summary: "Use MiniMax M2.1 in TraversalAI"
read_when:
  - You want MiniMax models in TraversalAI
...
```bash
traversalai plugins enable minimax-portal-auth  # skip if already loaded.
traversalai gateway restart  # restart if gateway is already running
traversalai onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/traversalai/traversalai/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `traversalai.json`:
...
- Use `traversalai models list` and `traversalai models set minimax/MiniMax-M2.1` to switch.
...
- Running `traversalai configure` and selecting **MiniMax M2.1**, or
