...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in TraversalAI
...
Running TraversalAI via Node with `tsx` fails at startup with:

```
[traversalai] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `traversalai status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch traversalai.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node traversalai.mjs status` works on Node 25.
