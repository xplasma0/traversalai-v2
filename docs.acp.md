# TraversalAI ACP Bridge

This document describes how the TraversalAI ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`traversalai acp` exposes an ACP agent over stdio and forwards prompts to a running
TraversalAI Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
- ACP client spawns `traversalai acp` and speaks ACP messages over stdio.
...
`traversalai acp` resolves the Gateway URL and auth from CLI flags or config:
