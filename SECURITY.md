...
If you believe you've found a security issue in TraversalAI, please report it privately.

## Reporting

Report vulnerabilities directly to the repository where the issue lives:

- **Core CLI and gateway** — [traversalai/traversalai](https://github.com/traversalai/traversalai)
- **macOS desktop app** — [traversalai/traversalai](https://github.com/traversalai/traversalai) (apps/macos)
- **iOS app** — [traversalai/traversalai](https://github.com/traversalai/traversalai) (apps/ios)
- **Android app** — [traversalai/traversalai](https://github.com/traversalai/traversalai) (apps/android)
- **ClawHub** — [traversalai/clawhub](https://github.com/traversalai/clawhub)
- **Trust and threat model** — [traversalai/trust](https://github.com/traversalai/trust)

For issues that don't fit a specific repo, or if you're unsure, email **[security@traversalai.ai](mailto:security@traversalai.ai)** and we'll route it.

For full reporting instructions see our [Trust page](https://trust.traversalai.ai).
...
- Tested version details (TraversalAI version and/or commit SHA).
- Reproducible PoC against latest `main` or latest released version.
- Demonstrated impact tied to TraversalAI's documented trust boundaries.
- For exposed-secret reports: proof the credential is TraversalAI-owned (or grants access to TraversalAI-operated infrastructure/services).
...
**Jamieson O'Reilly** ([@theonejvo](https://twitter.com/theonejvo)) is Security & Trust at TraversalAI. Jamieson is the founder of [Dvuln](https://dvuln.com) and brings extensive experience in offensive security, penetration testing, and security program development.

## Bug Bounties

TraversalAI is a labor of love. There is no bug bounty program and no budget for paid reports. Please still disclose responsibly so we can fix issues quickly.
...
TraversalAI does **not** model one gateway as a multi-tenant, adversarial user boundary.

- Authenticated Gateway callers are treated as trusted operators for that gateway instance.
- Session identifiers (`sessionKey`, session IDs, labels) are routing controls, not per-user authorization boundaries.
- If one operator can view data from another operator on the same gateway, that is expected in this trust model.
- TraversalAI can technically run multiple gateway instances on one machine, but recommended operations are clean separation by trust boundary.
- Recommended mode: one user per machine/host (or VPS), one gateway for that user, and one or more agents inside that gateway.
- If multiple users need TraversalAI, use one VPS (or host/OS user boundary) per user.
...
- This is expected in TraversalAI's one-user trusted-operator model. If you need isolation, enable sandbox mode (`non-main`/`all`) and keep strict tool policy.

## Trusted Plugin Concept (Core)

Plugins/extensions are part of TraversalAI's trusted computing base for a gateway.
...
- Using TraversalAI in ways that the docs recommend not to
...
- Reports that require write access to trusted local state (`~/.traversalai`, workspace files like `MEMORY.md` / `memory/*.md`)
...
- Exposed secrets that are third-party/user-controlled credentials (not TraversalAI-owned and not granting access to TraversalAI-operated infrastructure/services) without demonstrated TraversalAI impact
- Reports whose only claim is host-side exec when sandbox runtime is disabled/unavailable (documented default behavior in the trusted-operator model), without a boundary bypass.
- Reports whose only claim is that a platform-provided upload destination URL is untrusted (for example Microsoft Teams `fileConsent/invoke` `uploadInfo.uploadUrl`) without proving attacker control in an authenticated production flow.

## Deployment Assumptions

TraversalAI security guidance assumes:

- The host where TraversalAI runs is within a trusted OS/admin boundary.
- Anyone who can modify `~/.traversalai` state/config (including `traversalai.json`) is effectively a trusted operator.
...
TraversalAI's security model is "personal assistant" (one trusted operator, potentially many agents), not "shared multi-tenant bus."
...
TraversalAI separates routing from execution, but both remain inside the same operator trust boundary:
...
- Plugins can execute with the same OS privileges as the TraversalAI process.
- Runtime helpers (for example `runtime.system.runCommandWithTimeout`) are convenience APIs, not a sandbox boundary.
- Only install plugins you trust, and prefer `plugins.allow` to pin explicit trusted plugin ids.

## Temp Folder Boundary (Media/Sandbox)

TraversalAI uses a dedicated temp root for local media handoff and sandbox-adjacent temp artifacts:

- Preferred temp root: `/tmp/traversalai` (when available and safe on the host).
- Fallback temp root: `os.tmpdir()/traversalai` (or `traversalai-<uid>` on multi-user hosts).

Security boundary notes:

- Sandbox media validation allows absolute temp paths only under the TraversalAI-managed temp root.
- Arbitrary host tmp paths are not treated as trusted media roots.
- Plugin/extension code should use TraversalAI temp helpers (`resolvePreferredTraversalAITmpDir`, `buildRandomTempFilePath`, `withTempDownloadPath`) rather than raw `os.tmpdir()` defaults when handling media files.
- Enforcement reference points:
  - temp root resolver: `src/infra/tmp-traversalai-dir.ts`
...
For threat model + hardening guidance (including `traversalai security audit --deep` and `--fix`), see:

- `https://docs.traversalai.ai/gateway/security`
...
TraversalAI's web interface (Gateway Control UI + HTTP endpoints) is intended for **local use only**.

- Recommended: keep the Gateway **loopback-only** (`127.0.0.1` / `::1`).
  - Config: `gateway.bind="loopback"` (default).
  - CLI: `traversalai gateway run --bind loopback`.
- `gateway.controlUi.dangerouslyDisableDeviceAuth` is intended for localhost-only break-glass use.
  - TraversalAI keeps deployment flexibility by design and does not hard-forbid non-local setups.
  - Non-local and other risky configurations are surfaced by `traversalai security audit` as dangerous findings.
...
- Canvas host note: network-visible canvas is **intentional** for trusted node scenarios (LAN/tailnet).
  ...
  - Expected routes: `/__traversalai__/canvas/`, `/__traversalai__/a2ui/`.
  - This deployment model alone is not a security vulnerability.
- Do **not** expose it to the public internet (no direct bind to `0.0.0.0`, no public reverse proxy). It is not hardened for public exposure.
- If you need remote access, prefer an SSH tunnel or Tailscale serve/funnel (so the Gateway still binds to loopback), plus strong Gateway auth.
- The Gateway HTTP surface includes the canvas host (`/__traversalai__/canvas/`, `/__traversalai__/a2ui/`). Treat canvas content as sensitive/untrusted and avoid exposing it beyond loopback unless you understand the risk.

## Runtime Requirements

### Node.js Version

TraversalAI requires **Node.js 22.12.0 or later** (LTS). This version includes important security patches:
...
When running TraversalAI in Docker:
...
docker run --read-only --cap-drop=ALL \
  -v traversalai-data:/app/data \
  traversalai/traversalai:latest
```
