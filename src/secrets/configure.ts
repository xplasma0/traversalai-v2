...
function getSecretProviders(config: TraversalAIConfig): Record<string, SecretProviderConfig> {
...
function removeSecretProvider(config: TraversalAIConfig, providerAlias: string): boolean {
  if (!isRecord(config.secrets?.providers)) {
    return false;
  }
...
function buildCandidates(config: TraversalAIConfig): ConfigureCandidate[] {
...
function toSourceChoices(config: TraversalAIConfig): Array<{ value: SecretRefSource; label: string }> {
...
async function configureProvidersInteractive(config: TraversalAIConfig): Promise<void> {
  while (true) {
...
function collectProviderPlanChanges(params: { original: TraversalAIConfig; next: TraversalAIConfig }): {
...
export async function runSecretsConfigureInteractive(
  ...
  if (!params.providersOnly) {
    ...
    if (candidates.length === 0) {
      throw new Error("No configurable secret-bearing fields found in traversalai.json.");
    }
  ...
  const plan: SecretsApplyPlan = {
    ...
    generatedBy: "traversalai secrets configure",
