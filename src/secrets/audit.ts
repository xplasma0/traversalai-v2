...
import { resolveStateDir, type TraversalAIConfig } from "../config/config.js";
...
function collectAuthStorePaths(config: TraversalAIConfig, stateDir: string): string[] {
...
export async function runSecretsAudit(
  ...
  const previousAuthStoreReadOnly = process.env.TRAVERSALAI_AUTH_STORE_READONLY;
  ...
  try {
    ...
    const config = snapshot.valid ? snapshot.config : ({} as TraversalAIConfig);
    ...
    if (previousAuthStoreReadOnly === undefined) {
      delete process.env.TRAVERSALAI_AUTH_STORE_READONLY;
    } else {
      process.env.TRAVERSALAI_AUTH_STORE_READONLY = previousAuthStoreReadOnly;
    }
  }
}
