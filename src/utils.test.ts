...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDirSync("traversalai-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    withTempDirSync("traversalai-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", () => {
    withTempDirSync("traversalai-lid-a-", (first) => {
      withTempDirSync("traversalai-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.traversalai when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "traversalai-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers TRAVERSALAI_HOME over HOME", () => {
    vi.stubEnv("TRAVERSALAI_HOME", "/srv/traversalai-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/traversalai-home"));
...
describe("shortenHomePath", () => {
  it("uses $TRAVERSALAI_HOME prefix when TRAVERSALAI_HOME is set", () => {
    vi.stubEnv("TRAVERSALAI_HOME", "/srv/traversalai-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/traversalai-home")}/.traversalai/traversalai.json`)).toBe(
      "$TRAVERSALAI_HOME/.traversalai/traversalai.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $TRAVERSALAI_HOME replacement when TRAVERSALAI_HOME is set", () => {
    vi.stubEnv("TRAVERSALAI_HOME", "/srv/traversalai-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/traversalai-home")}/.traversalai/traversalai.json`),
    ).toBe("config: $TRAVERSALAI_HOME/.traversalai/traversalai.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/traversalai")).toBe(path.resolve(os.homedir(), "traversalai"));
  });
  ...
  it("prefers TRAVERSALAI_HOME for tilde expansion", () => {
    vi.stubEnv("TRAVERSALAI_HOME", "/srv/traversalai-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/traversalai")).toBe(path.resolve("/srv/traversalai-home", "traversalai"));
