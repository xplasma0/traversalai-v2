...
vi.mock("./traversalai-root.js", () => ({
  resolveTraversalAIPackageRoot: vi.fn(async () => null),
  resolveTraversalAIPackageRootSync: vi.fn(() => null),
}));
...
let traversalaiRoot: typeof import("./traversalai-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    traversalaiRoot = await import("./traversalai-root.js");
  });
  ...
  it("uses resolveTraversalAIPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/traversalai");
    (
      traversalaiRoot.resolveTraversalAIPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/traversalai"))).resolves.toBe(
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "traversalai" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "traversalai.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-traversalai");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    const pkgRoot = abs("fixtures/traversalai-bundle");
    (
      traversalaiRoot.resolveTraversalAIPackageRootSync as unknown as ReturnType<typeof vi.fn>
