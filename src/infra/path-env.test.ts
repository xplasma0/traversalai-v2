...
let ensureTraversalAICliOnPath: typeof import("./path-env.js").ensureTraversalAICliOnPath;

describe("ensureTraversalAICliOnPath", () => {
  const envKeys = [
    "PATH",
    "TRAVERSALAI_PATH_BOOTSTRAPPED",
    "TRAVERSALAI_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureTraversalAICliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling traversalai exists", () => {
    const tmp = abs("/tmp/traversalai-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "traversalai");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.TRAVERSALAI_PATH_BOOTSTRAPPED;
  ...
  it("is idempotent", () => {
    ...
    process.env.TRAVERSALAI_PATH_BOOTSTRAPPED = "1";
    ensureTraversalAICliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/traversalai-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "traversalai");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/traversalai-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "traversalai");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "traversalai");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/traversalai-path/case-linuxbrew");
