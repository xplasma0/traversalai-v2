...
import { POSIX_TRAVERSALAI_TMP_DIR, resolvePreferredTraversalAITmpDir } from "./tmp-traversalai-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredTraversalAITmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `traversalai-${uid}`);
}
...
function resolveWithMocks(params: {
  ...
  const wrappedLstatSync = vi.fn((target: string) => {
    if (target === POSIX_TRAVERSALAI_TMP_DIR) {
  ...
  const resolved = resolvePreferredTraversalAITmpDir({
    accessSync,
    chmodSync,
...
describe("resolvePreferredTraversalAITmpDir", () => {
  it("prefers /tmp/traversalai when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_TRAVERSALAI_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/traversalai when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_TRAVERSALAI_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/traversalai when /tmp/traversalai is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/traversalai when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/traversalai is a symlink", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => true,
      uid: 501,
  ...
  it("falls back when /tmp/traversalai is not owned by the current user", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 0,
  ...
  it("falls back when /tmp/traversalai is group/other writable", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 501,
  ...
  it("throws when fallback path is a symlink", () => {
    ...
    ).toThrow(/Unsafe fallback TraversalAI temp dir/);
  });
