...
type EnsureTraversalAIPathOpts = {
  execPath?: string;
  cwd?: string;
  homeDir?: string;
...
function candidateBinDirs(opts: EnsureTraversalAIPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `traversalai` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "traversalai");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.TRAVERSALAI_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "traversalai"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `traversalai` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureTraversalAICliOnPath(opts: EnsureTraversalAIPathOpts = {}) {
  if (isTruthyEnvValue(process.env.TRAVERSALAI_PATH_BOOTSTRAPPED)) {
    return;
  }
