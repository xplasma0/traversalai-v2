...
export function triggerTraversalAIRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.TRAVERSALAI_SYSTEMD_UNIT,
        process.env.TRAVERSALAI_PROFILE,
      );
  ...
  const label =
    process.env.TRAVERSALAI_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.TRAVERSALAI_PROFILE);
