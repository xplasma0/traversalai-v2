...
const DEFAULT_PACKAGE_NAME = "traversalai";
const ALT_PACKAGE_NAME = "traversalai";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME, ALT_PACKAGE_NAME]);
...
function normalizeTag(tag?: string) {
  ...
  if (trimmed.startsWith(`${ALT_PACKAGE_NAME}@`)) {
    return trimmed.slice(`${ALT_PACKAGE_NAME}@`.length);
  }
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "traversalai-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "traversalai.mjs");
    ...
    const doctorStep = await runStep(
      step("traversalai doctor", doctorArgv, gitRoot, { TRAVERSALAI_UPDATE_IN_PROGRESS: "1" }),
    );
