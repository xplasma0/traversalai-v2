...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.TRAVERSALAI_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*traversalai-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "traversalai";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "traversalai");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "traversalai-gw");
  ...
  records.push(`_traversalai-gw._tcp IN PTR ${instanceLabel}._traversalai-gw._tcp`);
  records.push(`${instanceLabel}._traversalai-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._traversalai-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; traversalai-content-hash: ${contentHash}\n${contentBody}`;
}
