...
describe("fs-safe", () => {
  it("reads a local file safely", async () => {
    const dir = await tempDirs.make("traversalai-fs-safe-");
  ...
  it("blocks traversal outside root", async () => {
    const root = await tempDirs.make("traversalai-fs-safe-root-");
    const outside = await tempDirs.make("traversalai-fs-safe-outside-");
