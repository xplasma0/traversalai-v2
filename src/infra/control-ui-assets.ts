...
import { resolveTraversalAIPackageRoot, resolveTraversalAIPackageRootSync } from "./traversalai-root.js";

const CONTROL_UI_DIST_PATH_SEGMENTS = ["dist", "control-ui", "index.html"] as const;
const CORE_PACKAGE_NAMES = new Set(["traversalai", "traversalai"]);
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveTraversalAIPackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "traversalai" + dist/control-ui/index.html
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  const packageRoot = resolveTraversalAIPackageRootSync({
    argv1,
    moduleUrl: opts.moduleUrl,
    cwd,
  });
  ...
  if (argv1Dir) {
    // traversalai.mjs or dist/<bundle>.js
