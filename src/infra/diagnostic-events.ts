...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __traversalaiDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__traversalaiDiagnosticEventsState) {
    globalStore.__traversalaiDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__traversalaiDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: TraversalAIConfig): boolean {
