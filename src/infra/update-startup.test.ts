...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "traversalai-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["TRAVERSALAI_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.TRAVERSALAI_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveTraversalAIPackageRoot } = await import("./traversalai-root.js"));
  ...
  function mockPackageUpdateStatus(tag = "latest", version = "2.0.0") {
    vi.mocked(resolveTraversalAIPackageRoot).mockResolvedValue("/opt/traversalai");
    vi.mocked(checkUpdateStatus).mockResolvedValue({
      root: "/opt/traversalai",
      installKind: "package",
      packageManager: "npm",
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/traversalai/dist/entry.js"];
    try {
      await runGatewayUpdateCheck({
        cfg: {
          update: {
