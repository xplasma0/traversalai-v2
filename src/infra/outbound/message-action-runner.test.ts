...
describe("runMessageAction sandboxed media validation", () => {
  ...
  it("allows media paths under preferred TraversalAI tmp root", async () => {
    const tmpRoot = resolvePreferredTraversalAITmpDir();
    ...
    try {
      ...
      const hostTmpOutsideTraversalAI = path.join(os.tmpdir(), "outside-traversalai", "test-media.png");
      await expect(
        runMessageAction({
          cfg: slackConfig,
          action: "send",
          params: {
            channel: "slack",
            target: "#C12345678",
            media: hostTmpOutsideTraversalAI,
            message: "",
          },
          sandboxRoot: sandboxDir,
          dryRun: true,
