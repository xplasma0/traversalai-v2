...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: TraversalAIConfig | null = null;
  ...
  get(key: string, cfg: TraversalAIConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: TraversalAIConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  clear(cfg?: TraversalAIConfig): void {
    this.cache.clear();
    if (cfg) {
  ...
  private resetIfConfigChanged(cfg: TraversalAIConfig): void {
