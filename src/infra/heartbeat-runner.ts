...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: TraversalAIConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: TraversalAIConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: TraversalAIConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: TraversalAIConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: TraversalAIConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: TraversalAIConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: TraversalAIConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
