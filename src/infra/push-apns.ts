...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.TRAVERSALAI_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.TRAVERSALAI_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set TRAVERSALAI_APNS_TEAM_ID and TRAVERSALAI_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.TRAVERSALAI_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.TRAVERSALAI_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.TRAVERSALAI_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set TRAVERSALAI_APNS_PRIVATE_KEY_P8 or TRAVERSALAI_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading TRAVERSALAI_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
...
function createTraversalAIPushMetadata(params: {
...
export async function sendApnsAlert(params: {
  ...
  const payload = {
    ...
    traversalai: createTraversalAIPushMetadata({
      kind: "push.test",
      nodeId: params.nodeId,
    }),
  };
