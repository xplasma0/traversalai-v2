...
describe("resolveEffectiveHomeDir", () => {
  it("prefers TRAVERSALAI_HOME over HOME and USERPROFILE", () => {
    const env = {
    ...
    expect(resolveEffectiveHomeDir(env, () => "/fallback")).toBe(
      path.resolve("/srv/traversalai-home"),
    );
  });
  ...
  it("expands TRAVERSALAI_HOME when set to ~", () => {
    const env = {
      TRAVERSALAI_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for TRAVERSALAI_HOME", () => {
    const result = resolveRequiredHomeDir(
      { TRAVERSALAI_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when TRAVERSALAI_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ TRAVERSALAI_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { TRAVERSALAI_HOME: "/srv/traversalai-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/traversalai-home")}/x`);
  });
