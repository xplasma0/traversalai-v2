...
export function isCommandEnabled(cfg: TraversalAIConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: TraversalAIConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as TraversalAIConfig),
...
export function getCommandDetection(_cfg?: TraversalAIConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: TraversalAIConfig) {
