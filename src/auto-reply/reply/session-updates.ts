...
export async function prependSystemEvents(params: {
  ...
  const resolveSystemEventTimezone = (cfg: TraversalAIConfig) => {
  ...
  const formatSystemEventTimestamp = (ts: number, cfg: TraversalAIConfig) => {
...
export async function ensureSkillSnapshot(params: {
  ...
  if (process.env.TRAVERSALAI_TEST_FAST === "1") {
