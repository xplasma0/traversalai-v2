...
import { scheduleGatewaySigusr1Restart, triggerTraversalAIRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting TraversalAI in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerTraversalAIRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting TraversalAI via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
