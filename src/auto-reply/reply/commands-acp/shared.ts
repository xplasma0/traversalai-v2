...
export function resolveConfiguredAcpBackendId(cfg: TraversalAIConfig): string {
  return cfg.acp?.backend?.trim() || "acpx";
}

export function resolveAcpInstallCommandHint(cfg: TraversalAIConfig): string {
  ...
  if (backendId === "acpx") {
    ...
    if (existsSync(localPath)) {
      return `traversalai plugins install ${localPath}`;
    }
    return "traversalai plugins install @traversalai/acpx";
  }
