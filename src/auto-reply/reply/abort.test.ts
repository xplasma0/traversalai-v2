...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "traversalai-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop traversalai!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@traversalai_bot", { botUsername: "traversalai_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@traversalai_bot", { botUsername: "traversalai_bot" })).toBe(true);
