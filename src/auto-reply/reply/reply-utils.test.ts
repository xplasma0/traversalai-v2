...
describe("matchesMentionWithExplicit", () => {
  const mentionRegexes = [/\btraversalai\b/i];
  ...
  it("resolves known variables, aliases, and case-insensitive tokens", () => {
    const cases = [
      ...
      },
      {
        ...
        values: { identityName: "TraversalAI" },
        expected: "[TraversalAI]",
      },
      {
      ...
      },
      {
        ...
        values: {
          identityName: "TraversalAI",
          provider: "anthropic",
          model: "claude-opus-4-5",
          thinkingLevel: "high",
        },
        expected: "[TraversalAI] anthropic/claude-opus-4-5 (think:high)",
      },
    ] as const;
    for (const testCase of cases) {
