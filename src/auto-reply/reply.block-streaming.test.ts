...
function createReplyConfig(home: string, streamMode?: "block"): TraversalAIConfig {
  return {
    agents: {
      defaults: {
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeHarness("traversalai-stream-", async (home) => {
...
describe("block streaming", () => {
  beforeEach(() => {
    vi.stubEnv("TRAVERSALAI_TEST_FAST", "1");
