...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with process.env", async () => {
    await withEnvAsync({ TRAVERSALAI_BASE_ENV: "base" }, async () => {
      const result = await runCommandWithTimeout(
        [
          process.execPath,
          "-e",
          'process.stdout.write((process.env.TRAVERSALAI_BASE_ENV ?? "") + "|" + (process.env.TRAVERSALAI_TEST_ENV ?? ""))',
        ],
        {
          timeoutMs: 5_000,
          env: { TRAVERSALAI_TEST_ENV: "ok" },
        },
      );
