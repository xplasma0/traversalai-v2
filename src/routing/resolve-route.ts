...
function listAgents(cfg: TraversalAIConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

function pickFirstExistingAgentId(cfg: TraversalAIConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: TraversalAIConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<TraversalAIConfig, EvaluatedBindingsCache>();
