...
const TEMP_MEDIA_PREFIX = "traversalai-media-";
...
function createGroqAudioConfig(): TraversalAIConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): TraversalAIConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): TraversalAIConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: TraversalAIConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  ...
  beforeAll(async () => {
    const baseDir = resolvePreferredTraversalAITmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        TRAVERSALAI_AGENT_DIR: isolatedAgentDir,
