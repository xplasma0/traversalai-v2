...
describe("canvas host state dir defaults", () => {
  it("uses TRAVERSALAI_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("traversalai-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("TraversalAI Canvas");
      } finally {
        await handler.close();
      }
    });
