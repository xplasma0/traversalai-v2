...
<title>TraversalAI Canvas</title>
<style>
...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.traversalaiCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.traversalaiCanvasA2UIAction &&
        typeof window.traversalaiCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.traversalaiSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("traversalai:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android TraversalAI node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.traversalaiSendUserAction === "function"
        ? window.traversalaiSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.TRAVERSALAI_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.TRAVERSALAI_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>TraversalAI Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
