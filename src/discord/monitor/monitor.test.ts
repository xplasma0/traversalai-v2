...
describe("agent components", () => {
  const createCfg = (): TraversalAIConfig => ({}) as TraversalAIConfig;
...
describe("discord component interactions", () => {
  const createCfg = (): TraversalAIConfig =>
    ({
      channels: {
        discord: {
          replyToMode: "first",
  ...
  beforeEach(() => {
    ...
    resolveStorePathMock.mockClear().mockReturnValue("/tmp/traversalai-sessions-test.json");
  });
...
describe("resolveDiscordPresenceUpdate", () => {
  ...
  it("includes streaming url when activityType is streaming", () => {
    ...
    expect(presence?.activities[0]).toMatchObject({
      type: 1,
      name: "Live",
      url: "https://twitch.tv/traversalai",
    });
  });
});
