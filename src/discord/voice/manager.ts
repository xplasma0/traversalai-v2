...
function resolveVoiceTtsConfig(params: { cfg: TraversalAIConfig; override?: TtsConfig }): {
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredTraversalAITmpDir(), "discord-voice-"));
