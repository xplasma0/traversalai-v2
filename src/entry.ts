...
const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "traversalai.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "traversalai.js", entryBasename: "entry.js" },
  { wrapperBasename: "traversalai.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "traversalai.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  process.title = process.env.TRAVERSALAI_APP === "1" ? "traversalai" : "traversalai";
  const cliLabel = process.env.TRAVERSALAI_APP === "1" ? "traversalai" : "traversalai";
  installProcessWarningFilter();
  normalizeEnv();

  if (shouldForceReadOnlyAuthStore(process.argv)) {
    process.env.TRAVERSALAI_AUTH_STORE_READONLY = "1";
  }
  ...
  function ensureExperimentalWarningSuppressed(): boolean {
    ...
    if (isTruthyEnvValue(process.env.TRAVERSALAI_NO_RESPAWN)) {
      return false;
    }
    if (isTruthyEnvValue(process.env.TRAVERSALAI_NODE_OPTIONS_READY)) {
      return false;
    }
    ...
    process.env.TRAVERSALAI_NODE_OPTIONS_READY = "1";
    ...
    child.once("error", (error) => {
      console.error(
        `[${cliLabel}] Failed to respawn CLI:`,
  ...
  if (!ensureExperimentalWarningSuppressed()) {
    ...
    if (!parsed.ok) {
      ...
      console.error(`[${cliLabel}] ${parsed.error}`);
      process.exit(2);
    }
