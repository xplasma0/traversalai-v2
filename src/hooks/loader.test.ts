...
describe("loader", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "traversalai-hooks-loader-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["TRAVERSALAI_BUNDLED_HOOKS_DIR"]);
    process.env.TRAVERSALAI_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
