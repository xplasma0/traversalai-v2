...
async function ensureTraversalAIHooks(manifest: HookPackageManifest) {
  ...
  if (list.length === 0) {
    throw new Error("package.json traversalai.hooks is empty");
  }
  return list;
}
...
async function installHookPackageFromDir(params: {
  ...
  try {
    hookEntries = await ensureTraversalAIHooks(manifest);
  } catch (err) {
  ...
  for (const entry of hookEntries) {
    ...
    if (!isPathInside(params.packageDir, hookDir)) {
      return {
        ok: false,
        error: `traversalai.hooks entry escapes package directory: ${entry}`,
      };
    }
    ...
    if (
      ...
      return {
        ok: false,
        error: `traversalai.hooks entry resolves outside package directory: ${entry}`,
      };
    }
...
export async function installHooksFromArchive(params: {
  ...
  return await withExtractedArchiveRoot({
    archivePath,
    tempDirPrefix: "traversalai-hook-",
    timeoutMs,
    logger,
...
export async function installHooksFromNpmSpec(params: {
  ...
  const flowResult = await installFromNpmSpecArchiveWithInstaller({
    tempDirPrefix: "traversalai-hook-pack-",
    spec,
    timeoutMs,
