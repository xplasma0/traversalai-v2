...
homepage: https://xplasma0.github.io/traversalai-docs/automation/hooks#command-logger
metadata:
  {
    "traversalai":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.traversalai/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.traversalai/logs/commands.log ~/.traversalai/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/traversalai`:
   ```
   /home/username/.traversalai/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
