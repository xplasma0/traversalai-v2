...
This directory contains hooks that ship with TraversalAI. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
homepage: https://xplasma0.github.io/traversalai-docs/automation/hooks#my-hook
metadata:
...
Hooks can be configured in `~/.traversalai/traversalai.json`:
...
2. Restart gateway: `pkill -9 -f 'traversalai.*gateway' && pnpm traversalai gateway`
3. Enable the hook: `traversalai hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://xplasma0.github.io/traversalai-docs/automation/hooks
