...
function stubChannelPlugin(params: {
  ...
  label: string;
  resolveAccount: (cfg: TraversalAIConfig, accountId: string | null | undefined) => unknown;
...
describe("security audit", () => {
  ...
  const withChannelSecurityStateDir = async (fn: (tmp: string) => Promise<void>) => {
    ...
    await withEnvAsync(
      { TRAVERSALAI_STATE_DIR: channelSecurityStateDir },
      async () => await fn(channelSecurityStateDir),
    );
  };

  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "traversalai-security-audit-"));
  ...
  it("flags non-loopback bind without auth as critical", async () => {
    ...
    const prevToken = process.env.TRAVERSALAI_GATEWAY_TOKEN;
    const prevPassword = process.env.TRAVERSALAI_GATEWAY_PASSWORD;
    delete process.env.TRAVERSALAI_GATEWAY_TOKEN;
    delete process.env.TRAVERSALAI_GATEWAY_PASSWORD;
    ...
    try {
      ...
      if (prevToken === undefined) {
        ...
        process.env.TRAVERSALAI_GATEWAY_TOKEN = prevToken;
      }
      if (prevPassword === undefined) {
        delete process.env.TRAVERSALAI_GATEWAY_PASSWORD;
      } else {
        process.env.TRAVERSALAI_GATEWAY_PASSWORD = prevPassword;
      }
    }
  });
  ...
  it("warns for risky safeBinTrustedDirs entries", async () => {
    const riskyGlobalTrustedDirs =
      process.platform === "win32"
        ...
        : ["/usr/local/bin", "/tmp/traversalai-safe-bins"];
    const cfg: TraversalAIConfig = {
      tools: {
        exec: {
  ...
  it("treats Windows ACL-only perms as secure", async () => {
    ...
    const configPath = path.join(stateDir, "traversalai.json");
  ...
  it("warns when sandbox browser containers have missing or stale hash labels", async () => {
    ...
    const execDockerRawFn = (async (args: string[]) => {
      if (args[0] === "ps") {
        return {
          stdout: Buffer.from("traversalai-sbx-browser-old\ntraversalai-sbx-browser-missing-hash\n"),
          stderr: Buffer.alloc(0),
          code: 0,
        };
      }
      if (args[0] === "inspect" && args.at(-1) === "traversalai-sbx-browser-old") {
        return {
      ...
      if (args[0] === "inspect" && args.at(-1) === "traversalai-sbx-browser-missing-hash") {
        return {
    ...
    expect(staleEpoch?.detail).toContain("traversalai-sbx-browser-old");
  });
  ...
  it("flags sandbox browser containers with non-loopback published ports", async () => {
    ...
    const execDockerRawFn = (async (args: string[]) => {
      if (args[0] === "ps") {
        return {
          stdout: Buffer.from("traversalai-sbx-browser-exposed\n"),
          stderr: Buffer.alloc(0),
          code: 0,
        };
      }
      if (args[0] === "inspect" && args.at(-1) === "traversalai-sbx-browser-exposed") {
        return {
      ...
      if (args[0] === "port" && args.at(-1) === "traversalai-sbx-browser-exposed") {
        return {
  ...
  it("uses symlink target permissions for config checks", async () => {
    ...
    const targetConfigPath = path.join(tmp, "managed-traversalai.json");
  ...
  it("scores X-Real-IP fallback risk by gateway exposure", async () => {
    const trustedProxyCfg = (trustedProxies: string[]): TraversalAIConfig => ({
      gateway: {
        bind: "loopback",
  ...
  it("warns when multiple DM senders share the main session", async () => {
    const cfg: TraversalAIConfig = { session: { dmScope: "main" } };
  ...
  it("warns when Discord allowlists contain name-based entries", async () => {
    await withChannelSecurityStateDir(async (tmp) => {
      ...
      expect(finding?.detail).toContain(
        "~/.traversalai/credentials/discord-allowFrom.json:team.owner",
      );
  ...
  it("adds probe_failed warnings for deep probe failure modes", async () => {
    const cfg: TraversalAIConfig = { gateway: { mode: "local" } };
    const cases: Array<{
      name: string;
  ...
  it("flags hooks token reuse of the gateway env token as critical", async () => {
    const prevToken = process.env.TRAVERSALAI_GATEWAY_TOKEN;
    process.env.TRAVERSALAI_GATEWAY_TOKEN = "shared-gateway-token-1234567890";
    const cfg: TraversalAIConfig = {
  ...
  it("scores hooks request sessionKey override by gateway exposure", async () => {
    ...
    } satisfies NonNullable<TraversalAIConfig["hooks"]>;
    const cases: Array<{
      name: string;
      cfg: TraversalAIConfig;
      expectedSeverity: "warn" | "critical";
  ...
  it("warns when state/config look like a synced folder", async () => {
    ...
    const res = await audit(cfg, {
      stateDir: "/Users/test/Dropbox/.traversalai",
      configPath: "/Users/test/Dropbox/.traversalai/traversalai.json",
    });
  ...
  it("flags group/world-readable config include files", async () => {
    ...
    try {
      const cfg: TraversalAIConfig = { logging: { redactSensitive: "off" } };
      const user = "DESKTOP-TEST\\Tester";
  ...
  it("flags extensions without plugins.allow", async () => {
    ...
    try {
      ...
      const res = await runSecurityAudit({
        ...
        includeChannelSecurity: false,
        stateDir,
        configPath: path.join(stateDir, "traversalai.json"),
      });
    ...
    const cfg: TraversalAIConfig = {
      plugins: {
        installs: {
          "voice-call": {
            source: "npm",
            spec: "@traversalai/voice-call@1.2.3",
      ...
      hooks: {
        internal: {
          installs: {
            "test-hooks": {
              source: "npm",
              spec: "@traversalai/test-hooks@1.2.3",
  ...
  it("warns when install records drift from installed package versions", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({ name: "@traversalai/voice-call", version: "9.9.9" }),
      "utf-8",
    );
    await fs.writeFile(
      path.join(hookDir, "package.json"),
      JSON.stringify({ name: "@traversalai/test-hooks", version: "8.8.8" }),
      "utf-8",
    );
  ...
  it("does not scan plugin code safety findings when deep audit is disabled", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "evil-plugin",
        traversalai: { extensions: [".hidden/index.js"] },
      }),
    );
    await fs.writeFile(
...
`,
  ...
  it("flags plugin extension entry path traversal in deep audit", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "escape-plugin",
        traversalai: { extensions: ["../outside.js"] },
      }),
    );
    ...
    try {
      ...
      await fs.writeFile(
        ...
        JSON.stringify({
          ...
          traversalai: { extensions: ["index.js"] },
        }),
      );
      await fs.writeFile(path.join(pluginDir, "index.js"), "export {};");
  ...
  describe("maybeProbeGateway auth selection", () => {
    ...
    const makeProbeEnv = (env?: { token?: string; password?: string }) => {
      ...
      if (env?.token !== undefined) {
        probeEnv.TRAVERSALAI_GATEWAY_TOKEN = env.token;
      }
      if (env?.password !== undefined) {
        probeEnv.TRAVERSALAI_GATEWAY_PASSWORD = env.password;
      }
      return probeEnv;
    };
