...
function summarizeGroupPolicy(cfg: TraversalAIConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function isGatewayRemotelyExposed(cfg: TraversalAIConfig): boolean {
...
function collectModels(cfg: TraversalAIConfig): ModelRef[] {
  const out: ModelRef[] = [];
  addModel(
    out,
...
function listKnownNodeCommands(cfg: TraversalAIConfig): Set<string> {
  const baseCfg: TraversalAIConfig = {
    ...cfg,
    gateway: {
      ...cfg.gateway,
      nodes: {
...
function hasWebSearchKey(cfg: TraversalAIConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: TraversalAIConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: TraversalAIConfig): boolean {
...
function isBrowserEnabled(cfg: TraversalAIConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: TraversalAIConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
function listPotentialMultiUserSignals(cfg: TraversalAIConfig): string[] {
  const out = new Set<string>();
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return [];
...
function collectRiskyToolExposureContexts(cfg: TraversalAIConfig): {
...
export function collectAttackSurfaceSummaryFindings(cfg: TraversalAIConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep TRAVERSALAI_STATE_DIR on a local-only volume and re-run "${formatCliCommand("traversalai security audit --fix")}".`,
    });
  }
  return findings;
}

export function collectSecretsInConfigFindings(cfg: TraversalAIConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer TRAVERSALAI_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(
  ...
  const traversalaiGatewayToken =
    typeof env.TRAVERSALAI_GATEWAY_TOKEN === "string" && env.TRAVERSALAI_GATEWAY_TOKEN.trim()
      ? env.TRAVERSALAI_GATEWAY_TOKEN.trim()
      : null;
  const gatewayToken =
...
export function collectGatewayHttpSessionKeyOverrideFindings(
  ...
  findings.push({
    ...
    detail:
      `${enabledEndpoints.join(", ")} accept x-traversalai-session-key for per-request session routing. ` +
...
export function collectSandboxDockerNoopFindings(cfg: TraversalAIConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectSandboxDangerousConfigFindings(cfg: TraversalAIConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const agents = Array.isArray(cfg.agents?.list) ? cfg.agents.list : [];
  ...
  if (browserExposurePaths.length > 0) {
    findings.push({
      ...
      remediation:
        "Set sandbox.browser.network to a dedicated bridge network (recommended default: traversalai-sandbox-browser), " +
...
export function collectNodeDenyCommandPatternFindings(cfg: TraversalAIConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectMinimalProfileOverrideFindings(cfg: TraversalAIConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectModelHygieneFindings(cfg: TraversalAIConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExposureMatrixFindings(cfg: TraversalAIConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectLikelyMultiUserSetupFindings(cfg: TraversalAIConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  ...
  findings.push({
    ...
    detail:
      ...
      `\n${impactLine}\n${riskyContextsDetail}\n` +
      "TraversalAI's default security model is personal-assistant (one trusted operator boundary), not hostile multi-tenant isolation on one shared gateway.",
    remediation:
