...
declare const __TRAVERSALAI_VERSION__: string | undefined;
...
export function resolveRuntimeServiceVersion(
  ...
  return (
    firstNonEmpty(
      env["TRAVERSALAI_VERSION"],
      env["TRAVERSALAI_SERVICE_VERSION"],
      env["npm_package_version"],
    ) ?? fallback
  );
}

// Single source of truth for the current TraversalAI version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION =
  (typeof __TRAVERSALAI_VERSION__ === "string" && __TRAVERSALAI_VERSION__) ||
  process.env.TRAVERSALAI_BUNDLED_VERSION ||
