...
describe("resolveGatewayConnection", () => {
  ...
  it("uses TRAVERSALAI_GATEWAY_TOKEN for local mode", () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });

    withEnv({ TRAVERSALAI_GATEWAY_TOKEN: "env-token" }, () => {
      const result = resolveGatewayConnection({});
  ...
  it("prefers TRAVERSALAI_GATEWAY_PASSWORD over remote password fallback", () => {
    ...
    withEnv({ TRAVERSALAI_GATEWAY_PASSWORD: "env-pass" }, () => {
      const result = resolveGatewayConnection({});
