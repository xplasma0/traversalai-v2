...
export function resolveGatewayDisconnectState(reason?: string): {
  ...
  const devicesListCommand = `${cliName} devices list`;
  const reasonLabel = reason?.trim() ? reason.trim() : "closed";
  if (/pairing required/i.test(reasonLabel)) {
    return {
      connectionStatus: `gateway disconnected: ${reasonLabel}`,
      activityStatus: `pairing required: run ${devicesListCommand}`,
      pairingHint: `Pairing required. Run \`${devicesListCommand}\`, approve your request ID, then reconnect.`,
    };
  }
  return {
    connectionStatus: `gateway disconnected: ${reasonLabel}`,
...
export async function runTui(opts: TuiOptions) {
  ...
  const updateHeader = () => {
    ...
    header.setText(
      theme.header(
        `${cliName} tui - ${client.connection.url} - agent ${agentLabel} - session ${sessionLabel}`,
      ),
    );
  };
