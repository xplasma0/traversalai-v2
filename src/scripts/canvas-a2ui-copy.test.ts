...
describe("canvas a2ui copy", () => {
  async function withA2uiFixture(run: (dir: string) => Promise<void>) {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "traversalai-a2ui-"));
    try {
      await run(dir);
    } finally {
  ...
  it("skips missing assets when TRAVERSALAI_A2UI_SKIP_MISSING=1", async () => {
    await withA2uiFixture(async (dir) => {
      const previous = process.env.TRAVERSALAI_A2UI_SKIP_MISSING;
      process.env.TRAVERSALAI_A2UI_SKIP_MISSING = "1";
      try {
        await expect(
        ...
        if (previous === undefined) {
          delete process.env.TRAVERSALAI_A2UI_SKIP_MISSING;
        } else {
          process.env.TRAVERSALAI_A2UI_SKIP_MISSING = previous;
        }
      }
    });
  });
