...
describe("chrome extension relay server", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv([
      ...
      "TRAVERSALAI_EXTENSION_RELAY_RECONNECT_GRACE_MS",
      "TRAVERSALAI_EXTENSION_RELAY_COMMAND_RECONNECT_WAIT_MS",
    ]);
    process.env.TRAVERSALAI_GATEWAY_TOKEN = TEST_GATEWAY_TOKEN;
    delete process.env.TRAVERSALAI_EXTENSION_RELAY_RECONNECT_GRACE_MS;
    delete process.env.TRAVERSALAI_EXTENSION_RELAY_COMMAND_RECONNECT_WAIT_MS;
  });
  ...
  it("uses relay-scoped token only for known relay ports", async () => {
    ...
    expect(Object.keys(headers)).toContain("x-traversalai-relay-token");
    expect(headers["x-traversalai-relay-token"]).not.toBe(TEST_GATEWAY_TOKEN);
  });
  ...
  it("allows CORS preflight from chrome-extension origins", async () => {
    ...
    const res = await fetch(`${cdpUrl}/json/version`, {
      ...
      headers: {
        ...
        "Access-Control-Request-Headers": "x-traversalai-relay-token",
      },
    });
  ...
  it("closes CDP clients after reconnect grace when extension stays disconnected", async () => {
    process.env.TRAVERSALAI_EXTENSION_RELAY_RECONNECT_GRACE_MS = "150";
  ...
  it("accepts extension websocket access with relay token query param", async () => {
    ...
    const token = relayAuthHeaders(`ws://127.0.0.1:${port}/extension`)["x-traversalai-relay-token"];
  ...
  it("accepts /json endpoints with relay token query param", async () => {
    ...
    const token = relayAuthHeaders(cdpUrl)["x-traversalai-relay-token"];
    expect(token).toBeTruthy();
  ...
  it("accepts raw gateway token for relay auth compatibility", async () => {
    ...
    const versionRes = await fetch(`${cdpUrl}/json/version`, {
      headers: { "x-traversalai-relay-token": TEST_GATEWAY_TOKEN },
    });
    expect(versionRes.status).toBe(200);
  ...
  it("reuses an already-bound relay port when another process owns it", async () => {
    ...
    const fakeRelay = createServer((req, res) => {
      if (req.url?.startsWith("/json/version")) {
        const header = req.headers["x-traversalai-relay-token"];
        ...
        res.end(JSON.stringify({ Browser: "TraversalAI/extension-relay" }));
        return;
      }
  ...
  it("does not swallow EADDRINUSE when occupied port is not an traversalai relay", async () => {
    const port = await getFreePort();
