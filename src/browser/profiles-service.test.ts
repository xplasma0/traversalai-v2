...
vi.mock("./chrome.js", () => ({
  resolveTraversalAIUserDataDir: vi.fn(() => "/tmp/traversalai-test/traversalai/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveTraversalAIUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "traversalai-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveTraversalAIUserDataDir).mockReturnValue(userDataDir);
