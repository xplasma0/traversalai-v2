...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredTraversalAITmpDir: vi.fn(() => "/tmp/traversalai"),
}));
vi.mock("../infra/tmp-traversalai-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredTraversalAITmpDir.mockReturnValue("/tmp/traversalai");
  });
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredTraversalAITmpDir.mockReturnValue("/tmp/traversalai-preferred");
    ...
    const expectedRootedDownloadsDir = path.join(
      path.sep,
      "tmp",
      "traversalai-preferred",
      "downloads",
    );
    const expectedDownloadsTail = `${path.join("tmp", "traversalai-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredTraversalAITmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.join(path.sep, "tmp", "traversalai-preferred", "downloads"),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "traversalai-preferred", "downloads")}${path.sep}`),
    );
  });
