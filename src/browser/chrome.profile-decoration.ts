...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".traversalai-profile-decorated");
}
 ...
 */
export function decorateTraversalAIProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_TRAVERSALAI_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_TRAVERSALAI_BROWSER_COLOR).toUpperCase();
