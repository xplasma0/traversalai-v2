...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    const traversalai = resolveProfile(resolved, "traversalai");
    expect(traversalai?.driver).toBe("traversalai");
    expect(traversalai?.cdpPort).toBe(18800);
    expect(traversalai?.cdpUrl).toBe("http://127.0.0.1:18800");
    expect(resolved.remoteCdpTimeoutMs).toBe(1500);
    expect(resolved.remoteCdpHandshakeTimeoutMs).toBe(3000);
  });

  it("derives default ports from TRAVERSALAI_GATEWAY_PORT when unset", () => {
    withEnv({ TRAVERSALAI_GATEWAY_PORT: "19001" }, () => {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const traversalai = resolveProfile(resolved, "traversalai");
      expect(traversalai?.cdpPort).toBe(19012);
      expect(traversalai?.cdpUrl).toBe("http://127.0.0.1:19012");
    });
  });

  it("derives default ports from gateway.port when env is unset", () => {
    withEnv({ TRAVERSALAI_GATEWAY_PORT: undefined }, () => {
      ...
      const traversalai = resolveProfile(resolved, "traversalai");
      expect(traversalai?.cdpPort).toBe(19022);
      expect(traversalai?.cdpUrl).toBe("http://127.0.0.1:19022");
    });
  });
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "traversalai");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        traversalai: { cdpPort: 18792, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("traversalai");
  });
