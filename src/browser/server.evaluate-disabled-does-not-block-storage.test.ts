  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          traversalai: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.TRAVERSALAI_GATEWAY_PORT;
    process.env.TRAVERSALAI_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.TRAVERSALAI_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.TRAVERSALAI_GATEWAY_PASSWORD;
