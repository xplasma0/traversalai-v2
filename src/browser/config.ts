import type { BrowserConfig, BrowserProfileConfig, TraversalAIConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_TRAVERSALAI_BROWSER_ENABLED,
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "traversalai" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_TRAVERSALAI_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "traversalai" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_TRAVERSALAI_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_TRAVERSALAI_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an TraversalAI browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for traversalai/traversalai CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: TraversalAIConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_TRAVERSALAI_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "traversalai";
