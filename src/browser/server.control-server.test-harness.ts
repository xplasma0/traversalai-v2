...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.TRAVERSALAI_GATEWAY_PORT = prevGatewayPort;
}
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/traversalai" }));
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        headless: true,
        defaultProfile: "traversalai",
        profiles: {
          traversalai: { cdpPort: state.testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
vi.mock("./chrome.js", () => ({
  ...
  launchTraversalAIChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.prevGatewayPort = process.env.TRAVERSALAI_GATEWAY_PORT;
  process.env.TRAVERSALAI_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.TRAVERSALAI_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.TRAVERSALAI_GATEWAY_PASSWORD;
...
export function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.TRAVERSALAI_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.TRAVERSALAI_GATEWAY_PASSWORD;
  } else {
    process.env.TRAVERSALAI_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
