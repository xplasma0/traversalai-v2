...
function withLoopbackBrowserAuthImpl(
  ...
  if (headers.has("authorization") || headers.has("x-traversalai-password")) {
    return { ...init, headers };
  }
  ...
  try {
    ...
    if (auth.password) {
      headers.set("x-traversalai-password", auth.password);
      return { ...init, headers };
    }
  } catch {
  ...
  try {
    ...
    if (bridgeAuth?.token) {
      ...
      headers.set("x-traversalai-password", bridgeAuth.password);
    }
  } catch {
    // ignore
  }
...
function enhanceBrowserFetchError(url: string, err: unknown, timeoutMs: number): Error {
  ...
  const operatorHint = isLocal
    ? `Restart the TraversalAI gateway (TraversalAI.app menubar, or \`${formatCliCommand("traversalai gateway")}\`).`
  ...
  if (looksLikeTimeout) {
    return new Error(
      `Can't reach the TraversalAI browser control service (timed out after ${timeoutMs}ms). ${operatorHint} ${modelHint}`,
    );
  }
  return new Error(
    `Can't reach the TraversalAI browser control service. ${operatorHint} ${modelHint} (${msg})`,
  );
}
