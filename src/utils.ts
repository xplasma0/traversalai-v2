...
export function resolveConfigDir(
  ...
  const override = env.TRAVERSALAI_STATE_DIR?.trim() || env.CLAWDBOT_STATE_DIR?.trim();
  if (override) {
    return resolveUserPath(override);
  }
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".traversalai");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.TRAVERSALAI_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$TRAVERSALAI_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via TRAVERSALAI_STATE_DIR.
