...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("traversalai-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("traversalai-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("traversalai-sbx-");
