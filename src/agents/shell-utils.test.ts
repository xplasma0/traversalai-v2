...
function createTempCommandDir(
  ...
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "traversalai-shell-"));
  tempDirs.push(dir);
...
describe("resolveShellFromPath", () => {
  ...
  it("returns undefined when command does not exist", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "traversalai-shell-empty-"));
    tempDirs.push(dir);
...
describe("resolvePowerShellPath", () => {
  ...
  it("prefers PowerShell 7 in ProgramFiles", () => {
    const base = fs.mkdtempSync(path.join(os.tmpdir(), "traversalai-pfiles-"));
    tempDirs.push(base);
  ...
  it("prefers ProgramW6432 PowerShell 7 when ProgramFiles lacks pwsh", () => {
    const programFiles = fs.mkdtempSync(path.join(os.tmpdir(), "traversalai-pfiles-"));
    const programW6432 = fs.mkdtempSync(path.join(os.tmpdir(), "traversalai-pw6432-"));
  ...
  it("finds pwsh on PATH when not in standard install locations", () => {
    const programFiles = fs.mkdtempSync(path.join(os.tmpdir(), "traversalai-pfiles-"));
    const binDir = fs.mkdtempSync(path.join(os.tmpdir(), "traversalai-bin-"));
  ...
  it("falls back to Windows PowerShell 5.1 path when pwsh is unavailable", () => {
    const programFiles = fs.mkdtempSync(path.join(os.tmpdir(), "traversalai-pfiles-"));
    const sysRoot = fs.mkdtempSync(path.join(os.tmpdir(), "traversalai-sysroot-"));
