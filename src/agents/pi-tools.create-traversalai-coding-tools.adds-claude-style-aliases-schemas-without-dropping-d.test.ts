...
describe("createTraversalAICodingTools", () => {
  it("returns image metadata for images and text-only blocks for text files", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "traversalai-read-"));
    try {
      ...
      const contents = "Hello from traversalai read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    ...
    const tools = createTraversalAICodingTools({ sandbox });
