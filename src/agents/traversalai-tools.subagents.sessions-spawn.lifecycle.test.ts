...
} from "./traversalai-tools.subagents.sessions-spawn.test-harness.js";
import { resetSubagentRegistryForTests } from "./subagent-registry.js";

const fastModeEnv = vi.hoisted(() => {
  const previous = process.env.TRAVERSALAI_TEST_FAST;
...
describe("traversalai-tools: subagents (sessions_spawn lifecycle)", () => {
  beforeEach(() => {
    resetSessionsSpawnConfigOverride();
    setSessionsSpawnConfigOverride({
      session: {
  ...
  afterAll(() => {
    if (fastModeEnv.previous === undefined) {
      delete process.env.TRAVERSALAI_TEST_FAST;
      return;
    }
    process.env.TRAVERSALAI_TEST_FAST = fastModeEnv.previous;
  });
