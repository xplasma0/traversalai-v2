...
describe("buildAgentSystemPrompt", () => {
  ...
  it("includes a CLI quick reference section", () => {
    ...
    expect(prompt).toContain("## TraversalAI CLI Quick Reference");
    expect(prompt).toContain("traversalai gateway restart");
    expect(prompt).toContain("Do not invent commands");
  });

  it("uses TraversalAI branding when TRAVERSALAI_APP is enabled", () => {
    vi.stubEnv("TRAVERSALAI_APP", "1");
    vi.stubEnv("TRAVERSALAI_CLI_NAME", "traversalai");
    ...
    expect(prompt).toContain("You are TraversalAI");
    expect(prompt).toContain("## TraversalAI CLI Quick Reference");
    expect(prompt).toContain("- traversalai gateway restart");
    expect(prompt).toContain("TraversalAI docs: /tmp/traversalai/docs");
    expect(prompt).toContain("For TraversalAI behavior, commands, config, or architecture");
  });
  ...
  it("omits ACP harness guidance when ACP is disabled", () => {
    ...
    expect(prompt).toContain("- agents_list: List TraversalAI agent ids allowed for sessions_spawn");
  });
  ...
  it("preserves tool casing in the prompt", () => {
    ...
    expect(prompt).toContain("TraversalAI docs: /tmp/traversalai/docs");
    expect(prompt).toContain(
      "For TraversalAI behavior, commands, config, or architecture: consult local docs first.",
    );
  });
  ...
  it("adds ClaudeBot self-update guidance when gateway tool is available", () => {
    ...
    expect(prompt).toContain("## TraversalAI Self-Update");
  ...
  it("describes sandboxed runtime and elevated when allowed", () => {
    ...
    expect(prompt).toContain(
      "For read/write/edit/apply_patch, file paths resolve against host workspace: /tmp/traversalai. For bash/exec commands, use sandbox container paths under /workspace (or relative paths from that workdir), not host paths.",
    );
...
describe("buildSubagentSystemPrompt", () => {
  it("renders depth-1 orchestrator guidance, labels, and recovery notes", () => {
    ...
    expect(prompt).toContain("Do not use `exec` (`traversalai ...`, `acpx ...`)");
    expect(prompt).toContain("Use `subagents` only for TraversalAI subagents");
