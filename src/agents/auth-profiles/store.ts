 ...
 * A common mistake — caused by the similarity with the `traversalai.json`
...
function normalizeRawCredentialEntry(raw: Record<string, unknown>): Partial<AuthProfileCredential> {
  ...
  // mode → type alias (traversalai.json uses "mode"; auth-profiles.json uses "type")
...
function loadAuthProfileStoreForAgent(
  ...
  const forceReadOnly = process.env.TRAVERSALAI_AUTH_STORE_READONLY === "1";
