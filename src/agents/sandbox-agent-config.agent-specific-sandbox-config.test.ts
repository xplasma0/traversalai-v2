...
async function resolveContext(config: TraversalAIConfig, sessionKey: string, workspaceDir: string) {
...
function createWorkSetupCommandConfig(scope: "agent" | "shared"): TraversalAIConfig {
  return {
    agents: {
      defaults: {
        sandbox: {
    ...
    const cfg: TraversalAIConfig = {
      agents: {
        ...
        list: [
          {
            id: "isolated",
            workspace: "~/traversalai-isolated",
            sandbox: {
              mode: "all",
              scope: "agent",
