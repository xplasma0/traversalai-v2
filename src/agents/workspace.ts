...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = (env.TRAVERSALAI_PROFILE ?? env.TRAVERSALAI_PROFILE)?.trim();
  const appDir = env.TRAVERSALAI_APP === "1" ? ".traversalai" : ".traversalai";
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, appDir, `workspace-${profile}`);
  }
  return path.join(home, appDir, "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = process.env.TRAVERSALAI_APP === "1" ? ".traversalai" : ".traversalai";
