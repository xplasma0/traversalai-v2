...
import { resolvePreferredTraversalAITmpDir } from "../infra/tmp-traversalai-dir.js";
...
async function withOutsideHardlinkInTraversalAITmp<T>(
  params: {
...
describe("resolveSandboxedMediaSource", () => {
  const openClawTmpDir = resolvePreferredTraversalAITmpDir();

  // Group 1: /tmp paths (the bug fix)
  it.each([
    {
      name: "absolute paths under preferred TraversalAI tmp root",
      media: path.join(openClawTmpDir, "image.png"),
      expected: path.join(openClawTmpDir, "image.png"),
    },
    {
      name: "file:// URLs pointing to preferred TraversalAI tmp root",
      media: pathToFileURL(path.join(openClawTmpDir, "photo.png")).href,
      expected: path.join(openClawTmpDir, "photo.png"),
    },
    {
      name: "nested paths under preferred TraversalAI tmp root",
  ...
  it.each([
    ...
    {
      name: "absolute paths under host tmp outside traversalai tmp root",
      media: path.join(os.tmpdir(), "outside-traversalai", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked TraversalAI tmp paths escaping tmp root", async () => {
  ...
  it("rejects hardlinked TraversalAI tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInTraversalAITmp(
      {
        openClawTmpDir,
  ...
  it("rejects symlinked TraversalAI tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInTraversalAITmp(
