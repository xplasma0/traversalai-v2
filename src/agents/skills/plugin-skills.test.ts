    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "traversalai.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "traversalai.plugin.json"),
      },
    ],
  };
}
...
describe("resolvePluginSkillDirs", () => {
  it("keeps acpx plugin skills when ACP is enabled", async () => {
    const workspaceDir = await tempDirs.make("traversalai-");
    const acpxRoot = await tempDirs.make("traversalai-acpx-plugin-");
    const helperRoot = await tempDirs.make("traversalai-helper-plugin-");
