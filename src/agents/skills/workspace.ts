...
function resolveSkillsLimits(config?: TraversalAIConfig): ResolvedSkillsLimits {
...
function loadSkillEntries(
  ...
  const extraSkills = mergedExtraDirs.flatMap((dir) => {
    ...
    return loadSkills({
      dir: resolved,
      source: "traversalai-extra",
    });
  });
  const managedSkills = loadSkills({
    dir: managedSkillsDir,
    source: "traversalai-managed",
  });
  ...
  const skillEntries: SkillEntry[] = Array.from(merged.values()).map((skill) => {
    ...
    return {
      skill,
      frontmatter,
      metadata: resolveTraversalAIMetadata(frontmatter),
...
function applySkillsPromptLimits(params: { skills: Skill[]; config?: TraversalAIConfig }): {
...
function resolveWorkspaceSkillPromptState(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}. Run \`traversalai skills check\` to audit.`
    : "";
  const prompt = [
    remoteNote,
    truncationNote,
