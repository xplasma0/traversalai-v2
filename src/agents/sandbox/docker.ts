...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "traversalai.sandbox=1");
  args.push("--label", `traversalai.sessionKey=${params.scopeKey}`);
  args.push("--label", `traversalai.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `traversalai.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "traversalai.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`traversalai sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`traversalai sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("traversalai sandbox recreate --all");
}
