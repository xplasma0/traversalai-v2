  ...
  return {
    ...
    spawn: (command: string, args: string[]) => {
      ...
      if (command !== "docker") {
        ...
        args[0] === "inspect" &&
        args[1] === "-f" &&
        args[2]?.includes('index .Config.Labels "traversalai.configHash"')
      ) {
...
function createSandboxConfig(dns: string[], binds?: string[]): SandboxConfig {
  return {
    ...
    docker: {
      image: "traversalai-sandbox:test",
    ...
    browser: {
      enabled: false,
      image: "traversalai-browser:test",
...
describe("ensureSandboxContainer config-hash recreation", () => {
  ...
  it("recreates shared container when array-order change alters hash", async () => {
    ...
    expect(createCall?.args).toContain(`traversalai.configHash=${newHash}`);
  ...
  it("applies custom binds after workspace mounts so overlapping binds can override", async () => {
    ...
    expect(createCall).toBeDefined();
    expect(createCall?.args).toContain(`traversalai.configHash=${expectedHash}`);
