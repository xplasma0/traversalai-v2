...
function requireGatewayTool(agentSessionKey?: string) {
  const tool = createTraversalAITools({
...
describe("gateway tool", () => {
  ...
  it("schedules SIGUSR1 restart", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "traversalai-test-"));

    try {
      await withEnvAsync(
        { TRAVERSALAI_STATE_DIR: stateDir, TRAVERSALAI_PROFILE: "isolated" },
        async () => {
          const tool = requireGatewayTool();
          ...
          expect(parsed.payload?.doctorHint).toBe(
            "Run: traversalai --profile isolated doctor --non-interactive",
          );
  ...
  it("passes config.apply through gateway call", async () => {
    ...
    const raw = '{\n  agents: { defaults: { workspace: "~/traversalai" } }\n}\n';
