...
type CoreToolDefinition = {
  ...
  includeInTraversalAIGroup?: boolean;
};
...
const CORE_TOOL_DEFINITIONS: CoreToolDefinition[] = [
  ...
  },
  {
    ...
    sectionId: "web",
    profiles: [],
    includeInTraversalAIGroup: true,
  },
  {
    id: "web_fetch",
    label: "web_fetch",
...
function buildCoreToolGroupMap() {
  ...
  const traversalaiTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInTraversalAIGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:traversalai": traversalaiTools,
