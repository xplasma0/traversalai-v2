...
const GOOGLE_LIVE_SINGLE_KEY = "TRAVERSALAI_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "TRAVERSALAI_LIVE_ANTHROPIC_KEY",
    listVar: "TRAVERSALAI_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "TRAVERSALAI_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `TRAVERSALAI_LIVE_${base}_KEY`;
