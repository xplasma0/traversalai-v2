...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    traversalai: process.env.TRAVERSALAI_GATEWAY_TOKEN,
  ...
  afterAll(() => {
    if (envSnapshot.traversalai === undefined) {
      delete process.env.TRAVERSALAI_GATEWAY_TOKEN;
    } else {
      process.env.TRAVERSALAI_GATEWAY_TOKEN = envSnapshot.traversalai;
    }
  ...
  it("uses TRAVERSALAI_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.TRAVERSALAI_GATEWAY_TOKEN = "local-env-token";
