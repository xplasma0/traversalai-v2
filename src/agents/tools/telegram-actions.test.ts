...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): TraversalAIConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as TraversalAIConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): TraversalAIConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as TraversalAIConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  async function expectAccountStickerSend(cfg: TraversalAIConfig, accountId = "media") {
    await handleTelegramAction(
