...
import { createTraversalAICodingTools } from "./pi-tools.js";
...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "traversalai-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: TraversalAIConfig) {
    return createTraversalAICodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<TraversalAIConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<TraversalAIConfig["agents"]>["list"]>[number]["tools"];
  }): TraversalAIConfig {
    return {
      tools: params.tools,
      agents: {
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createTraversalAICodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createTraversalAICodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createTraversalAICodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createTraversalAICodingTools({
      config: cfg,
    ...
    const bobTools = createTraversalAICodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createTraversalAICodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createTraversalAICodingTools({
      config: cfg,
