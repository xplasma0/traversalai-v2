...
type ModelsConfig = NonNullable<TraversalAIConfig["models"]>;
...
export async function ensureTraversalAIModelsJson(
  config?: TraversalAIConfig,
  agentDirOverride?: string,
): Promise<{ agentDir: string; wrote: boolean }> {
  const cfg = config ?? loadConfig();
  const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : resolveTraversalAIAgentDir();
