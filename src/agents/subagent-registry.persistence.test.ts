...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["TRAVERSALAI_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "traversalai-subagent-"));
  ...
  it("uses isolated temp state when TRAVERSALAI_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "traversalai-test-state"));
  });
});
