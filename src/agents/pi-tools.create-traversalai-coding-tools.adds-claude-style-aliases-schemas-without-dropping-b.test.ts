...
const defaultTools = createTraversalAICodingTools({ senderIsOwner: true });
...
describe("createTraversalAICodingTools", () => {
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const enabledConfig: TraversalAIConfig = {
      tools: {
        exec: {
    ...
    const openAiTools = createTraversalAICodingTools({
    ...
    const anthropicTools = createTraversalAICodingTools({
      config: enabledConfig,
    ...
    const allowModelsConfig: TraversalAIConfig = {
      tools: {
        exec: {
    ...
    const allowed = createTraversalAICodingTools({
    ...
    const denied = createTraversalAICodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
    ...
    const oauthTools = createTraversalAICodingTools({
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
