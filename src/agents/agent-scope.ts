...
type AgentEntry = NonNullable<NonNullable<TraversalAIConfig["agents"]>["list"]>[number];
...
export function listAgentEntries(cfg: TraversalAIConfig): AgentEntry[] {
...
export function listAgentIds(cfg: TraversalAIConfig): string[] {
...
export function resolveDefaultAgentId(cfg: TraversalAIConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
function resolveAgentEntry(cfg: TraversalAIConfig, agentId: string): AgentEntry | undefined {
...
export function resolveAgentModelPrimary(cfg: TraversalAIConfig, agentId: string): string | undefined {
...
export function resolveAgentWorkspaceDir(cfg: TraversalAIConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
...
export function resolveAgentDir(cfg: TraversalAIConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
