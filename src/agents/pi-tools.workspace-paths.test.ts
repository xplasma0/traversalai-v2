...
describe("workspace path resolution", () => {
  it("resolves relative read/write/edit paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("traversalai-ws-", async (workspaceDir) => {
      await withTempDir("traversalai-cwd-", async (otherDir) => {
        const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(otherDir);
        try {
          const tools = createTraversalAICodingTools({ workspaceDir });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("traversalai-ws-", async (workspaceDir) => {
      await withTempDir("traversalai-override-", async (overrideDir) => {
        const tools = createTraversalAICodingTools({
          workspaceDir,
          exec: { host: "gateway", ask: "off", security: "full" },
        });
  ...
  it("rejects @-prefixed absolute paths outside workspace when workspaceOnly is enabled", async () => {
    await withTempDir("traversalai-ws-", async (workspaceDir) => {
      const cfg: TraversalAIConfig = { tools: { fs: { workspaceOnly: true } } };
      const tools = createTraversalAICodingTools({ workspaceDir, config: cfg });
      const { readTool } = expectReadWriteEditTools(tools);

      const outsideAbsolute = path.resolve(path.parse(workspaceDir).root, "outside-traversalai.txt");
      await expect(
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("traversalai-sandbox-", async (sandboxDir) => {
      await withTempDir("traversalai-workspace-", async (workspaceDir) => {
        ...
        const tools = createTraversalAICodingTools({ workspaceDir, sandbox });
        const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
