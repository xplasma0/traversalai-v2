...
export async function withTempWorkspace(
  ...
  const tempHome = await createTempHomeEnv("traversalai-skills-install-home-");
  const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "traversalai-skills-install-"));
  try {
