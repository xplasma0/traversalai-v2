...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses TRAVERSALAI_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      TRAVERSALAI_HOME: "/srv/traversalai-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/traversalai-home"), ".traversalai", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".traversalai", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("traversalai-workspace-");
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "traversalai-workspace-hardlink-"));
    try {
