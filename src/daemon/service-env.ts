...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.TRAVERSALAI_PROFILE;
  const resolvedLaunchdLabel =
    launchdLabel ||
    (process.platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  const stateDir = env.TRAVERSALAI_STATE_DIR;
  const configPath = env.TRAVERSALAI_CONFIG_PATH;
  const traversalAiApp = env.TRAVERSALAI_APP;
  const cliName = env.TRAVERSALAI_CLI_NAME;
  ...
  return {
    ...
    ...proxyEnv,
    TRAVERSALAI_PROFILE: profile,
    TRAVERSALAI_APP: traversalAiApp,
    TRAVERSALAI_CLI_NAME: cliName,
    TRAVERSALAI_STATE_DIR: stateDir,
    TRAVERSALAI_CONFIG_PATH: configPath,
    TRAVERSALAI_GATEWAY_PORT: String(port),
    TRAVERSALAI_GATEWAY_TOKEN: token,
    TRAVERSALAI_LAUNCHD_LABEL: resolvedLaunchdLabel,
    TRAVERSALAI_SYSTEMD_UNIT: systemdUnit,
    TRAVERSALAI_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    TRAVERSALAI_SERVICE_KIND: GATEWAY_SERVICE_KIND,
...
export function buildNodeServiceEnvironment(params: {
  ...
  return {
    ...
    TRAVERSALAI_STATE_DIR: stateDir,
    TRAVERSALAI_CONFIG_PATH: configPath,
    TRAVERSALAI_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    TRAVERSALAI_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    TRAVERSALAI_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    TRAVERSALAI_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    TRAVERSALAI_LOG_PREFIX: "node",
    TRAVERSALAI_SERVICE_MARKER: NODE_SERVICE_MARKER,
    TRAVERSALAI_SERVICE_KIND: NODE_SERVICE_KIND,
    TRAVERSALAI_SERVICE_VERSION: VERSION,
  };
}
