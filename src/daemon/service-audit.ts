...
function auditGatewayToken(
  ...
  const serviceToken = command?.environment?.TRAVERSALAI_GATEWAY_TOKEN?.trim();
  ...
  issues.push({
    ...
    message:
      "Gateway service TRAVERSALAI_GATEWAY_TOKEN does not match gateway.auth.token in traversalai.json",
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `traversalai gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
