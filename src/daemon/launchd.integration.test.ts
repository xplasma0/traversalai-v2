...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `traversalai-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      TRAVERSALAI_LAUNCHD_LABEL: `ai.traversalai.launchd-int-${testId}`,
      TRAVERSALAI_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
