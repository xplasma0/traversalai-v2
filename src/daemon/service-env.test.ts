...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.TRAVERSALAI_GATEWAY_PORT).toBe("18789");
    expect(env.TRAVERSALAI_GATEWAY_TOKEN).toBe("secret");
    expect(env.TRAVERSALAI_SERVICE_MARKER).toBe("traversalai");
    expect(env.TRAVERSALAI_SERVICE_KIND).toBe("gateway");
    expect(typeof env.TRAVERSALAI_SERVICE_VERSION).toBe("string");
    expect(env.TRAVERSALAI_SYSTEMD_UNIT).toBe("traversalai-gateway.service");
    if (process.platform === "darwin") {
      expect(env.TRAVERSALAI_LAUNCHD_LABEL).toBe("ai.traversalai.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", TRAVERSALAI_PROFILE: "work" },
      port: 18789,
    });
    expect(env.TRAVERSALAI_SYSTEMD_UNIT).toBe("traversalai-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.TRAVERSALAI_LAUNCHD_LABEL).toBe("ai.traversalai.work");
    }
  });
  ...
  it("passes TraversalAI runtime markers to gateway services", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", TRAVERSALAI_APP: "1", TRAVERSALAI_CLI_NAME: "traversalai" },
      port: 28789,
      token: "secret",
    });

    expect(env.TRAVERSALAI_APP).toBe("1");
    expect(env.TRAVERSALAI_CLI_NAME).toBe("traversalai");
  });
});
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("passes TraversalAI runtime markers to node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", TRAVERSALAI_APP: "1", TRAVERSALAI_CLI_NAME: "traversalai" },
    });
    expect(env.TRAVERSALAI_APP).toBe("1");
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".traversalai"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", TRAVERSALAI_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".traversalai-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", TRAVERSALAI_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".traversalai"));
  });

  it("uses TRAVERSALAI_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", TRAVERSALAI_STATE_DIR: "/var/lib/traversalai" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/traversalai"));
  });

  it("expands ~ in TRAVERSALAI_STATE_DIR", () => {
    const env = { HOME: "/Users/test", TRAVERSALAI_STATE_DIR: "~/traversalai-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/traversalai-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { TRAVERSALAI_STATE_DIR: "C:\\State\\traversalai" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\traversalai");
  });
});
