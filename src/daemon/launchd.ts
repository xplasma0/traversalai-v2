...
function resolveLaunchAgentLabel(args?: { env?: Record<string, string | undefined> }): string {
  const envLabel = args?.env?.TRAVERSALAI_LAUNCHD_LABEL?.trim();
  if (envLabel) {
    return envLabel;
  }
  return resolveGatewayLaunchAgentLabel(args?.env?.TRAVERSALAI_PROFILE);
}
...
export function resolveGatewayLogPaths(env: GatewayServiceEnv): {
  ...
  const prefix = env.TRAVERSALAI_LOG_PREFIX?.trim() || "gateway";
  return {
    logDir,
...
export async function findLegacyLaunchAgents(env: GatewayServiceEnv): Promise<LegacyLaunchAgent[]> {
  ...
  for (const label of resolveLegacyGatewayLaunchAgentLabels(env.TRAVERSALAI_PROFILE)) {
...
export async function installLaunchAgent({
  ...
  const domain = resolveGuiDomain();
  const label = resolveLaunchAgentLabel({ env });
  for (const legacyLabel of resolveLegacyGatewayLaunchAgentLabels(env.TRAVERSALAI_PROFILE)) {
    ...
    if (isUnsupportedGuiDomain(detail)) {
      throw new Error(
        [
          ...
          "Fix: sign in to the macOS desktop as the target user and rerun `traversalai gateway install --force`.",
          "Headless deployments should use a dedicated logged-in user session or a custom LaunchDaemon (not shipped): https://xplasma0.github.io/traversalai-docs/gateway",
        ].join("\n"),
      );
    }
    ...
    if (isUnsupportedGuiDomain(detail)) {
      throw new Error(
        [
          ...
          "This usually means you are running from SSH/headless context or as the wrong user (including sudo).",
          "Fix: sign in to the macOS desktop as the target user and rerun `traversalai gateway restart`.",
          "Headless deployments should use a dedicated logged-in user session or a custom LaunchDaemon (not shipped): https://xplasma0.github.io/traversalai-docs/gateway",
        ].join("\n"),
      );
    }
