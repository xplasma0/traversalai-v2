...
export function resolveGatewayStateDir(env: Record<string, string | undefined>): string {
  ...
  const suffix = resolveGatewayProfileSuffix(env.TRAVERSALAI_PROFILE ?? env.TRAVERSALAI_PROFILE);
  const dirname = env.TRAVERSALAI_APP === "1" ? ".traversalai" : ".traversalai";
  return path.join(home, `${dirname}${suffix}`);
}
