...
async function makeStorePath() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "traversalai-cron-store-"));
  return {
    dir,
...
describe("resolveCronStorePath", () => {
  ...
  it("uses TRAVERSALAI_HOME for tilde expansion", () => {
    vi.stubEnv("TRAVERSALAI_HOME", "/srv/traversalai-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/traversalai-home", "cron", "jobs.json"));
  });
});
