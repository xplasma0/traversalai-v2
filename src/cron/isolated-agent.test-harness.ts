...
export async function withTempCronHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "traversalai-cron-" });
}
...
export async function writeSessionStoreEntries(
  ...
  const dir = path.join(home, ".traversalai", "sessions");
...
export function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<TraversalAIConfig> = {},
): TraversalAIConfig {
  const base: TraversalAIConfig = {
    agents: {
      defaults: {
