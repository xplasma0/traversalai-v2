...
export const registerTelegramNativeCommands = ({
  ...
  const hasExplicitPairCommand = allCommandsFull.some((entry) => entry.command === "pair");
  ...
  if (commandsToRegister.length > 0 || pluginCatalog.commands.length > 0) {
    if (typeof (bot as unknown as { command?: unknown }).command !== "function") {
      ...
      if (!hasExplicitPairCommand) {
        bot.command("pair", async (ctx: TelegramNativeCommandContext) => {
          ...
          if (isGroup) {
            await withTelegramApiErrorLogging({
              ...
              runtime,
              fn: () =>
                bot.api.sendMessage(chatId, "This command is only available in direct messages."),
            });
            return;
          }
          ...
          if (dmPolicy !== "pairing") {
            await withTelegramApiErrorLogging({
              ...
              runtime,
              fn: () =>
                bot.api.sendMessage(chatId, "Pairing is not enabled for Telegram direct messages."),
            });
            return;
          }
          const senderId = msg.from?.id != null ? String(msg.from.id) : String(chatId);
          ...
          try {
            ...
            runtime.error?.(danger(`telegram /pair failed: ${String(err)}`));
          }
        });
      }
