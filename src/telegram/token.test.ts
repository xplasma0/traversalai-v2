...
function withTempDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "traversalai-telegram-token-"));
}
...
describe("resolveTelegramToken", () => {
  ...
  it("uses tokenFile when configured", () => {
    ...
    const cfg = { channels: { telegram: { tokenFile } } } as TraversalAIConfig;
    const res = resolveTelegramToken(cfg);
...
describe("telegram update offset store", () => {
  it("persists and reloads the last update id", async () => {
    await withStateDirEnv("traversalai-telegram-", async () => {
