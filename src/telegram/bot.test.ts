...
describe("createTelegramBot", () => {
    ...
    await handler({
      message: {
        ...
        reply_to_message: {
          message_id: 42,
          text: "original reply",
          from: { id: 999, first_name: "TraversalAI" },
        },
      },
      me: { id: 999, username: "traversalai_bot" },
      getFile: async () => ({ download: async () => new Uint8Array() }),
    });
  ...
  it("allows /pair in DM and returns a pairing code for unpaired users", async () => {
    onSpy.mockClear();
    sendMessageSpy.mockClear();
    commandSpy.mockClear();
    replySpy.mockClear();
    ...
    createTelegramBot({ token: "tok" });
    const handler = commandSpy.mock.calls.find((call) => call[0] === "pair")?.[1] as
      | ((ctx: Record<string, unknown>) => Promise<void>)
      | undefined;
    if (!handler) {
      throw new Error("pair command handler missing");
    }
    ...
    expect(sendMessageSpy).toHaveBeenCalledWith(
      12345,
      expect.stringContaining("Pairing code: PAIRME12"),
    );
    expect(
      sendMessageSpy.mock.calls.some(
        (call) => call[1] === "You are not authorized to use this command.",
