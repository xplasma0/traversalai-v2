...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: TraversalAIConfig, policy: ChannelAccessPolicy) => TraversalAIConfig;
  resolveAllowlist: (params: { cfg: TraversalAIConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: TraversalAIConfig; resolved: TResolved }) => TraversalAIConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: TraversalAIConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: TraversalAIConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: TraversalAIConfig, policy: ChannelAccessPolicy): TraversalAIConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: TraversalAIConfig, policy: ChannelAccessPolicy): TraversalAIConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: TraversalAIConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: TraversalAIConfig; resolved: string[] }) => {
      calls.push("apply");
