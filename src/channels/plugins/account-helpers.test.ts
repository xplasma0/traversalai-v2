...
function cfg(accounts?: Record<string, unknown> | null): TraversalAIConfig {
  if (accounts === null) {
    return { channels: { testchannel: {} } } as unknown as TraversalAIConfig;
  }
  if (accounts === undefined) {
    return {} as unknown as TraversalAIConfig;
  }
  return { channels: { testchannel: { accounts } } } as unknown as TraversalAIConfig;
}

describe("createAccountListHelpers", () => {
  describe("listConfiguredAccountIds", () => {
    it("returns empty for missing config", () => {
      expect(listConfiguredAccountIds({} as TraversalAIConfig)).toEqual([]);
    });
  ...
  describe("listAccountIds", () => {
    it('returns ["default"] for empty config', () => {
      expect(listAccountIds({} as TraversalAIConfig)).toEqual(["default"]);
    });
  ...
  describe("resolveDefaultAccountId", () => {
    ...
    it('returns "default" for empty config', () => {
      expect(resolveDefaultAccountId({} as TraversalAIConfig)).toBe("default");
    });
  });
});
