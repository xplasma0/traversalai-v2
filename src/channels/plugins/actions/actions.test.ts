...
function telegramCfg(): TraversalAIConfig {
  return { channels: { telegram: { botToken: "tok" } } } as TraversalAIConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: TraversalAIConfig; accountId?: string },
) {
...
async function runSignalAction(
  ...
  const cfg =
    options?.cfg ?? ({ channels: { signal: { account: "+15550001111" } } } as TraversalAIConfig);
...
function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as TraversalAIConfig;
...
function createSignalAccountOverrideCfg(): TraversalAIConfig {
  return {
    channels: {
      signal: {
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as TraversalAIConfig;
  ...
  it("lists moderation when at least one account enables it", () => {
    ...
    for (const channelConfig of cases) {
      const cfg = channelConfig as unknown as TraversalAIConfig;
      const actions = discordMessageActions.listActions?.({ cfg }) ?? [];
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as TraversalAIConfig,
        accountId: undefined,
        params: {
