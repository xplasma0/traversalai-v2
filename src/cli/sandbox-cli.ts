...
const SANDBOX_EXAMPLES = {
  main: [
    ["traversalai sandbox list", "List all sandbox containers."],
    ["traversalai sandbox list --browser", "List only browser containers."],
    ["traversalai sandbox recreate --all", "Recreate all containers."],
    ["traversalai sandbox recreate --session main", "Recreate a specific session."],
    ["traversalai sandbox recreate --agent mybot", "Recreate agent containers."],
    ["traversalai sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["traversalai sandbox list", "List all sandbox containers."],
    ["traversalai sandbox list --browser", "List only browser containers."],
  ...
  recreate: [
    ["traversalai sandbox recreate --all", "Recreate all containers."],
    ["traversalai sandbox recreate --session main", "Recreate a specific session."],
    ["traversalai sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["traversalai sandbox recreate --browser --all", "Recreate only browser containers."],
    ["traversalai sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["traversalai sandbox explain", "Show effective sandbox config."],
    ["traversalai sandbox explain --session agent:main:main", "Explain a specific session."],
    ["traversalai sandbox explain --agent work", "Explain an agent sandbox."],
    ["traversalai sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "xplasma0.github.io/traversalai-docs/cli/sandbox")}\n`,
    )
    .action(() => {
