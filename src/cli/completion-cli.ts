...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# TraversalAI Completion";
}
...
function isSlowDynamicCompletionLine(line: string, binName: string): boolean {
  // Matches patterns like: source <(traversalai completion --shell zsh)
  return (
...
function updateCompletionProfile(
  ...
  const block = `# TraversalAI Completion\n${sourceLine}`;
 ...
 * Returns true if profile has `source <(traversalai completion ...)` instead of cached file.
 */
...
export function registerCompletionCli(program: Command) {
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/completion", "xplasma0.github.io/traversalai-docs/cli/completion")}\n`,
    )
    .addOption(
    ...
    .option(
      "--write-state",
      "Write completion scripts to $TRAVERSALAI_STATE_DIR/completions (no stdout)",
    )
...
export async function installCompletion(shell: string, yes: boolean, binName = "traversalai") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
...
function generateFishCompletion(program: Command): string {
  ...
  const visit = (cmd: Command, parents: string[]) => {
    ...
    // For root: complete -c traversalai -n "__fish_use_subcommand" -a "subcmd" -d "desc"
    ...
    if (parents.length === 0) {
      ...
      // To properly scope to 'traversalai gateway' and not 'traversalai other gateway', we need to check the sequence.
