...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "traversalai-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "traversalai-bundled-skills-test-"));
    process.env.TRAVERSALAI_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
