...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["traversalai security audit", "Run a local security audit."],
          ["traversalai security audit --deep", "Include best-effort live Gateway probe checks."],
          ["traversalai security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["traversalai security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "xplasma0.github.io/traversalai-docs/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("TraversalAI security audit"));
      lines.push(muted(`Summary: ${formatSummary(report.summary)}`));
      lines.push(muted(`Run deeper: ${formatCliCommand("traversalai security audit --deep")}`));

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("traversalai security audit --fix")}`));
        if (!fixResult) {
