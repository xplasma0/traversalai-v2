...
import type { TraversalAIConfig, ConfigFileSnapshot } from "../config/types.traversalai.js";
...
vi.mock("../infra/traversalai-root.js", () => ({
  resolveTraversalAIPackageRoot: vi.fn(),
}));
...
const { resolveTraversalAIPackageRoot } = await import("../infra/traversalai-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/traversalai-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as TraversalAIConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/traversalai-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveTraversalAIPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("traversalai-update");
    setTty(false);
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveTraversalAIPackageRoot).mockClear();
    ...
    vi.mocked(resolveTraversalAIPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("traversalai");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/traversalai-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "traversalai-gateway" }],
      hints: [],
    });
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("TraversalAI update status");
  });
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as TraversalAIConfig,
        });
      },
  ...
  it("honors --tag override", async () => {
    ...
    vi.mocked(resolveTraversalAIPackageRoot).mockResolvedValue(tempDir);
    vi.mocked(runGatewayUpdate).mockResolvedValue(
      makeOkUpdateResult({
        mode: "npm",
      }),
  ...
  it("updateCommand refreshes service env from updated install root when available", async () => {
    const root = createCaseDir("traversalai-updated-root");
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ TRAVERSALAI_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.TRAVERSALAI_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("traversalai-update-wizard");
    await withEnvAsync({ TRAVERSALAI_GIT_DIR: tempDir }, async () => {
      setTty(true);
