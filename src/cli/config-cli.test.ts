...
import type { ConfigFileSnapshot, TraversalAIConfig } from "../config/types.js";

/**
 * Test for issue #6070:
 * `traversalai config set/unset` must update snapshot.resolved (user config after $include/${ENV},
 * but before runtime defaults), so runtime defaults don't leak into the written config.
 */

const mockReadConfigFileSnapshot = vi.fn<() => Promise<ConfigFileSnapshot>>();
const mockWriteConfigFile = vi.fn<
  (cfg: TraversalAIConfig, options?: { unsetPaths?: string[][] }) => Promise<void>
>(async () => {});

vi.mock("../config/config.js", () => ({
  readConfigFileSnapshot: () => mockReadConfigFileSnapshot(),
  writeConfigFile: (cfg: TraversalAIConfig, options?: { unsetPaths?: string[][] }) =>
...
function setSnapshot(resolved: TraversalAIConfig, config: TraversalAIConfig) {
...
describe("config cli", () => {
  ...
  describe("config set - issue #6070", () => {
    it("preserves existing config keys when setting a new value", async () => {
      const resolved: TraversalAIConfig = {
        agents: {
      ...
      const runtimeMerged: TraversalAIConfig = {
        ...resolved,
        agents: {
          ...resolved.agents,
          defaults: {
  ...
  describe("config get", () => {
    it("redacts sensitive values", async () => {
      ...
      expect(mockLog).toHaveBeenCalledWith("__TRAVERSALAI_REDACTED__");
    });
  });

  describe("config set parsing flags", () => {
    it("falls back to raw string when parsing fails and strict mode is off", async () => {
      const resolved: TraversalAIConfig = { gateway: { port: 18789 } };
      setSnapshot(resolved, resolved);
