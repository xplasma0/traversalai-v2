    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["traversalai memory status", "Show index and provider status."],
          ["traversalai memory index --force", "Force a full reindex."],
          ['traversalai memory search --query "deployment notes"', "Search indexed memory entries."],
          ["traversalai memory status --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "xplasma0.github.io/traversalai-docs/cli/memory")}\n`,
    );
