...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@traversalai/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@traversalai/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@traversalai/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@traversalai/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @traversalai/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      ...
      resolvedName: "@traversalai/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@traversalai/plugin-alpha@1.2.3",
        installPath: "/tmp/traversalai/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@traversalai/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @traversalai/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@traversalai/plugin-alpha@latest",
      true,
      "/tmp/traversalai/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@traversalai/plugin-alpha@latest",
      installPath: "/tmp/traversalai/extensions/alpha",
      version: "1.2.3",
