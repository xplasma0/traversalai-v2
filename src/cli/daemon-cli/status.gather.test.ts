...
const serviceReadCommand = vi.fn(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    TRAVERSALAI_STATE_DIR: "/tmp/traversalai-daemon",
    TRAVERSALAI_CONFIG_PATH: "/tmp/traversalai-daemon/traversalai.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.TRAVERSALAI_STATE_DIR ?? "/tmp/traversalai-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.TRAVERSALAI_CONFIG_PATH ?? `${stateDir}/traversalai.json`;
});

vi.mock("../../config/config.js", () => ({
  createConfigIO: ({ configPath }: { configPath: string }) => {
    const isDaemon = configPath.includes("/traversalai-daemon/");
    return {
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.TRAVERSALAI_STATE_DIR = "/tmp/traversalai-cli";
    process.env.TRAVERSALAI_CONFIG_PATH = "/tmp/traversalai-cli/traversalai.json";
