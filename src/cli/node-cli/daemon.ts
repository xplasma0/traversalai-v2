...
import { resolveDefaultGatewayPort, resolveIsNixMode } from "../../config/paths.js";
import {
...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("traversalai node install"), formatCliCommand("traversalai node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("traversalai node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    TRAVERSALAI_LOG_PREFIX: baseEnv.TRAVERSALAI_LOG_PREFIX ?? "node",
