    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            `traversalai node run --host 127.0.0.1 --port ${DEFAULT_GATEWAY_PORT}`,
            "Run the node host in the foreground.",
          ],
          ["traversalai node status", "Check node host service status."],
          ["traversalai node install", "Install the node host service."],
          ["traversalai node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "xplasma0.github.io/traversalai-docs/cli/node")}\n`,
    );
