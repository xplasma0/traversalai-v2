...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "xplasma0.github.io/traversalai-docs/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + TraversalAI hooks")
    ...
    .option("--hook-url <url>", "TraversalAI hook URL")
    .option("--hook-token <token>", "TraversalAI hook token")
