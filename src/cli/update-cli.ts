...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update TraversalAI and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["traversalai update", "Update a source checkout (git)"],
        ["traversalai update --channel beta", "Switch to beta channel (git + npm)"],
        ["traversalai update --channel dev", "Switch to dev channel (git + npm)"],
        ["traversalai update --tag beta", "One-off update to a dist-tag or version"],
        ["traversalai update --dry-run", "Preview actions without changing anything"],
        ["traversalai update --no-restart", "Update without restarting the service"],
        ["traversalai update --json", "Output result as JSON"],
        ["traversalai update --yes", "Non-interactive (accept downgrade prompts)"],
        ["traversalai update wizard", "Interactive update wizard"],
        ["traversalai --update", "Shorthand for traversalai update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run traversalai update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "xplasma0.github.io/traversalai-docs/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["traversalai update status", "Show channel + version status."],
          ["traversalai update status --json", "JSON output."],
          ["traversalai update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "xplasma0.github.io/traversalai-docs/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
