...
async function loadValidConfig(runtime: RuntimeEnv = defaultRuntime) {
  ...
  runtime.error(`Run \`${formatCliCommand("traversalai doctor")}\` to repair, then retry.`);
  runtime.exit(1);
  return snapshot;
}
...
export function registerConfigCli(program: Command) {
  const cmd = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/config", "xplasma0.github.io/traversalai-docs/cli/config")}\n`,
    )
    .option(
      "--section <section>",
