...
const shouldRegisterPrimaryOnly = (argv: string[]) => {
  if (isTruthyEnvValue(process.env.TRAVERSALAI_DISABLE_LAZY_SUBCOMMANDS)) {
    return false;
  }
...
const shouldEagerRegisterSubcommands = (_argv: string[]) => {
  return isTruthyEnvValue(process.env.TRAVERSALAI_DISABLE_LAZY_SUBCOMMANDS);
};

const loadConfig = async (): Promise<TraversalAIConfig> => {
...
const entries: SubCliEntry[] = [
  ...
  {
    name: "docs",
    description: "Search the live TraversalAI docs",
    hasSubcommands: false,
    register: async (program) => {
  ...
  {
    name: "plugins",
    description: "Manage TraversalAI plugins and extensions",
    hasSubcommands: true,
    register: async (program) => {
  ...
  {
    name: "chat",
    description: "Open a standalone chat interface connected to the Gateway",
    hasSubcommands: false,
    register: async (program) => {
      const mod = await import("../standalone-chat-cli.js");
      mod.registerStandaloneChatCli(program);
    },
  },
];
