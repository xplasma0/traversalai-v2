...
describe("registerSubCliCommands", () => {
  ...
  const originalDisableLazySubcommands = process.env.TRAVERSALAI_DISABLE_LAZY_SUBCOMMANDS;
  ...
  beforeEach(() => {
    if (originalDisableLazySubcommands === undefined) {
      delete process.env.TRAVERSALAI_DISABLE_LAZY_SUBCOMMANDS;
    } else {
      process.env.TRAVERSALAI_DISABLE_LAZY_SUBCOMMANDS = originalDisableLazySubcommands;
    }
  ...
  it("registers only the primary placeholder and dispatches", async () => {
    const program = createRegisteredProgram(["node", "traversalai", "acp"]);
  ...
  it("registers placeholders for all subcommands when no primary", () => {
    const program = createRegisteredProgram(["node", "traversalai"]);
  ...
  it("re-parses argv for lazy subcommands", async () => {
    const program = createRegisteredProgram(["node", "traversalai", "nodes", "list"], "traversalai");
  ...
  it("replaces placeholder when registering a subcommand by name", async () => {
    const program = createRegisteredProgram(["node", "traversalai", "acp", "--help"], "traversalai");
