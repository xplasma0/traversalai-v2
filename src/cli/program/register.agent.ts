...
${formatHelpExamples([
  ['traversalai agent --to +15555550123 --message "status update"', "Start a new session."],
  ['traversalai agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'traversalai agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'traversalai agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['traversalai agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "xplasma0.github.io/traversalai-docs/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "xplasma0.github.io/traversalai-docs/cli/agents")}\n`,
    );
...
${formatHelpExamples([
  [
    'traversalai agents set-identity --agent main --name "TraversalAI" --emoji "🦞"',
    "Set name + emoji.",
  ],
  [
  ...
  [
    "traversalai agents set-identity --identity-file ~/.traversalai/workspace/IDENTITY.md --agent main",
