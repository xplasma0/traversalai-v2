...
beforeEach(() => {
  ...
  originalHideBanner = process.env.TRAVERSALAI_HIDE_BANNER;
  delete process.env.NODE_NO_WARNINGS;
  delete process.env.TRAVERSALAI_HIDE_BANNER;
});
...
afterEach(() => {
  ...
  if (originalHideBanner === undefined) {
    ...
    process.env.TRAVERSALAI_HIDE_BANNER = originalHideBanner;
  }
});
...
describe("registerPreActionHooks", () => {
  ...
  it("emits banner, resolves config, and enables verbose from --debug", async () => {
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "traversalai", "status", "--debug"],
    });
    ...
    expect(process.title).toBe("traversalai-status");
  });

  it("loads plugin registry for plugin-required commands", async () => {
    await runCommand({
      parseArgv: ["message", "send"],
      processArgv: ["node", "traversalai", "message", "send"],
    });
  ...
  it("loads plugin registry for configure command", async () => {
    await runCommand({
      parseArgv: ["configure"],
      processArgv: ["node", "traversalai", "configure"],
    });
  ...
  it("loads plugin registry for onboard command", async () => {
    await runCommand({
      parseArgv: ["onboard"],
      processArgv: ["node", "traversalai", "onboard"],
    });
  ...
  it("loads plugin registry for agents command", async () => {
    await runCommand({
      parseArgv: ["agents"],
      processArgv: ["node", "traversalai", "agents"],
    });
  ...
  it("skips config guard for doctor, completion, and secrets commands", async () => {
    await runCommand({
      parseArgv: ["doctor"],
      processArgv: ["node", "traversalai", "doctor"],
    });
    await runCommand({
      parseArgv: ["completion"],
      processArgv: ["node", "traversalai", "completion"],
    });
    await runCommand({
      parseArgv: ["secrets"],
      processArgv: ["node", "traversalai", "secrets"],
    });
  ...
  it("skips preaction work when argv indicates help/version", async () => {
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "traversalai", "--version"],
    });
  ...
  it("hides banner when TRAVERSALAI_HIDE_BANNER is truthy", async () => {
    process.env.TRAVERSALAI_HIDE_BANNER = "1";
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "traversalai", "status"],
    });
