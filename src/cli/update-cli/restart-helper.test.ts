...
describe("restart-helper", () => {
  ...
  describe("prepareRestartScript", () => {
    it("creates a systemd restart script on Linux", async () => {
      ...
      expect(content).toContain("systemctl --user restart 'traversalai-gateway.service'");
      // Script should self-cleanup
      expect(content).toContain('rm -f "$0"');
      await cleanupScript(scriptPath);
    });

    it("uses TRAVERSALAI_SYSTEMD_UNIT override for systemd scripts", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        TRAVERSALAI_PROFILE: "default",
        TRAVERSALAI_SYSTEMD_UNIT: "custom-gateway",
      });
    ...
    it("creates a launchd restart script on macOS", async () => {
      ...
      expect(scriptPath.endsWith(".sh")).toBe(true);
      expect(content).toContain("#!/bin/sh");
      expect(content).toContain("launchctl kickstart -k 'gui/501/ai.traversalai.gateway'");
      expect(content).toContain('rm -f "$0"');
      await cleanupScript(scriptPath);
    });

    it("uses TRAVERSALAI_LAUNCHD_LABEL override on macOS", async () => {
      Object.defineProperty(process, "platform", { value: "darwin" });
      process.getuid = () => 501;

      const { scriptPath, content } = await prepareAndReadScript({
        TRAVERSALAI_PROFILE: "default",
        TRAVERSALAI_LAUNCHD_LABEL: "com.custom.traversalai",
      });
      expect(content).toContain("launchctl kickstart -k 'gui/501/com.custom.traversalai'");
      await cleanupScript(scriptPath);
    });
    ...
    it("creates a schtasks restart script on Windows", async () => {
      ...
      expect(content).toContain('schtasks /End /TN "TraversalAI Gateway"');
      expect(content).toContain('schtasks /Run /TN "TraversalAI Gateway"');
      // Batch self-cleanup
      expect(content).toContain('del "%~f0"');
      await cleanupScript(scriptPath);
    });

    it("uses TRAVERSALAI_WINDOWS_TASK_NAME override on Windows", async () => {
      Object.defineProperty(process, "platform", { value: "win32" });

      const { scriptPath, content } = await prepareAndReadScript({
        TRAVERSALAI_PROFILE: "default",
        TRAVERSALAI_WINDOWS_TASK_NAME: "TraversalAI Gateway (custom)",
      });
      expect(content).toContain('schtasks /End /TN "TraversalAI Gateway (custom)"');
      expect(content).toContain('schtasks /Run /TN "TraversalAI Gateway (custom)"');
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in service names", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        TRAVERSALAI_PROFILE: "production",
      });
      expect(content).toContain("traversalai-gateway-production.service");
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in macOS launchd label", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        TRAVERSALAI_PROFILE: "staging",
      });
      expect(content).toContain("gui/502/ai.traversalai.staging");
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in Windows task name", async () => {
      ...
      expect(content).toContain('schtasks /End /TN "TraversalAI Gateway (production)"');
      await cleanupScript(scriptPath);
    });
    ...
    it("escapes single quotes in profile names for shell scripts", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        TRAVERSALAI_PROFILE: "it's-a-test",
      });
    ...
    it("rejects unsafe batch profile names on Windows", async () => {
      Object.defineProperty(process, "platform", { value: "win32" });
      const scriptPath = await prepareRestartScript({
        TRAVERSALAI_PROFILE: "test&whoami",
      });
