...
import type { TraversalAIConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: TraversalAIConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: TraversalAIConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = TraversalAISchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as TraversalAIConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as TraversalAIConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  return {
    ok: true,
    config: validated.data as TraversalAIConfig,
  };
}
