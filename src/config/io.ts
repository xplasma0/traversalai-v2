...
import type { TraversalAIConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareTraversalAIVersions } from "./version.js";
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  traversalai config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  traversalai config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: TraversalAIConfig,
  pathSegments: string[],
): { changed: boolean; next: TraversalAIConfig } {
...
function coerceConfig(value: unknown): TraversalAIConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as TraversalAIConfig;
}
...
function stampConfigVersion(cfg: TraversalAIConfig): TraversalAIConfig {
...
function warnIfConfigFromFuture(cfg: TraversalAIConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareTraversalAIVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    ...
    logger.warn(
      `Config was last written by a newer ${appName} (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as TraversalAIConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): TraversalAIConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as TraversalAIConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: TraversalAIConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as TraversalAIConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.TRAVERSALAI_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.TRAVERSALAI_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.TRAVERSALAI_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.TRAVERSALAI_WATCH_SESSION === "string" &&
        deps.env.TRAVERSALAI_WATCH_SESSION.trim().length > 0
          ? deps.env.TRAVERSALAI_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.TRAVERSALAI_WATCH_COMMAND === "string" &&
        deps.env.TRAVERSALAI_WATCH_COMMAND.trim().length > 0
          ? deps.env.TRAVERSALAI_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `TRAVERSALAI_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSnapshot: TraversalAIConfig | null = null;
let runtimeConfigSourceSnapshot: TraversalAIConfig | null = null;

function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.TRAVERSALAI_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.TRAVERSALAI_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function setRuntimeConfigSnapshot(
  config: TraversalAIConfig,
  sourceConfig?: TraversalAIConfig,
): void {
...
export function getRuntimeConfigSnapshot(): TraversalAIConfig | null {
  return runtimeConfigSnapshot;
}

export function loadConfig(): TraversalAIConfig {
