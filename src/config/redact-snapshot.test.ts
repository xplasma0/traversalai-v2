...
import type { ConfigFileSnapshot } from "./types.traversalai.js";
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.traversalai/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/traversalai/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/traversalai/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/traversalai/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/traversalai/nickserv-password.txt");
  ...
  it("contract-covers dynamic catchall/record paths for redact+restore", () => {
    const hints = mapSensitivePaths(TraversalAISchema, "", {});
    const snapshot = makeSnapshot({
      env: {
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = TraversalAISchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "TraversalAIConfig";
    const hints = mapSensitivePaths(TraversalAISchema, "", {});
