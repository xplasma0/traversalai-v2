...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("traversalai-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "traversalai-sandbox-browser-global" },
      agentBrowser: { network: "traversalai-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("traversalai-sandbox-browser-agent");
  });
