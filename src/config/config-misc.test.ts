...
describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = TraversalAISchema.safeParse({
      $schema: "https://traversalai.ai/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://traversalai.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = TraversalAISchema.safeParse({ $schema: 123 });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = TraversalAISchema.safeParse({
      cron: {
        enabled: true,
