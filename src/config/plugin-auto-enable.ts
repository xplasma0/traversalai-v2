...
function isWhatsAppConfigured(cfg: TraversalAIConfig): boolean {
...
function isGenericChannelConfigured(cfg: TraversalAIConfig, channelId: string): boolean {
  const entry = resolveChannelConfig(cfg, channelId);
  return recordHasKeys(entry);
}
...
function collectModelRefs(cfg: TraversalAIConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: TraversalAIConfig, providerId: string): boolean {
...
function collectCandidateChannelIds(cfg: TraversalAIConfig): string[] {
...
function isPluginExplicitlyDisabled(cfg: TraversalAIConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: TraversalAIConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: TraversalAIConfig, pluginId: string): TraversalAIConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
