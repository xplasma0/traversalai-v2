...
type GetReplyFromConfigFn = (
  ...
  configOverride?: TraversalAIConfig,
...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `traversalai-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.TRAVERSALAI_CONFIG_PATH = path.join(root, "traversalai.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "traversalai.json");
  ...
  return {
    ...
    loadConfig: () => {
      ...
      const defaults = {
        ...
        workspace: path.join(os.tmpdir(), "traversalai-gateway-test"),
        ...fileDefaults,
...
vi.mock("../plugins/loader.js", async () => {
  ...
  return {
    ...actual,
    loadTraversalAIPlugins: () => pluginRegistryState.registry,
  };
});
