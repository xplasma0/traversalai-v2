...
import { createTraversalAITools } from "../agents/traversalai-tools.js";
import {
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-traversalai-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-traversalai-account-id")?.trim() || undefined;
  const agentTo = getHeader(req, "x-traversalai-message-to")?.trim() || undefined;
  const agentThreadId = getHeader(req, "x-traversalai-thread-id")?.trim() || undefined;
  ...
  const allTools = createTraversalAITools({
