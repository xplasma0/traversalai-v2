...
let lastCreateTraversalAIToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/traversalai-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateTraversalAIToolsContext?.agentTo,
          agentThreadId: lastCreateTraversalAIToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createTraversalAITools: (ctx: Record<string, unknown>) => {
      lastCreateTraversalAIToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateTraversalAIToolsContext = undefined;
});
...
describe("POST /tools/invoke", () => {
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-traversalai-message-to": "channel:24514",
        "x-traversalai-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
