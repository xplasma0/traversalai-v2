...
const { createTraversalAITools } = await import("../agents/traversalai-tools.js");
...
type SessionSendTool = ReturnType<typeof createTraversalAITools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createTraversalAITools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["TRAVERSALAI_GATEWAY_PORT", "TRAVERSALAI_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.TRAVERSALAI_GATEWAY_PORT = String(gatewayPort);
  process.env.TRAVERSALAI_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("TRAVERSALAI_CONFIG_PATH missing in gateway test environment");
      }
