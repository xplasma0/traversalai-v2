...
export function resolveHooksConfig(cfg: TraversalAIConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: TraversalAIConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-traversalai-token"] === "string"
      ? req.headers["x-traversalai-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
