...
const GATEWAY_TEST_ENV_KEYS = [
  "HOME",
  "USERPROFILE",
  "TRAVERSALAI_STATE_DIR",
  "TRAVERSALAI_CONFIG_PATH",
  "TRAVERSALAI_SKIP_BROWSER_CONTROL_SERVER",
  "TRAVERSALAI_SKIP_GMAIL_WATCHER",
  "TRAVERSALAI_SKIP_CANVAS_HOST",
  "TRAVERSALAI_BUNDLED_PLUGINS_DIR",
  "TRAVERSALAI_SKIP_CHANNELS",
  "TRAVERSALAI_SKIP_PROVIDERS",
  "TRAVERSALAI_SKIP_CRON",
  "TRAVERSALAI_TEST_MINIMAL_GATEWAY",
] as const;
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "traversalai-gateway-home-"));
...
function applyGatewaySkipEnv() {
  process.env.TRAVERSALAI_SKIP_BROWSER_CONTROL_SERVER = "1";
  ...
  process.env.TRAVERSALAI_SKIP_PROVIDERS = "1";
  process.env.TRAVERSALAI_SKIP_CRON = "1";
  process.env.TRAVERSALAI_TEST_MINIMAL_GATEWAY = "1";
  process.env.TRAVERSALAI_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "traversalai-test-no-bundled-extensions")
    : "traversalai-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    tempConfigRoot = await fs.mkdtemp(path.join(tempHome, "traversalai-test-"));
  } else {
    tempConfigRoot = path.join(tempHome, ".traversalai-test");
...
const CONNECT_CHALLENGE_NONCE_KEY = "__traversalaiTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__traversalaiTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["TRAVERSALAI_GATEWAY_TOKEN"]);
  const prev = process.env.TRAVERSALAI_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.TRAVERSALAI_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.TRAVERSALAI_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
