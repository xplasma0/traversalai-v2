...
export class GatewayClient {
  ...
  start() {
    ...
    const url = this.opts.url ?? `ws://127.0.0.1:${defaultPort}`;
    ...
    if (!isSecureWebSocketUrl(url)) {
      ...
      const error = new Error(
        `SECURITY ERROR: Cannot connect to "${displayHost}" over plaintext ws://. ` +
          ...
          `(ssh -N -L ${defaultPort}:127.0.0.1:${defaultPort} user@gateway-host), or use Tailscale Serve/Funnel. ` +
          "Run `traversalai doctor --fix` for guidance.",
      );
