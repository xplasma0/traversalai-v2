...
const connectNodeClient = async (params: {
  ...
  const token = process.env.TRAVERSALAI_GATEWAY_TOKEN;
  if (!token) {
    throw new Error("TRAVERSALAI_GATEWAY_TOKEN is required for node test clients");
  }
...
describe("gateway update.run", () => {
  test("writes sentinel and schedules restart", async () => {
    ...
    try {
      ...
      const sentinelPath = path.join(os.homedir(), ".traversalai", "restart-sentinel.json");
...
describe("gateway node command allowlist", () => {
  ...
  test("rejects reconnect metadata spoof for paired node devices", async () => {
    ...
    const deviceIdentityPath = path.join(
      os.tmpdir(),
      `traversalai-spoof-test-device-${Date.now()}-${Math.random().toString(36).slice(2)}.json`,
    );
