...
describe("shared/frontmatter", () => {
  ...
  test("resolveTraversalAIManifestBlock parses JSON5 metadata and picks traversalai block", () => {
    const frontmatter = {
      metadata: "{ traversalai: { foo: 1, bar: 'baz' } }",
    };
    expect(resolveTraversalAIManifestBlock({ frontmatter })).toEqual({ foo: 1, bar: "baz" });
  });

  test("resolveTraversalAIManifestBlock returns undefined for invalid input", () => {
    expect(resolveTraversalAIManifestBlock({ frontmatter: {} })).toBeUndefined();
    expect(
      resolveTraversalAIManifestBlock({ frontmatter: { metadata: "not-json5" } }),
    ).toBeUndefined();
    expect(
      resolveTraversalAIManifestBlock({ frontmatter: { metadata: "{ nope: { a: 1 } }" } }),
    ).toBeUndefined();
  });
});
