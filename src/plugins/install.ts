...
async function ensureTraversalAIExtensions(manifest: PackageManifest) {
  ...
  if (list.length === 0) {
    throw new Error("package.json traversalai.extensions is empty");
  }
  return list;
}
...
async function installPluginFromPackageDir(params: {
  ...
  try {
    extensions = await ensureTraversalAIExtensions(manifest);
  } catch (err) {
  ...
  // Prefer the canonical `id` from traversalai.plugin.json over the npm package name.
  // This avoids a latent key-mismatch bug: if the manifest id (e.g. "memory-cognee")
  // differs from the npm package name (e.g. "cognee-traversalai"), the plugin registry
  ...
  try {
    ...
    if (scanSummary.critical > 0) {
      ...
      logger.warn?.(
        `Plugin "${pluginId}" has ${scanSummary.warn} suspicious code pattern(s). Run "traversalai security audit --deep" for details.`,
      );
    }
  } catch (err) {
    logger.warn?.(
      `Plugin "${pluginId}" code safety scan failed (${String(err)}). Installation continues; run "traversalai security audit --deep" after install.`,
    );
  }
...
export async function installPluginFromArchive(params: {
  ...
  return await withExtractedArchiveRoot({
    archivePath,
    tempDirPrefix: "traversalai-plugin-",
    timeoutMs,
    logger,
...
export async function installPluginFromNpmSpec(params: {
  ...
  const flowResult = await installFromNpmSpecArchiveWithInstaller({
    tempDirPrefix: "traversalai-npm-pack-",
    spec,
    timeoutMs,
