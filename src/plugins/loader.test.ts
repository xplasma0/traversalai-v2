...
import { __testing, loadTraversalAIPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const fixtureRoot = path.join(os.tmpdir(), `traversalai-plugin-${randomUUID()}`);
let tempDirIndex = 0;
const prevBundledDir = process.env.TRAVERSALAI_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "traversalai.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  process.env.TRAVERSALAI_BUNDLED_PLUGINS_DIR = bundledDir;

  return loadTraversalAIPlugins({
    cache: false,
    config: {
      plugins: {
        slots: {
...
function expectTelegramLoaded(registry: ReturnType<typeof loadTraversalAIPlugins>) {
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    delete process.env.TRAVERSALAI_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.TRAVERSALAI_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadTraversalAIPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const enabledRegistry = loadTraversalAIPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("loads plugins from config paths", () => {
    process.env.TRAVERSALAI_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
  ...
  it("warns when loaded non-bundled plugin has no install/load-path provenance", () => {
    ...
    withEnv({ TRAVERSALAI_STATE_DIR: stateDir, CLAWDBOT_STATE_DIR: undefined }, () => {
