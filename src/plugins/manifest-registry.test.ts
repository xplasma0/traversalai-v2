...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `traversalai-manifest-registry-${randomUUID()}`);
...
function writeManifest(dir: string, manifest: Record<string, unknown>) {
  fs.writeFileSync(path.join(dir, "traversalai.plugin.json"), JSON.stringify(manifest), "utf-8");
}
...
describe("loadPluginManifestRegistry", () => {
  ...
  it("rejects manifest paths that escape plugin root via symlink", () => {
    ...
    const outsideManifest = path.join(outsideDir, "traversalai.plugin.json");
    const linkedManifest = path.join(rootDir, "traversalai.plugin.json");
