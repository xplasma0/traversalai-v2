...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: TraversalAIPluginDefinition;
  register?: TraversalAIPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as TraversalAIPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as TraversalAIPluginDefinition;
...
export function loadTraversalAIPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverTraversalAIPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const getJiti = () => {
    ...
    jitiLoader = createJiti(import.meta.url, {
      ...
      ...(pluginSdkAlias || pluginSdkAccountIdAlias
        ? {
            alias: {
              ...(pluginSdkAlias ? { "traversalai/plugin-sdk": pluginSdkAlias } : {}),
              ...(pluginSdkAlias ? { "traversalai/plugin-sdk": pluginSdkAlias } : {}),
              ...(pluginSdkAccountIdAlias
                ? { "traversalai/plugin-sdk/account-id": pluginSdkAccountIdAlias }
                : {}),
              ...(pluginSdkAccountIdAlias
                ? { "traversalai/plugin-sdk/account-id": pluginSdkAccountIdAlias }
                : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: TraversalAIPluginModule | null = null;
    try {
      mod = getJiti()(safeSource) as TraversalAIPluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,
