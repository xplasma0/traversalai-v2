...
describe("writeOAuthCredentials", () => {
  const lifecycle = createAuthTestLifecycle([
    ...
    "TRAVERSALAI_AGENT_DIR",
    "PI_CODING_AGENT_DIR",
    "TRAVERSALAI_OAUTH_DIR",
  ]);
  ...
  it("writes auth-profiles.json under TRAVERSALAI_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("traversalai-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "traversalai-oauth-sync-"));
    ...
    process.env.TRAVERSALAI_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "traversalai-oauth-nosync-"));
    process.env.TRAVERSALAI_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside TRAVERSALAI_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "traversalai-oauth-external-"));
    process.env.TRAVERSALAI_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* TRAVERSALAI_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to TRAVERSALAI_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("traversalai-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
