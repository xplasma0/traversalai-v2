...
import { TRAVERSALAI_BUILTIN_AGENT_IDS } from "../agents/builtins.js";
...
export function summarizeExistingConfig(config: TraversalAIConfig): string {
  const rows: string[] = [];
...
export function formatControlUiSshHint(params: {
  ...
  return [
    ...
    authedUrl,
    "Docs:",
    "https://docs.traversalai.ai/gateway/remote",
    "https://docs.traversalai.ai/web/control-ui",
  ]
    .filter(Boolean)
    .join("\n");
}
...
export async function ensureTraversalAiBuiltInAgentResources(
  cfg: TraversalAIConfig,
  runtime: RuntimeEnv,
  ...
  for (const agentId of TRAVERSALAI_BUILTIN_AGENT_IDS) {
    await ensureWorkspaceAndSessions(resolveAgentWorkspaceDir(cfg, agentId), runtime, {
