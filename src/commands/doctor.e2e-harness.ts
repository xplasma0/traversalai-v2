...
export const resolveTraversalAIPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadTraversalAIPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../config/config.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    CONFIG_PATH: "/tmp/traversalai.json",
    createConfigIO,
...
beforeEach(() => {
  ...
  resolveTraversalAIPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.TRAVERSALAI_STATE_DIR;
  originalUpdateInProgress = process.env.TRAVERSALAI_UPDATE_IN_PROGRESS;
  process.env.TRAVERSALAI_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "traversalai-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.TRAVERSALAI_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.TRAVERSALAI_UPDATE_IN_PROGRESS;
  } else {
    process.env.TRAVERSALAI_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
