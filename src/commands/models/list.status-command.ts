...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveTraversalAIAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("traversalai models auth setup-token")}\` or \`${formatCliCommand("traversalai configure")}\`.`
          : `Run \`${formatCliCommand("traversalai configure")}\` or set an API key env var.`;
