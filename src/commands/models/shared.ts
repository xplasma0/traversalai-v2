...
export async function loadValidConfigOrThrow(): Promise<TraversalAIConfig> {
...
export async function updateConfig(
  mutator: (cfg: TraversalAIConfig) => TraversalAIConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: TraversalAIConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: TraversalAIConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("traversalai agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
