...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveTraversalAIAgentDir: vi.fn().mockReturnValue("/tmp/traversalai-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/traversalai-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/traversalai-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveTraversalAIAgentDir: mocks.resolveTraversalAIAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/traversalai-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveTraversalAIAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/traversalai-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/traversalai-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/traversalai-agent-custom");
