...
describe("onboard auth credentials secret refs", () => {
  ...
  it("keeps env-backed moonshot key as plaintext by default", async () => {
    const env = await setupAuthTestEnv("traversalai-onboard-auth-credentials-");
  ...
  it("stores env-backed moonshot key as keyRef when secret-input-mode=ref", async () => {
    const env = await setupAuthTestEnv("traversalai-onboard-auth-credentials-ref-");
    lifecycle.setStateDir(env.stateDir);
    process.env.MOONSHOT_API_KEY = "sk-moonshot-env";
  ...
  it("stores ${ENV} moonshot input as keyRef even when env value is unset", async () => {
    const env = await setupAuthTestEnv("traversalai-onboard-auth-credentials-inline-ref-");
    lifecycle.setStateDir(env.stateDir);
  ...
  it("keeps plaintext moonshot key when no env ref applies", async () => {
    const env = await setupAuthTestEnv("traversalai-onboard-auth-credentials-plaintext-");
    lifecycle.setStateDir(env.stateDir);
  ...
  it("preserves cloudflare metadata when storing keyRef", async () => {
    const env = await setupAuthTestEnv("traversalai-onboard-auth-credentials-cloudflare-");
    lifecycle.setStateDir(env.stateDir);
  ...
  it("keeps env-backed openai key as plaintext by default", async () => {
    const env = await setupAuthTestEnv("traversalai-onboard-auth-credentials-openai-");
    lifecycle.setStateDir(env.stateDir);
  ...
  it("stores env-backed openai key as keyRef in ref mode", async () => {
    const env = await setupAuthTestEnv("traversalai-onboard-auth-credentials-openai-ref-");
    lifecycle.setStateDir(env.stateDir);
    process.env.OPENAI_API_KEY = "sk-openai-env";
  ...
  it("stores env-backed volcengine and byteplus keys as keyRef in ref mode", async () => {
    const env = await setupAuthTestEnv("traversalai-onboard-auth-credentials-volc-byte-");
    lifecycle.setStateDir(env.stateDir);
