...
function resolveAgentName(cfg: TraversalAIConfig, agentId: string) {
...
function resolveAgentModel(cfg: TraversalAIConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: TraversalAIConfig): AgentSummary[] {
