 ...
 * Runs as part of `traversalai doctor` — config-only, no network calls.
 */
  ...
  if (resolved.provider !== "auto") {
    if (resolved.provider === "local") {
      ...
      note(
        [
          ...
          `- Switch to a remote provider: ${formatCliCommand("traversalai config set agents.defaults.memorySearch.provider openai")}`,
          "",
          `Verify: ${formatCliCommand("traversalai memory status --deep")}`,
        ].join("\n"),
        "Memory search",
      );
      return;
    ...
    note(
      [
        ...
        `- Configure credentials: ${formatCliCommand("traversalai configure --section model")}`,
        `- To disable: ${formatCliCommand("traversalai config set agents.defaults.memorySearch.enabled false")}`,
        "",
        `Verify: ${formatCliCommand("traversalai memory status --deep")}`,
      ].join("\n"),
      "Memory search",
