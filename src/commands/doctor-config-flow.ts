...
function stripUnknownConfigKeys(config: TraversalAIConfig): {
  config: TraversalAIConfig;
  removed: string[];
} {
  const parsed = TraversalAISchema.safeParse(config);
  if (parsed.success) {
...
function noteOpencodeProviderOverrides(cfg: TraversalAIConfig) {
...
export function collectMissingDefaultAccountBindingWarnings(cfg: TraversalAIConfig): string[] {
...
function scanTelegramAllowFromUsernameEntries(cfg: TraversalAIConfig): TelegramAllowFromUsernameHit[] {
...
async function maybeRepairTelegramAllowFromUsernames(cfg: TraversalAIConfig): Promise<{
  config: TraversalAIConfig;
  changes: string[];
}> {
...
function scanDiscordNumericIdEntries(cfg: TraversalAIConfig): DiscordNumericIdHit[] {
...
function maybeRepairDiscordNumericIds(cfg: TraversalAIConfig): {
  config: TraversalAIConfig;
  changes: string[];
} {
...
function scanMutableAllowlistEntries(cfg: TraversalAIConfig): MutableAllowlistHit[] {
 ...
 */
function maybeRepairOpenPolicyAllowFrom(cfg: TraversalAIConfig): {
  config: TraversalAIConfig;
  changes: string[];
} {
 ...
 * file support.
 */
function detectEmptyAllowlistPolicy(cfg: TraversalAIConfig): string[] {
  const channels = cfg.channels;
  if (!channels || typeof channels !== "object") {
    return [];
  }
...
function collectExecSafeBinScopes(cfg: TraversalAIConfig): ExecSafeBinScopeRef[] {
...
function scanExecSafeBinCoverage(cfg: TraversalAIConfig): ExecSafeBinCoverageHit[] {
...
function scanExecSafeBinTrustedDirHints(cfg: TraversalAIConfig): ExecSafeBinTrustedDirHintHit[] {
...
function maybeRepairExecSafeBinProfiles(cfg: TraversalAIConfig): {
  config: TraversalAIConfig;
  changes: string[];
  warnings: string[];
} {
...
function scanLegacyToolsBySenderKeys(cfg: TraversalAIConfig): LegacyToolsBySenderKeyHit[] {
  const hits: LegacyToolsBySenderKeyHit[] = [];
  collectLegacyToolsBySenderKeyHits(cfg, [], hits);
  return hits;
}

function maybeRepairLegacyToolsBySenderKeys(cfg: TraversalAIConfig): {
  config: TraversalAIConfig;
  changes: string[];
} {
  const next = structuredClone(cfg);
...
async function maybeMigrateLegacyConfig(): Promise<string[]> {
  ...
  const targetDir = path.join(home, ".traversalai");
  const targetPath = path.join(targetDir, "traversalai.json");
  try {
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  let cfg: TraversalAIConfig = baseCfg;
  ...
  if (snapshot.legacyIssues.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(
        `Run "${formatCliCommand("traversalai doctor --fix")}" to apply compatibility migrations.`,
      );
    }
  }
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("traversalai doctor --fix")}" to apply these changes.`);
    }
  }
  ...
  if (shouldRepair) {
    ...
    if (hits.length > 0) {
      note(
        [
          ...
          `- Run "${formatCliCommand("traversalai doctor --fix")}" to auto-resolve @username entries to numeric IDs (requires a Telegram bot token).`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (discordHits.length > 0) {
      note(
        [
          ...
          `- Discord IDs must be strings; run "${formatCliCommand("traversalai doctor --fix")}" to convert numeric IDs to quoted strings.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (allowFromScan.changes.length > 0) {
      note(
        [
          ...
          `- Run "${formatCliCommand("traversalai doctor --fix")}" to add missing allowFrom wildcards.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (toolsBySenderHits.length > 0) {
      ...
      note(
        [
          ...
          `- Run "${formatCliCommand("traversalai doctor --fix")}" to migrate legacy keys to typed id: entries.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (safeBinCoverage.length > 0) {
      ...
      lines.push(
        `- Run "${formatCliCommand("traversalai doctor --fix")}" to scaffold missing custom safeBinProfiles entries.`,
      );
      note(lines.join("\n"), "Doctor warnings");
    }
  ...
  if (unknown.removed.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push('Run "traversalai doctor --fix" to remove these keys.');
    }
  }
