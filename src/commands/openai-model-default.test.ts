...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: TraversalAIConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: TraversalAIConfig) => { changed: boolean; next: TraversalAIConfig };
  defaultModel: string;
  overrideConfig: TraversalAIConfig;
  alreadyDefaultConfig: TraversalAIConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: TraversalAIConfig) => config,
      applyDefaultConfig: (config: TraversalAIConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: TraversalAIConfig = { agents: { defaults: {} } };
