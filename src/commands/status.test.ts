...
beforeAll(() => {
  envSnapshot = captureEnv(["TRAVERSALAI_PROFILE"]);
  process.env.TRAVERSALAI_PROFILE = "isolated";
});
...
vi.mock("../infra/traversalai-root.js", () => ({
  resolveTraversalAIPackageRoot: vi.fn().mockResolvedValue("/tmp/traversalai"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/traversalai/pnpm-lock.yaml",
      markerPath: "/tmp/traversalai/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.traversalai.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.traversalai.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    await statusCommand({}, runtime as never);
    const logs = runtimeLogMock.mock.calls.map((c: unknown[]) => String(c[0]));
    expect(logs.some((l: string) => l.includes("TraversalAI status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l: string) =>
          l.includes("traversalai status --all") ||
          l.includes("traversalai --profile isolated status --all") ||
          l.includes("traversalai status --all") ||
          l.includes("traversalai --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.TRAVERSALAI_GATEWAY_TOKEN = "abcd1234";
    try {
