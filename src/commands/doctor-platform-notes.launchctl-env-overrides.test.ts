...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "TRAVERSALAI_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("TRAVERSALAI_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv TRAVERSALAI_GATEWAY_TOKEN");
    expect(message).not.toContain("TRAVERSALAI_GATEWAY_PASSWORD");
  });
