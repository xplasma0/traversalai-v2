...
type PromptDefaultModelResult = { model?: string; config?: TraversalAIConfig };
...
function resolveConfiguredModelRaw(cfg: TraversalAIConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: TraversalAIConfig): string[] {
...
export function applyPrimaryModel(cfg: TraversalAIConfig, model: string): TraversalAIConfig {
...
export function applyModelAllowlist(cfg: TraversalAIConfig, models: string[]): TraversalAIConfig {
  const defaults = cfg.agents?.defaults;
