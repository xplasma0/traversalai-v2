...
import type { TraversalAIConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: TraversalAIConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("traversalai security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.traversalai/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("traversalai approvals get --gateway")}`,
    );
  }
  ...
  const defaultPort = resolveDefaultGatewayPort(process.env);
  const bindDescriptor = `"${gatewayBind}" (${resolvedBindHost})`;
  const saferRemoteAccessLines = [
    "  Safer remote access: keep bind loopback and use Tailscale Serve/Funnel or an SSH tunnel.",
    `  Example tunnel: ssh -N -L ${defaultPort}:127.0.0.1:${defaultPort} user@gateway-host`,
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("traversalai configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("traversalai config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("traversalai doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("traversalai config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('traversalai config set session.dmScope "per-channel-peer"') +
