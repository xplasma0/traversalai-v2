...
export async function onboardCommand(opts: OnboardOptions, runtime: RuntimeEnv = defaultRuntime) {
  ...
  if (normalizedOpts.nonInteractive && normalizedOpts.acceptRisk !== true) {
    runtime.error(
      [
        ...
        "Read: https://xplasma0.github.io/traversalai-docs/security",
        `Re-run with: ${formatCliCommand("traversalai onboard --non-interactive --accept-risk ...")}`,
      ].join("\n"),
    );
    runtime.exit(1);
    return;
  ...
  if (process.platform === "win32") {
    ...
    runtime.log(
      [
        `Windows detected — ${appName} runs great on WSL2!`,
        "Native Windows might be trickier.",
        "Quick setup: wsl --install (one command, one reboot)",
        "Guide: https://xplasma0.github.io/traversalai-docs/windows",
      ].join("\n"),
    );
  }
