...
export async function statusCommand(
  ...
  runtime.log(theme.heading("TraversalAI status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`traversalai devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("traversalai devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("traversalai devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("traversalai security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("traversalai security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://xplasma0.github.io/traversalai-docs/faq");
  runtime.log("Troubleshooting: https://xplasma0.github.io/traversalai-docs/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("traversalai status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("traversalai logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("traversalai status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("traversalai gateway probe")}`);
  }
}
