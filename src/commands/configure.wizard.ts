...
async function runGatewayHealthCheck(params: {
  ...
  const token = params.cfg.gateway?.auth?.token ?? process.env.TRAVERSALAI_GATEWAY_TOKEN;
  const password = params.cfg.gateway?.auth?.password ?? process.env.TRAVERSALAI_GATEWAY_PASSWORD;
    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from traversalai.json",
        },
      ],
...
export async function runConfigureWizard(
  ...
  try {
    ...
    intro(opts.command === "update" ? "TraversalAI update wizard" : "TraversalAI configure");
    const prompter = createClackPrompter();

    const snapshot = await readConfigFileSnapshot();
    const baseConfig: TraversalAIConfig = snapshot.valid ? snapshot.config : {};
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("traversalai doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    const localUrl = `ws://127.0.0.1:${resolveDefaultGatewayPort(process.env)}`;
    const localProbe = await probeGatewayReachable({
      url: localUrl,
      token: baseConfig.gateway?.auth?.token ?? process.env.TRAVERSALAI_GATEWAY_TOKEN,
      password: baseConfig.gateway?.auth?.password ?? process.env.TRAVERSALAI_GATEWAY_PASSWORD,
    });
    ...
    const newPassword = nextConfig.gateway?.auth?.password ?? process.env.TRAVERSALAI_GATEWAY_PASSWORD;
    const oldPassword = baseConfig.gateway?.auth?.password ?? process.env.TRAVERSALAI_GATEWAY_PASSWORD;
    const token = nextConfig.gateway?.auth?.token ?? process.env.TRAVERSALAI_GATEWAY_TOKEN;
