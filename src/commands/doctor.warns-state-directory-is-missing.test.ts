...
describe("doctor command", () => {
  ...
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "traversalai-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.TRAVERSALAI_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when TRAVERSALAI_GATEWAY_TOKEN is set", async () => {
    mockDoctorConfigSnapshot({
      config: {
        gateway: { mode: "local" },
      },
    ...
    process.env.TRAVERSALAI_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
