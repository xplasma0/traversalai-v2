...
export function normalizeCompatibilityConfigValues(cfg: TraversalAIConfig): {
  config: TraversalAIConfig;
  changes: string[];
} {
  const changes: string[] = [];
  let next: TraversalAIConfig = cfg;
  ...
  const seedMissingDefaultAccountsFromSingleAccountBase = () => {
    ...
    next = {
      ...next,
      channels: nextChannels as TraversalAIConfig["channels"],
    };
  };
  ...
  const normalizeBrowserSsrFPolicyAlias = () => {
    ...
    next = {
      ...next,
      browser: migratedBrowser as TraversalAIConfig["browser"],
    };
    changes.push(
