...
const SEARCH_TOOL = "https://xplasma0.github.io/traversalai-docs/mcp.SearchTraversalAI";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    ...
    if (isRich()) {
      ...
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('traversalai docs "your query"')}`);
    } else {
      runtime.log("Docs: https://xplasma0.github.io/traversalai-docs/");
      runtime.log(`Search: ${formatCliCommand('traversalai docs "your query"')}`);
    }
    return;
  }
