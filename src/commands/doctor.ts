...
function resolveMode(cfg: TraversalAIConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("TraversalAI doctor");

  const root = await resolveTraversalAIPackageRoot({
  ...
  let cfg: TraversalAIConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("traversalai configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("traversalai config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("traversalai setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("traversalai doctor --fix")}" to apply changes.`);
  }
