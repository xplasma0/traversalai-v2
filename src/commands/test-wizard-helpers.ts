...
export async function setupAuthTestEnv(
  ...
  process.env.TRAVERSALAI_AGENT_DIR = agentDir;
...
export function requireTraversalAIAgentDir(): string {
  const agentDir = process.env.TRAVERSALAI_AGENT_DIR;
  if (!agentDir) {
    throw new Error("TRAVERSALAI_AGENT_DIR not set");
  }
  return agentDir;
}
