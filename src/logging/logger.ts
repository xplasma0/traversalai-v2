...
import type { TraversalAIConfig } from "../config/types.js";
...
export const DEFAULT_LOG_DIR = resolvePreferredTraversalAITmpDir();
export const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "traversalai.log"); // legacy single-file path

const LOG_PREFIX = "traversalai";
...
function resolveSettings(): ResolvedSettings {
  let cfg: TraversalAIConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? readLoggingConfig();
  if (!cfg) {
    try {
      const loaded = requireConfig?.("../config/config.js") as
        | {
            loadConfig?: () => TraversalAIConfig;
          }
        | undefined;
  ...
  const defaultLevel =
    process.env.VITEST === "true" && process.env.TRAVERSALAI_TEST_FILE_LOG !== "1" ? "silent" : "info";
      ...
      if (nextBytes > settings.maxFileBytes) {
        if (!warnedAboutSizeCap) {
          ...
          process.stderr.write(
            `[traversalai] log file size cap reached; suppressing writes file=${settings.file} maxFileBytes=${settings.maxFileBytes}\n`,
          );
        }
        return;
      }
