...
function resolveGatewayPort(cfg: TraversalAIConfig, env: NodeJS.ProcessEnv): number {
  const envRaw = env.TRAVERSALAI_GATEWAY_PORT?.trim() || env.CLAWDBOT_GATEWAY_PORT?.trim();
  if (envRaw) {
...
function resolveAuth(cfg: TraversalAIConfig, env: NodeJS.ProcessEnv): ResolveAuthResult {
  ...
  const password =
    env.TRAVERSALAI_GATEWAY_PASSWORD?.trim() ||
